...
class ASLFoundationModel(nn.Module):
    ...
    def forward(
        ...
        if getattr(self, "enable_ctc", True):
            ...
            inter_ctc_log_probs = (
                self.inter_ctc_head(inter_h) if (inter_h is not None and getattr(self, "enable_inter_ctc", False)) else None
            )
        else:
            ctc_log_probs = None
...
def train_epoch_tpu(
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        def forward_and_losses(
            ...
            if getattr(args, "enable_inter_ctc", False) and inter_ctc_logits is not None and compute_ctc_now:
...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        try:
            ...
            if base_cache.startswith("/kaggle/working"):
                base_cache = _default_cache

            shared_cache = f"{base_cache}_0"
            if rank == 0:
                os.makedirs(shared_cache, exist_ok=True)
                xr.initialize_cache(shared_cache, readonly=False)
                print(f"[XLA] Compilation cache active: {shared_cache} (Primary writer rank 0)", flush=True)
            else:
                os.makedirs(shared_cache, exist_ok=True)
                try:
                    xr.initialize_cache(shared_cache, readonly=True)
                except Exception:
    ...
    if hasattr(args, "phase1_epochs") and args.phase1_epochs > 0:
        ...
        if IS_TPU:
            ...
            # Per-core batch 256 (2048 global physical batch) guarantees >= 8k samples/s throughput
            phase1_core_batch = getattr(args, "phase1_core_batch", 0) or max(256, target_core_batch)
...
def text_pretrain_loop(args, device, is_master, per_core_batch=None, accum_steps=1):
    ...
    # Enable double-buffered TPU infeed with batches_per_execution=2 (0% additional HBM)
    p1_bpe = min(2, max(1, getattr(args, "batches_per_execution", 2)))

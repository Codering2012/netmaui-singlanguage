...
class SyntheticASLStream:
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            yield {
                "feature": jnp.zeros((self.batch_size, self.max_len, 225), dtype=jnp.float32),
                "mask": jnp.ones((self.batch_size, self.max_len), dtype=jnp.bool_),
                "gloss_seq": jnp.ones((self.batch_size, self.text_len), dtype=jnp.int32),
                "gloss_len": jnp.full((self.batch_size,), self.text_len, dtype=jnp.int32),
                "chicago_seq": jnp.ones((self.batch_size, self.text_len), dtype=jnp.int32),
                "english_seq": jnp.ones((self.batch_size, self.text_len), dtype=jnp.int32),
            }
...
def main():
    ...
    model = ASLFoundationModel(
        vocab_size=17800,
        chicago_vocab_size=128,
        english_vocab_size=50257,
    ...
    print(f"[+] Model constructed: {len(model.trainable_variables)} trainable variables.")
    ...
    def loss_function(t_vars, nt_vars, feats, gloss_toks, chi_toks, eng_toks, t_lens, mask=None):
        ...
        # 1. Autoregressive Gloss Decoder Loss
        loss_dec = label_smoothed_ce(preds["dec_logits"], gloss_toks, num_classes=17800, smoothing=0.10)
        # 2. Sequence Length Prediction Loss
        loss_len = sequence_length_loss(preds["pred_len"], t_lens)
        # 3. Frame-Level CTC Loss
        loss_ctc = ctc_loss(preds["ctc_logits"], gloss_toks)
        # 4. Chicago Fingerspelling Projection Loss
        loss_chi = (
            label_smoothed_ce(preds["chicago_logits"], chi_toks, num_classes=128, smoothing=0.05)
            if preds["chicago_logits"] is not None
            else jnp.array(0.0)
        )
        # 5. English Translation Projection Loss
        loss_eng = (
            label_smoothed_ce(preds["english_logits"], eng_toks, num_classes=50257, smoothing=0.05)

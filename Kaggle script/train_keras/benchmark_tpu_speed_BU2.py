...
cache_dir = "/kaggle/working/jax_cache"
try:
...
def benchmark_phase1(num_devices: int, per_core_batch: int = 256, num_layers: int = 4, warmup_steps: int = 2, active_steps: int = 10):
...
def benchmark_phase2(num_devices: int, per_core_batch: int = 256, warmup_steps: int = 2, active_steps: int = 10):
    global_batch = num_devices * per_core_batch
    seq_len = 128
    d_model = 512
    vocab_size = 17800
...
def benchmark_phase3(num_devices: int, per_core_batch: int = 128, num_layers: int = 2, warmup_steps: int = 2, active_steps: int = 10):
    global_batch = num_devices * per_core_batch
    l_land = 384
    l_tok = 128
    d_model = 512
...
if __name__ == "__main__":
    ...
    print(f"[+] Detected {num_devices} JAX device(s): {devices}")

    if num_devices < 8:
        print("[!] Local environment detected: running lightweight mock benchmark.")
        benchmark_phase1(num_devices=num_devices, per_core_batch=2, warmup_steps=1, active_steps=3)
        benchmark_phase2(num_devices=num_devices, per_core_batch=2, warmup_steps=1, active_steps=3)
    else:
        # 1. Phase 1 (Target: >= 12,000 samples/s, 10-20s compile)
        c1, s1 = benchmark_phase1(num_devices=8, per_core_batch=256, num_layers=4, warmup_steps=2, active_steps=10)

        # 2. Phase 2 (Target: >= 6,000 seq/s, 10-20s compile)
        c2, s2 = benchmark_phase2(num_devices=8, per_core_batch=256, warmup_steps=2, active_steps=10)

        # 3. Phase 3 (Optional reference, 2-Layer Continuous Model)
        c3, s3 = benchmark_phase3(num_devices=8, per_core_batch=128, num_layers=2, warmup_steps=2, active_steps=5)
        ...
        print(f" Phase 3 (Conformer+Dec):   Compile = {c3:.2f}s | Throughput = {s3:,.1f} seq/s     (Reference: 2 Layers)")
        print("=" * 70)

                    ...
                    for k, v in batch.items():
                        if isinstance(v, torch.Tensor):
                            ...
                            try:
                                ...
                                # Graceful fallback if runtime does not support bfloat16 dlpack
                                jax_batch[k] = from_dlpack(t_cont.to(torch.float32))
        ...
        for s_path in assigned_shards:
            try:
                ...
                for r in records:
                    ...
                    # Pad to max_len preserving native dtype (e.g. bfloat16 for 2x RAM/PCIe bandwidth savings)
                    f_dtype = feats.dtype if feats.dtype in (torch.bfloat16, torch.float32) else torch.float32
                    if feats.ndim == 3:
                        padded_feats = torch.zeros((self.max_len, feats.shape[1], feats.shape[2]), dtype=f_dtype)
                        padded_feats[:t_clip] = feats[:t_clip].to(f_dtype)
                    else:
                        padded_feats = torch.zeros((self.max_len, feats.shape[-1]), dtype=f_dtype)
                        padded_feats[:t_clip] = feats[:t_clip].to(f_dtype)
                    ...
                    if phon is not None:
                        ...
                        p_dtype = phon.dtype if phon.dtype in (torch.bfloat16, torch.float32) else f_dtype
                        padded_phon = torch.zeros((self.max_len, 19), dtype=p_dtype)
                        p_clip = min(phon.shape[0], self.max_len)
                        padded_phon[:p_clip] = phon[:p_clip].to(p_dtype)
                    else:
                        padded_phon = torch.zeros((self.max_len, 19), dtype=f_dtype)

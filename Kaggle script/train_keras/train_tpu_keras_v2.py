...
def main():
    ...
    # Data Loader instantiated to calculate full dataset step budget
    ...
    def loss_fn(t_vars_all, nt_vars, feats, phon, roi, g_toks, c_toks, e_toks, t_lens, mask):
        ...
        l_chi = (
            ...
            if "chicago_logits" in preds and preds["chicago_logits"] is not None and c_toks is not None
            else ops.array(0.0)
        )
        l_eng = (
            label_smoothed_ce(
            ...
            if "english_logits" in preds and preds["english_logits"] is not None and e_toks is not None
            else ops.array(0.0)
        )
    ...
    # Data Loader already instantiated above
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch in loader:
            ...
            if step_in_epoch % args.log_freq == 0 or (args.max_steps > 0 and step_in_epoch >= args.max_steps):
                now = time.time()
                elapsed = now - log_timer
                steps_done = args.log_freq if (step_in_epoch % args.log_freq == 0) else (step_in_epoch % args.log_freq)
                speed = (args.batch_size * steps_done) / max(1e-4, elapsed)
                loss_val = float(tot_l)
                running_loss += loss_val
                l_dec_val, l_ctc_val, l_chi_val, l_eng_val = float(l_dec), float(l_ctc), float(l_chi), float(l_eng)
                print(
                    f"  [Epoch {epoch:03d} | Step {step_in_epoch:04d}] "
                    f"Loss: {loss_val:.4f} (Dec: {l_dec_val:.3f}, CTC: {l_ctc_val:.3f}, "
                    f"Chi: {l_chi_val:.3f}, Eng: {l_eng_val:.3f}) | "
                ...
                try:
                    with open(csv_path, "a", encoding="utf-8") as f:
                        f.write(
                            f"{epoch},{global_step},{loss_val:.4f},{l_dec_val:.4f},{l_ctc_val:.4f},"
                            f"{float(l_len):.4f},{l_chi_val:.4f},{l_eng_val:.4f},{speed:.1f},"
                            f"{now - t_train_start:.2f}\n"
                        )
                except Exception:
                    pass

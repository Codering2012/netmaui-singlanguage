...
def apply_rope(x, cos, sin, layout: str = "BLHD"):
    """
    Apply Rotary Position Embedding to input tensor x.
    x: (B, L, H, D) [default native layout] or (B, H, L, D) or (B, L, D)
    cos: (L, D // 2)
    sin: (L, D // 2)
    """
    d = ops.shape(x)[-1]
    ...
    shape_len = len(ops.shape(x))
    if shape_len == 4:
        if layout == "BLHD":
            seq_len = ops.shape(x)[1]
            c = ops.reshape(cos[:seq_len], (1, seq_len, 1, half))
            s = ops.reshape(sin[:seq_len], (1, seq_len, 1, half))
        else: # BHLD
            seq_len = ops.shape(x)[2]
            c = ops.reshape(cos[:seq_len], (1, 1, seq_len, half))
            s = ops.reshape(sin[:seq_len], (1, 1, seq_len, half))
    elif shape_len == 3:
        seq_len = ops.shape(x)[1]
        c = ops.reshape(cos[:seq_len], (1, seq_len, half))
        s = ops.reshape(sin[:seq_len], (1, seq_len, half))
    else:

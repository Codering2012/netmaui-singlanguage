...
class BiMamba2SSMBlock(layers.Layer):
    ...
    def _ssd_pass(self, x, dt, B, C, a_val, is_reverse: bool = False):
        """
        ...
        decay_diff = ops.where(causal_tril, decay_diff, -1e4)
        decay_mask = ops.exp(decay_diff)  # (B, H, T, T)

        # SSD Gram matrix: CB = (C @ B^T) / sqrt(d_state)
        scale = 1.0 / math.sqrt(self.d_state)
        cb = ops.matmul(C_t, ops.transpose(B_t, (0, 1, 3, 2))) * scale  # (B, H, T, T)
    ...
    def call(self, u, mask: Optional[any] = None):
        ...
        # 2. Forward SSD
        y_fwd = self._ssd_pass(x_4d, dt_raw, B_4d, C_4d, a_val, is_reverse=False)

        # 3. Backward SSD (if not strictly causal)
        if not self.is_causal:
            y_bwd = self._ssd_pass(x_4d, dt_raw, B_4d, C_4d, a_val, is_reverse=True)
            y = (y_fwd + y_bwd) * 0.5
        else:
            y = y_fwd

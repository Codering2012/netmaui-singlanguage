...
def run_phase1_real_data(num_devices: int = 8, per_core_batch: int = 256, num_layers: int = 4, num_steps: int = 20):
    ...
    csv_path = "/kaggle/input/datasets/thedevastator/unlock-the-power-of-english-asl-with-aslg-pc12-c/train.csv"
    if not os.path.exists(csv_path):
        print(f"[!] Warning: {csv_path} not found, generating in-memory mock text corpus.")
    ...
    # Exclude step 0 warmup
    avg_dt = sum(step_latencies[2:]) / len(step_latencies[2:])
    avg_throughput = global_batch / avg_dt

    print("=" * 80)
    print(f"[+] PHASE 1 REAL DATA RESULT: {avg_throughput:,.1f} samples/s (Target: >= 12,000 samples/s)")
    print(f"[+] STATUS: {'PASSED' if avg_throughput >= 12000 else 'FAILED'}")
    print("=" * 80)
...
if __name__ == "__main__":
    devices = jax.devices()
    num_dev = len(devices)
    run_phase1_real_data(num_devices=num_dev, per_core_batch=256, num_steps=25)

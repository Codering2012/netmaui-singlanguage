...
class ASLFoundationModel(keras.Model):
    ...
    def build(self, input_shape):
        ...
        if self.enable_aux_decoders:
            ...
            if not self.tie_word_embeddings:
                ...
                self.english_head = None
        else:
            self.chicago_embed = None
            self.chicago_head = None
            self.english_embed = None
            self.english_head = None
    ...
    def call(
        ...
        # Flatten 4D landmark coordinates (B, L, K, C) to 3D (B, L, K*C) if needed
        if ops.ndim(features) == 4:
            features = ops.reshape(features, (ops.shape(features)[0], ops.shape(features)[1], -1))
        cur_ch = ops.shape(features)[-1]
        if cur_ch > self.in_channels:
            features = features[..., :self.in_channels]
        elif cur_ch < self.in_channels:
            features = ops.pad(features, [[0, 0], [0, 0], [0, self.in_channels - cur_ch]])
        ...
        chicago_logits = None
        if self.enable_aux_decoders and chicago_seq is not None:
            h_chi = self._decode_tokens(self.chicago_embed, chicago_seq, enc_out, mask)
            if self.tie_word_embeddings:
                chicago_logits = ops.matmul(h_chi, ops.transpose(self.chicago_embed.embeddings))
            else:
                chicago_logits = self.chicago_head(h_chi)

        # 6. English Translation Logits
        english_logits = None
        if self.enable_aux_decoders and english_seq is not None:
            h_eng = self._decode_tokens(self.english_embed, english_seq, enc_out, mask)
            if self.tie_word_embeddings:
                english_logits = ops.matmul(h_eng, ops.transpose(self.english_embed.embeddings))
            else:

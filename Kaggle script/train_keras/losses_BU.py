...
def label_smoothed_ce(
    ...
    Closed-form O(1) memory: eliminates allocating (B, L, V) one-hot matrices.
    """
    logits_f32 = ops.cast(logits, "float32")
    log_probs = ops.log_softmax(logits_f32, axis=-1)

    targets_exp = ops.expand_dims(targets, axis=-1)
    target_log_p = ops.squeeze(ops.take_along_axis(log_probs, targets_exp, axis=-1), axis=-1)
    mean_log_p = ops.mean(log_probs, axis=-1)

    token_loss = - ((1.0 - smoothing) * target_log_p + smoothing * mean_log_p)
...
class HomoscedasticLossWrapper(layers.Layer):
    ...
    """
    def __init__(self, task_names: Optional[list] = None, **kwargs):

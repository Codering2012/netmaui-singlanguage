...
def run_phase1_real_data(num_devices: int = 8, per_core_batch: int = 256, num_layers: int = 4, num_steps: int = 20):
    ...
    target_samples_s = 1000 if num_devices == 1 else 12000
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    if not is_tpu:
        print("[!] Local CPU environment detected: running lightweight mock verification.")
        run_phase1_real_data(num_devices=num_dev, per_core_batch=2, num_layers=1, num_steps=2)
    else:

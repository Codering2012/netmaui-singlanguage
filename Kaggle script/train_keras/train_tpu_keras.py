...
def parse_args():
    ...
    parser.add_argument("--warmup-steps", type=int, default=0, help="Warmup steps (default: 0, full dataset training immediately at peak lr)")
    parser.add_argument("--decay-steps", type=int, default=0, help="Total decay steps across full dataset (0 = auto-calculate from dataset)")
    parser.add_argument("--min-lr", type=float, default=1e-6, help="Minimum learning rate at end of cosine decay")
    ...
    parser.add_argument("--tie-embeddings", action="store_true", default=False, help="Tie input embedding and lm_head projection weights")
    parser.add_argument("--enable-aux-decoders", action="store_true", default=False, help="Enable Chicago & English auxiliary decoders (Phase 1/multitask)")
    parser.add_argument("--grad-accum-steps", type=int, default=1, help="Gradient accumulation steps via optax.MultiSteps (default 1)")
    parser.add_argument("--poly1-epsilon", type=float, default=0.0, help="PolyLoss epsilon-1 factor for zero-intermediate focal loss")
...
def build_data_loader(args):
    ...
    if has_shards:
        # 1. First check if Preprocessor V4 shards exist in data_dir
        import glob
        v4_files = glob.glob(os.path.join(args.data_dir, "shard_*.pt")) + glob.glob(os.path.join(args.data_dir, "**", "shard_*.pt"), recursive=True)
        if v4_files:
            try:
                from train_keras.dataloader import V4ShardedDataset, fast_v4_collate_fn, JAXDataLoader
                from torch.utils.data import DataLoader
                print(f"[*] Initializing V4ShardedDataset (Preprocessor V4) from: {args.data_dir}")
                ...
                print(f"[!] Warning: Could not instantiate V4ShardedDataset ({e}). Trying ASLStreamedDataset...")

        # 2. Fall back to legacy ASLStreamedDataset
        try:
...
def save_epoch_loss_curves_png(epoch: int, csv_path: str = "training_metrics.csv", out_path: str = "loss_curves.png"):
    """
    Renders high-resolution multi-panel loss and throughput curves using headless matplotlib.
    """
    ...
    # Normalize column names across various logging schemas
    if "loss" not in df.columns:
        if "total_loss" in df.columns:
            df["loss"] = df["total_loss"]
        elif "train_loss" in df.columns:
            df["loss"] = df["train_loss"]

    if "samples_per_sec" not in df.columns and "sps" in df.columns:
        df["samples_per_sec"] = df["sps"]
    ...
    # 1. Panel 1: Total Loss
    ax1 = axes[0]
    ax1.plot(steps, df["loss"].values, label="Total Loss (Raw)", alpha=0.35, color="#1f77b4", linewidth=1.0)
    if len(df) >= 3:
        smooth_loss = df["loss"].rolling(10, min_periods=1).mean()
        ax1.plot(steps, smooth_loss.values, label="Total Loss (Smoothed)", color="#0055aa", linewidth=2.0)
    ax1.set_title(f"Total Loss Trajectory (Epoch {epoch})", fontsize=12, fontweight="bold")
    ax1.set_xlabel("Global Step", fontsize=10)
    ax1.set_ylabel("Loss", fontsize=10)
    ax1.grid(True, linestyle="--", alpha=0.5)
    ax1.legend(loc="upper right", fontsize=9)

    # 2. Panel 2: Multi-Task Sub-Loss Breakdown
    ax2 = axes[1]
    loss_keys = [
        (["loss_dec", "decoder_loss"], "Gloss Decoder", "#2ca02c"),
        (["loss_ctc", "ctc_loss"], "CTC Loss", "#d62728"),
        (["loss_chi", "chicago_loss"], "Chicago FS", "#9467bd"),
        (["loss_eng", "english_loss"], "English Translation", "#ff7f0e"),
    ]
    for col_candidates, lbl, col_color in loss_keys:
        for col_name in col_candidates:
            if col_name in df.columns and (df[col_name] > 0).any() and not df[col_name].isnull().all():
                vals = df[col_name].values
                if len(df) >= 3:
                    vals = df[col_name].rolling(5, min_periods=1).mean().values
                ax2.plot(steps, vals, label=lbl, color=col_color, linewidth=1.5)
                break

    ax2.set_title("Multi-Task Auxiliary Losses", fontsize=12, fontweight="bold")
    ax2.set_xlabel("Global Step", fontsize=10)
    ax2.set_ylabel("Sub-Loss", fontsize=10)
    ax2.grid(True, linestyle="--", alpha=0.5)
    ax2.legend(loc="upper right", fontsize=9)

    # 3. Panel 3: Throughput (samples/s)
    ax3 = axes[2]
    if "samples_per_sec" in df.columns:
        ax3.plot(steps, df["samples_per_sec"].values, label="Throughput (samples/s)", alpha=0.35, color="#17becf", linewidth=1.0)
        if len(df) >= 3:
            smooth_sps = df["samples_per_sec"].rolling(10, min_periods=1).mean()
            ax3.plot(steps, smooth_sps.values, label="Throughput (Smoothed)", color="#0e8088", linewidth=2.0)
    ax3.set_title("TPU Hardware Throughput", fontsize=12, fontweight="bold")
    ax3.set_xlabel("Global Step", fontsize=10)
    ax3.set_ylabel("Samples / Sec", fontsize=10)
    ...
    plt.close(fig)
    print(f"[+] [Matplotlib] Saved epoch {epoch} loss/throughput curves to: {out_path}", flush=True)
...
def main():
    ...
    model = ASLFoundationModel(
        ...
        enable_aux_decoders=(args.phase == 1 or args.enable_aux_decoders),
    )
    ...
    if args.phase == 2 and not args.enable_aux_decoders:
    ...
    print(f"[+] Model constructed: {len(model.trainable_variables)} trainable variables (Phase {args.phase}, TieEmbeddings={args.tie_embeddings}, AuxDecoders={args.phase == 1 or args.enable_aux_decoders}).")
    ...
    # 1. Instantiate Data Pipeline to calculate full dataset step budget
    loader = build_data_loader(args)

    # Calculate full dataset step count
    steps_per_epoch = 1000  # Default fallback
    if hasattr(loader, "pt_loader") and hasattr(loader.pt_loader, "__len__"):
        try:
            steps_per_epoch = len(loader.pt_loader)
        except TypeError:
            pass
    ...
    if steps_per_epoch == 1000 and hasattr(loader, "pt_loader") and hasattr(loader.pt_loader, "dataset"):
        ds = loader.pt_loader.dataset
        if hasattr(ds, "shard_files") and ds.shard_files:
            est_samples = len(ds.shard_files) * getattr(ds, "shard_size", 1000)
            steps_per_epoch = max(1, est_samples // args.batch_size)

    total_dataset_steps = args.decay_steps if args.decay_steps > 0 else (steps_per_epoch * args.epochs)
    print(f"[+] Full Dataset Scheduled: {total_dataset_steps:,} total steps ({steps_per_epoch:,} steps/epoch x {args.epochs} epochs, Warmup: {args.warmup_steps})")
    ...
    if args.warmup_steps > 0:
        lr_schedule = optax.warmup_cosine_decay_schedule(
            ...
            decay_steps=total_dataset_steps,
            end_value=args.min_lr,
        )
    else:
        # Full dataset cosine decay without warmup: starts at peak learning rate immediately
        lr_schedule = optax.cosine_decay_schedule(
            init_value=args.lr,
            decay_steps=total_dataset_steps,
            alpha=max(1e-9, args.min_lr) / max(1e-8, args.lr),
        )
    base_tx = optax.chain(
        optax.zero_nans(),
    ...
    if args.grad_accum_steps > 1:
        tx = optax.MultiSteps(base_tx, every_k_schedule=args.grad_accum_steps)
        print(f"[+] Optax MultiSteps Gradient Accumulation enabled: {args.grad_accum_steps} steps.")
    else:
        tx = base_tx
    ...
    def loss_function(t_vars, nt_vars, feats, gloss_toks, chi_toks, eng_toks, t_lens, mask=None):
        if args.phase == 2 and not args.enable_aux_decoders:
            ...
            loss_dec = label_smoothed_ce(
                preds["dec_logits"],
                gloss_toks,
                num_classes=model.vocab_size,
                smoothing=0.10,
                poly1_epsilon=args.poly1_epsilon,
            )
            loss_len = sequence_length_loss(preds["pred_len"], t_lens)
            loss_ctc = ctc_loss(preds["ctc_logits"], gloss_toks, target_lengths=t_lens)
            ...
            loss_chi = (
                label_smoothed_ce(
                    preds["chicago_logits"],
                    chi_toks,
                    num_classes=model.chicago_vocab_size,
                    smoothing=0.05,
                    poly1_epsilon=args.poly1_epsilon,
                )
                if "chicago_logits" in preds and preds["chicago_logits"] is not None and chi_toks is not None
                else jnp.array(0.0)
            )
            loss_eng = (
                label_smoothed_ce(
                    preds["english_logits"],
                    eng_toks,
                    num_classes=model.english_vocab_size,
                    smoothing=0.05,
                    poly1_epsilon=args.poly1_epsilon,
                )
                if "english_logits" in preds and preds["english_logits"] is not None and eng_toks is not None
                else jnp.array(0.0)
            )
    ...
    # Data pipeline already instantiated above
    # Main Training Loop
    total_samples = 0
    ...
    if not os.path.exists(csv_path):
        try:
            with open(csv_path, "w", encoding="utf-8") as f:
                f.write("epoch,step,loss,loss_dec,loss_ctc,loss_chi,loss_eng,samples_per_sec,elapsed_sec\n")
        except Exception:
            pass
    ...
    for epoch in range(start_epoch, args.epochs + 1):
            ...
            if step_in_epoch % args.log_freq == 0 or (args.max_steps and step_in_epoch >= args.max_steps):
                ...
                try:
                    with open(csv_path, "a", encoding="utf-8") as f:
                        f.write(f"{epoch},{global_step},{loss_val:.4f},{float(l_dec):.4f},{float(l_ctc):.4f},{float(l_chi):.4f},{float(l_eng):.4f},{sps:.2f},{now - t_train_start:.2f}\n")
                except Exception:
                    pass
                log_timer = time.time()
        ...
        # Plot loss curves via Matplotlib at end of every epoch
        loss_curves_path = os.path.join(args.save_dir, "loss_curves.png")
        try:
            save_epoch_loss_curves_png(epoch, csv_path=csv_path, out_path=loss_curves_path)
            if epoch % args.checkpoint_freq == 0 or epoch == args.epochs:
                epoch_snapshot_path = os.path.join(args.save_dir, f"loss_curves_epoch_{epoch:03d}.png")
                save_epoch_loss_curves_png(epoch, csv_path=csv_path, out_path=epoch_snapshot_path)
        except Exception as e:
            print(f"[!] Warning: Matplotlib plot generation failed: {e}", flush=True)
    ...
    # Final Matplotlib loss curve export
    final_curves_path = os.path.join(args.save_dir, "loss_curves.png")
    try:
        save_epoch_loss_curves_png(args.epochs, csv_path=csv_path, out_path=final_curves_path)
    except Exception:
        pass
    print("\n" + "=" * 65)

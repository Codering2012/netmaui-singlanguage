    ...
    def _load_records_metadata(self) -> None:
        ...
        if not temp_metadata:
            ...
            if not cache_path.exists():
                lock_path = cache_dir / f"{cache_name}.lock"
                if lock_path.exists():
                    is_stale = False
                    try:
                    ...
                    if is_stale:
                        print(f"[Worker {self.worker_idx}] Removing stale dataset lock file: {lock_path}", flush=True)
                        try:
                            lock_path.unlink(missing_ok=True)
                        except Exception:
                            pass
                    else:
                        print(f"[Worker {self.worker_idx}] Waiting for metadata cache build to complete...", flush=True)
                        start_wait = time.time()
                        while not cache_path.exists() and lock_path.exists() and (time.time() - start_wait < 300):
                            time.sleep(1.0)
    ...
    def __iter__(self):
        ...
        for shard_path in shards:
            try:
                ...
                for rec in items:
                    if isinstance(rec, dict):
                        if self.split == "train" and len(buffer) >= self.shuffle_buffer_size:
    ...
    def _process_record(self, shard_path: Path, rec: dict) -> Optional[Dict[str, torch.Tensor]]:
        ...
        if T > 0:
            ...
            if T > self.max_len:
                ...
                padded_frame_indices[:self.max_len] = np.arange(self.max_len, dtype=np.float32)
                    
            T_cap = min(T, self.max_len)
            feat_slice = feat_arr[:T_cap]
            
            lh_nz = (feat_slice[:, 0:21, :2] != 0).astype(np.float32)
            rh_nz = (feat_slice[:, 21:42, :2] != 0).astype(np.float32)
            
            feat_slice[:, 0:21, :2] = (feat_slice[:, 0:21, :2] - feat_slice[:, 0:1, :2]) * lh_nz
            feat_slice[:, 21:42, :2] = (feat_slice[:, 21:42, :2] - feat_slice[:, 21:22, :2]) * rh_nz

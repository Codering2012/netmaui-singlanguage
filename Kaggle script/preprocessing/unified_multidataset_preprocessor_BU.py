...
def compute_kinematics_and_phonology(pos: np.ndarray, is_static: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # 1. Reference-Part Normalization
    mid_shoulder = (pos[:, 42:43, :3] + pos[:, 43:44, :3]) * 0.5
    shoulder_dist = np.linalg.norm(pos[:, 42:43, :3] - pos[:, 43:44, :3], axis=-1, keepdims=True)
    scale = np.where(shoulder_dist > 1e-4, shoulder_dist, 1.0)
    norm_pos = (pos - mid_shoulder) / scale
...
def _init_worker_model():
    ...
    if _RTMLIB_AVAILABLE and _GLOBAL_WB is None:
        try:
            _GLOBAL_WB = Wholebody(to_openpose=False, mode='balanced', backend='onnxruntime', device='cpu')
        except Exception:
            _GLOBAL_WB = None
...
def _worker_video(task_arg: Tuple[str, str, str, str, str, str, int, int]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Ensure at least 40 frames
        if len(clip_f) < 40:
            idx_interp = np.linspace(0, len(clip_f) - 1, 40, dtype=int)
            clip_f = [clip_f[i] for i in idx_interp]
        elif len(clip_f) > 64:
...
class ZipShardWriter:
    def __init__(self, zip_path: Union[str, Path], compress_level: int = 6):
...
def main():
    ...
    parser.add_argument("--datasets", nargs="+", default=["how2sign", "alphabet", "numbers", "citizen", "wlasl"])
    ...
    writer = ZipShardWriter(args.output_zip, compress_level=6)
    t_start = time.time()
    total_stats = {}
    ...
    if "how2sign" in args.datasets:
        ...
        if feat_root.exists():
            log_msg("\n>>> [1/5] PREPROCESSING HOW2SIGN (35,000 continuous sentences)...")
            # Build text map
            text_map = {}
    ...
    if "alphabet" in args.datasets:
        ...
        if train_dir.exists() and _RTMLIB_AVAILABLE:
            log_msg("\n>>> [2/5] PREPROCESSING ASL ALPHABET (87,000 images, T=40 static hand)...")
            ...
            t0 = time.time()
            alpha_count = 0
            with mp.Pool(processes=num_workers, initializer=_init_worker_model) as pool:
                for res in pool.imap_unordered(_worker_image, alpha_tasks, chunksize=32):
                    if res is not None:
    ...
    if "numbers" in args.datasets:
        ...
        if num_root.exists() and _RTMLIB_AVAILABLE:
            log_msg("\n>>> [3/5] PREPROCESSING SYNTHETIC NUMBERS (T=40 static hand)...")
            num_tasks = []
            ...
            num_count = 0
            with mp.Pool(processes=num_workers, initializer=_init_worker_model) as pool:
                for res in pool.imap_unordered(_worker_image, num_tasks, chunksize=32):
                    if res is not None:
                        sp = res["split"]
                        buffers[sp].append(res)
                        num_count += 1
    ...
    if "wlasl" in args.datasets:
        ...
        if json_p.exists() and vid_dir.exists() and _RTMLIB_AVAILABLE:
            log_msg("\n>>> [4/5] PREPROCESSING WLASL (11,980 videos, T >= 40)...")
    ...
    if "citizen" in args.datasets:
        ...
        if vid_dir.exists() and splits_dir.exists() and _RTMLIB_AVAILABLE:
            log_msg("\n>>> [5/5] PREPROCESSING ASL CITIZEN (83,399 videos, T >= 40)...")
            cit_tasks = []
            for split in ["train", "val", "test"]:
    ...
    manifest = {
        ...
        "compressed_zip_bytes": Path(args.output_zip).stat().st_size if Path(args.output_zip).exists() else 0,
    ...
    final_sz = Path(args.output_zip).stat().st_size / (1024 * 1024) if Path(args.output_zip).exists() else 0

...
def canonicalize_handedness(
    ...
    """
    lh_valid = val_mask[:, 0:21].any(axis=-1).mean()
    rh_valid = val_mask[:, 21:42].any(axis=-1).mean()
    ...
    lh_pts = canonical[:, 0:21, :].copy()
    rh_pts = canonical[:, 21:42, :].copy()
    canonical[:, 0:21, :] = rh_pts
    canonical[:, 21:42, :] = lh_pts

    lh_m = c_mask[:, 0:21].copy()
    rh_m = c_mask[:, 21:42].copy()
    c_mask[:, 0:21] = rh_m
    c_mask[:, 21:42] = lh_m

    # Swap Left Shoulder (42) and Right Shoulder (43)
    l_sh = canonical[:, 42, :].copy()
    r_sh = canonical[:, 43, :].copy()
    canonical[:, 42, :] = r_sh
    canonical[:, 43, :] = l_sh

    l_sh_m = c_mask[:, 42].copy()
    r_sh_m = c_mask[:, 43].copy()
    c_mask[:, 42] = r_sh_m
    c_mask[:, 43] = l_sh_m

    # Swap Left Hip (44) and Right Hip (45)
    l_hip = canonical[:, 44, :].copy()
    r_hip = canonical[:, 45, :].copy()
    canonical[:, 44, :] = r_hip
    canonical[:, 45, :] = l_hip

    l_hip_m = c_mask[:, 44].copy()
    r_hip_m = c_mask[:, 45].copy()
    c_mask[:, 44] = r_hip_m
    c_mask[:, 45] = l_hip_m

    # Swap Left Elbow (46) and Right Elbow (47)
    l_el = canonical[:, 46, :].copy()
    r_el = canonical[:, 47, :].copy()
    canonical[:, 46, :] = r_el
    canonical[:, 47, :] = l_el

    l_el_m = c_mask[:, 46].copy()
    r_el_m = c_mask[:, 47].copy()
    c_mask[:, 46] = r_el_m
    c_mask[:, 47] = l_el_m
        ...
        if self.rtmw_model is not None:
            ...
            try:
                ...
                if results.pose_landmarks:
                    # Pose 42..47: L_Shoulder=11, R_Shoulder=12, L_Hip=23, R_Hip=24, L_Elbow=13, R_Elbow=14
                ...
                if results.face_landmarks:
                    # 12 representative face landmarks (nose, midline, eyes, mouth corners, forehead)

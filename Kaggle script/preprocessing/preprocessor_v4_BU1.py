...
# ==============================================================================
# 14 Face points: Outer eyes, eyebrows, nose, mouth corners, lips
RTMW_FACE_14 = [23, 31, 37, 39, 41, 44, 46, 48, 52, 55, 74, 77, 85, 88]
# 4 Pose points: Left Shoulder=5, Right Shoulder=6, Left Elbow=7, Right Elbow=8
RTMW_POSE_4 = [5, 6, 7, 8]
# 21 Left Hand points
...
class UpperBodyTrackerEMA:
    ...
    def compute_raw_box(
        ...
        # Indices 14 and 15 correspond to Left Shoulder and Right Shoulder
        l_sh = landmarks[14]
        r_sh = landmarks[15]
...
class ImageEnhancer:
    ...
    unsharp masking for fine finger boundary recovery, and blur estimation.
    """
    ...
    def enhance_frame(self, rgb_img: np.ndarray, apply_deblur: bool = True) -> np.ndarray:
        """
        Enhances frame via CLAHE + Unsharp Masking to recover motion-blurred finger edges.
        """
        ...
        try:
            ...
            # 2. Adaptive Unsharp Masking for motion blur mitigation
            if apply_deblur and self.unsharp_strength > 0.0:
    ...
    @staticmethod
    def estimate_blur(rgb_img: np.ndarray) -> float:
        """Returns Laplacian variance (higher = sharper, <50 = blurry)."""
        try:
            gray = cv2.cvtColor(rgb_img, cv2.COLOR_RGB2GRAY)
...
def reference_part_normalize(landmarks: np.ndarray, val_mask: np.ndarray) -> Tuple[np.ndarray, float]:
    ...
    Applies Reference-Part Normalization to [T, 60, 3] landmarks:
      - Anchors (0,0,0) to mid-shoulder point (average of Left Shoulder=14, Right Shoulder=15)
    ...
    if both_valid.any():
        ...
        hand_pts = landmarks[:, 18:60, :3]
        hand_mask = val_mask[:, 18:60]
        if hand_mask.any():
...
def compute_9d_kinematics(landmarks_seq: np.ndarray) -> np.ndarray:
    """
    ...
    vel = np.zeros_like(landmarks_seq)
    if t_len > 1:
        vel[1:-1] = (landmarks_seq[2:] - landmarks_seq[:-2]) * 0.5
        vel[0] = landmarks_seq[1] - landmarks_seq[0]
        vel[-1] = landmarks_seq[-1] - landmarks_seq[-2]

    acc = np.zeros_like(landmarks_seq)
    if t_len > 2:
        acc[1:-1] = landmarks_seq[2:] - 2.0 * landmarks_seq[1:-1] + landmarks_seq[:-2]
        acc[0] = acc[1]
        acc[-1] = acc[-2]

    kinematics_9d = np.concatenate([landmarks_seq, vel, acc], axis=-1).astype(np.float32)
    return kinematics_9d


def interpolate_missing_hand_landmarks(landmarks_seq: np.ndarray, valid_mask: np.ndarray) -> np.ndarray:
    """Smoothly interpolates landmark trajectories during dropped hand detections."""
    t_len, num_pts, _ = landmarks_seq.shape
    if t_len <= 1:
    ...
    for pt_idx in range(num_pts):
        ...
        elif len(valid_t) < t_len:
            for dim in range(3):
                valid_vals = cleaned_seq[valid_t, pt_idx, dim]
                interpolated = np.interp(time_indices, valid_t, valid_vals)
                cleaned_seq[:, pt_idx, dim] = interpolated
...
def compute_19d_phonology(landmarks_seq: np.ndarray, val_mask: np.ndarray) -> np.ndarray:
    ...
    # Hand point indices:
    # LH: 18=wrist, 22=thumb tip, 23=index MCP, 26=index tip, 30=mid tip, 34=ring tip, 35=pinky MCP, 38=pinky tip
    # RH: 39=wrist, 43=thumb tip, 44=index MCP, 47=index tip, 51=mid tip, 55=ring tip, 56=pinky MCP, 59=pinky tip
    lh_w, lh_idx_mcp, lh_pky_mcp = 18, 23, 35
    rh_w, rh_idx_mcp, rh_pky_mcp = 39, 44, 56
    lh_tips = [22, 26, 30, 34, 38]
    rh_tips = [43, 47, 51, 55, 59]

    # Face centroid from face points (indices 0..13)
    face_centroid = np.mean(pos[:, :14, :3], axis=1, keepdims=True)  # [T, 1, 3]
    ...
    # 4. Finger Curl / Aperture (10 Dims: 5 LH + 5 RH)
    lh_wrist_expanded = pos[:, lh_w:lh_w+1, :3]  # [T, 1, 3]
    lh_curl = np.linalg.norm(pos[:, lh_tips, :3] - lh_wrist_expanded, axis=-1)  # [T, 5]

    rh_wrist_expanded = pos[:, rh_w:rh_w+1, :3]  # [T, 1, 3]
    rh_curl = np.linalg.norm(pos[:, rh_tips, :3] - rh_wrist_expanded, axis=-1)  # [T, 5]

    phonology = np.concatenate(
        [lh_normal, rh_normal, bimanual_sync, lh_face_dist, rh_face_dist, lh_curl, rh_curl],
        axis=-1
...
def canonicalize_handedness(
    ...
    """
    lh_valid = val_mask[:, 18:39].any(axis=-1).mean()
    rh_valid = val_mask[:, 39:60].any(axis=-1).mean()

    # Left-dominant condition: LH is active while RH is virtually absent
    is_left_dominant = (lh_valid > 0.40 and rh_valid < 0.15)
    if not is_left_dominant:
        return landmarks_seq, val_mask, False

    canonical = landmarks_seq.copy()
    c_mask = val_mask.copy()
    # Mirror X coordinates
    canonical[:, :, 0] = -canonical[:, :, 0]

    # Swap Left Hand (18..38) and Right Hand (39..59)
    lh_pts = canonical[:, 18:39, :].copy()
    rh_pts = canonical[:, 39:60, :].copy()
    canonical[:, 18:39, :] = rh_pts
    canonical[:, 39:60, :] = lh_pts

    lh_m = c_mask[:, 18:39].copy()
    rh_m = c_mask[:, 39:60].copy()
    c_mask[:, 18:39] = rh_m
    c_mask[:, 39:60] = lh_m

    # Swap Left Shoulder (14) and Right Shoulder (15)
    l_sh = canonical[:, 14, :].copy()
    r_sh = canonical[:, 15, :].copy()
    canonical[:, 14, :] = r_sh
    canonical[:, 15, :] = l_sh

    l_sh_m = c_mask[:, 14].copy()
    r_sh_m = c_mask[:, 15].copy()
    c_mask[:, 14] = r_sh_m
    c_mask[:, 15] = l_sh_m

    # Swap Left Elbow (16) and Right Elbow (17)
    l_el = canonical[:, 16, :].copy()
    r_el = canonical[:, 17, :].copy()
    canonical[:, 16, :] = r_el
    canonical[:, 17, :] = l_el

    l_el_m = c_mask[:, 16].copy()
    r_el_m = c_mask[:, 17].copy()
    c_mask[:, 16] = r_el_m
    c_mask[:, 17] = l_el_m
...
def compute_sample_quality(
    ...
    lh_present = val_mask[:, 18:39].any(axis=-1).mean() if val_mask.shape[1] >= 39 else 0.0
    rh_present = val_mask[:, 39:60].any(axis=-1).mean() if val_mask.shape[1] >= 60 else 0.0
...
class VideoPreprocessorV4:
    ...
    def extract_landmarks_from_rgb(self, rgb_frame: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
        """Extracts 60 canonical keypoints from a single RGB frame."""
        ...
        if self.rtmw_model is not None:
            ...
            try:
                ...
                if results.face_landmarks:
                    face_pts = [[lm.x, lm.y, lm.z] for lm in results.face_landmarks.landmark[:14]]
                    lm_60[:14] = np.array(face_pts, dtype=np.float32)
                    val_60[:14] = True
                    pts_found += 14
    ...
    def extract_from_video(
        ...
        while cap.isOpened():
            ...
            if frame_idx >= next_sample_idx:
                ...
                rgb_enhanced = self.enhancer.enhance_frame(rgb_frame)
                blur_scores.append(self.enhancer.estimate_blur(rgb_frame))
                ...
                if include_hand_crop:
                    # Pick dominant hand: right hand if valid, else left hand
                    if val_60[39:60].sum() >= 5:
                        h_crop = self.roi_tracker.extract_hand_crop(rgb_frame, lm_60[39:60])
                    elif val_60[18:39].sum() >= 5:
                        h_crop = self.roi_tracker.extract_hand_crop(rgb_frame, lm_60[18:39])
                    else:
                        h_crop = None
                    if h_crop is None:
    ...
    def extract_from_image(
        ...
        rgb_frame = cv2.cvtColor(bgr_frame, cv2.COLOR_BGR2RGB)
        rgb_enhanced = self.enhancer.enhance_frame(rgb_frame)
        blur_val = self.enhancer.estimate_blur(rgb_frame)
...
def _init_multiprocessing_worker(backend: str, target_roi_size: int, hand_crop_size: int):
    ...
    try:
        if item_type == "video":
            res = _GLOBAL_WORKER_PROCESSOR.extract_from_video(
                item_path, max_frames=max_len, include_roi=include_roi, include_hand_crop=include_hand_crop
            )
        else:
            res = _GLOBAL_WORKER_PROCESSOR.extract_from_image(
                item_path, include_roi=include_roi, include_hand_crop=include_hand_crop
            )
...
# ==============================================================================
#  8. PRODUCTION SHARD & METADATA WRITER (Phase 1-3 Conformance)
# ==============================================================================
...
def main():
    ...
    if args.image_dir and Path(args.image_dir).exists():
        ...
        for img_path in image_paths:
            ...
            meta_info = label_meta.get(p_obj.stem, label_meta.get(p_obj.name, {}))
            if not meta_info:
    ...
    if args.num_workers > 1:
        ...
        _init_multiprocessing_worker(args.backend, 256, 128)

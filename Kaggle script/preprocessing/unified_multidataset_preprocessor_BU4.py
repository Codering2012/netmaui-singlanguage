...
def compute_kinematics_and_phonology(pos: np.ndarray, is_static: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # 1. Hierarchical Anatomical Reference-Part Normalization
    pos_max = np.nanmax(np.abs(pos[:, :, :2]))
    pos_scaled = (pos / pos_max) if pos_max > 5.0 else pos.copy()

    mid_shoulder = (pos_scaled[:, 42:43, :3] + pos_scaled[:, 43:44, :3]) * 0.5 # [T, 1, 3]
    shoulder_dist = np.linalg.norm(pos_scaled[:, 42:43, :3] - pos_scaled[:, 43:44, :3], axis=-1, keepdims=True) # [T, 1, 1]
    
    # Level 1: Full-body (valid shoulders)
    sh_present = np.all(pos_scaled[:, 42:44, :2] > 0, axis=(1, 2))[:, np.newaxis, np.newaxis] # [T, 1, 1]
    has_shoulders = (shoulder_dist > 0.05) & sh_present
    
    # Level 2: Face fallback
    face_kpts = pos_scaled[:, 48:60, :3] # [T, 12, 3]
    has_face = np.mean(np.linalg.norm(face_kpts, axis=-1), axis=1)[:, np.newaxis, np.newaxis] > 0.01 # [T, 1, 1]
    face_cen = np.mean(face_kpts, axis=1, keepdims=True) # [T, 1, 3]
    face_span = np.max(np.linalg.norm(face_kpts - face_cen, axis=-1, keepdims=True), axis=1, keepdims=True) * 2.0 # [T, 1, 1]
    
    # Level 3: Hand-only fallback (e.g. Alphabet, Numbers crops)
    rh_kpts = pos_scaled[:, 21:42, :3]
    lh_kpts = pos_scaled[:, 0:21, :3]
    rh_valid = np.mean(np.linalg.norm(rh_kpts, axis=-1), axis=1)[:, np.newaxis, np.newaxis] > 0.01 # [T, 1, 1]
    lh_valid = np.mean(np.linalg.norm(lh_kpts, axis=-1), axis=1)[:, np.newaxis, np.newaxis] > 0.01 # [T, 1, 1]
    
    rh_wrist = pos_scaled[:, 21:22, :3] # [T, 1, 3]
    lh_wrist = pos_scaled[:, 0:1, :3] # [T, 1, 3]
    rh_span = np.maximum(np.linalg.norm(pos_scaled[:, 30:31, :3] - rh_wrist, axis=-1, keepdims=True) * 2.5, 0.05) # [T, 1, 1]
    lh_span = np.maximum(np.linalg.norm(pos_scaled[:, 9:10, :3] - lh_wrist, axis=-1, keepdims=True) * 2.5, 0.05) # [T, 1, 1]
    
    anchor = np.where(has_shoulders, mid_shoulder,
             np.where(has_face, face_cen,
             np.where(rh_valid, rh_wrist,
             np.where(lh_valid, lh_wrist, np.zeros_like(mid_shoulder))))) # [T, 1, 3]
             
    scale = np.where(has_shoulders, shoulder_dist,
            np.where(has_face, np.maximum(face_span, 0.05),
            np.where(rh_valid, rh_span,
            np.where(lh_valid, lh_span, np.ones_like(shoulder_dist))))) # [T, 1, 1]
            
    scale = np.maximum(scale, 1e-4)
    # Temporal median scale stabilization: eliminates detector breathing jitter and false acceleration spikes
    scale_med = np.median(scale, axis=0, keepdims=True)
    scale = np.where(scale_med > 1e-4, scale_med, scale)
    norm_pos = (pos_scaled - anchor) / scale
    kinematics[:, :, :3] = norm_pos

    # 2. Savitzky-Golay Velocity & Acceleration
    if not is_static:
        vel = kinematics[:, :, 3:6]
        vel[2:-2] = (-2.0 * norm_pos[:-4] - norm_pos[1:-3] + norm_pos[3:-1] + 2.0 * norm_pos[4:]) / 10.0
        vel[1] = (norm_pos[2] - norm_pos[0]) * 0.5
        vel[0] = norm_pos[1] - norm_pos[0]
        vel[-2] = (norm_pos[-1] - norm_pos[-3]) * 0.5
        vel[-1] = norm_pos[-1] - norm_pos[-2]
        np.clip(vel, -3.5, 3.5, out=vel)

        acc = kinematics[:, :, 6:9]
        acc[2:-2] = (2.0 * norm_pos[:-4] - norm_pos[1:-3] - 2.0 * norm_pos[2:-2] - norm_pos[3:-1] + 2.0 * norm_pos[4:]) / 7.0
        acc[1] = norm_pos[2] - 2.0 * norm_pos[1] + norm_pos[0]
        acc[0] = acc[1]
        acc[-2] = norm_pos[-1] - 2.0 * norm_pos[-2] + norm_pos[-3]
        acc[-1] = acc[-2]
        np.clip(acc, -8.0, 8.0, out=acc)
    else:
        kinematics[:, :, 3:9] = 0.0
    ...
    face_cen = np.mean(norm_pos[:, 48:60], axis=1, keepdims=True)
    phon[:, 7:8] = np.linalg.norm(norm_pos[:, lh_w:lh_w+1] - face_cen, axis=-1)
    phon[:, 8:9] = np.linalg.norm(norm_pos[:, rh_w:rh_w+1] - face_cen, axis=-1)

    lh_tips = [4, 8, 12, 16, 20]
    rh_tips = [25, 29, 33, 37, 41]
    phon[:, 9:14] = np.linalg.norm(norm_pos[:, lh_tips] - norm_pos[:, [lh_w]], axis=-1)
    phon[:, 14:19] = np.linalg.norm(norm_pos[:, rh_tips] - norm_pos[:, [rh_w]], axis=-1)
...
def _init_worker_model(mode: str = 'light'):
    global _GLOBAL_WB
...
def main():
    ...
    parser.add_argument("--datasets", nargs="+", default=["how2sign", "alphabet", "numbers", "citizen", "wlasl", "chicagofswild"])
    ...
    log_msg(f"  Parallel CPU Workers: {num_workers} (Host CPUs: {os.cpu_count()})")
    ...
    if "alphabet" in args.datasets:
        if total_stats.get("ASL_Alphabet", 0) >= 80000:
            ...
            if train_dir.exists() and _RTMLIB_AVAILABLE:
                ...
                t0 = time.time()
                alpha_count = 0
                with mp.Pool(processes=num_workers, initializer=_init_worker_model, initargs=(args.mode,)) as pool:
    ...
    if "chicagofswild" in args.datasets or "chicago" in args.datasets:
        if total_stats.get("ChicagoFSWild", 0) >= 5000:
            ...
            if found_pre is not None:
                ...
                if csv_p.exists():
                    log_msg(f"[ChicagoFSWild] Found CSV metadata at {csv_p} (tarball mode).")

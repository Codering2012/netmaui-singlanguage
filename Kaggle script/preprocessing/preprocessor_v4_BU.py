...
def reference_part_normalize(landmarks: np.ndarray, val_mask: np.ndarray) -> Tuple[np.ndarray, float]:
    ...
    Optimized single-allocation in-place multiplication.
    """
    ...
    if both_valid.any():
        ...
        anchor = np.mean(mid_sh, axis=0)  # [3]
...
def clean_out_of_bounds_hands(feat_arr: np.ndarray) -> np.ndarray:
    ...
    out = feat_arr.copy()
    if np.any(invalid[:, 0]):
        out[invalid[:, 0], 0:21, :] = 0.0
    if np.any(invalid[:, 1]):
        out[invalid[:, 1], 21:42, :] = 0.0
    return out
...
def compute_9d_kinematics(landmarks_seq: np.ndarray, smooth: bool = True) -> np.ndarray:
    ...
    Fused zero-copy Savitzky-Golay stencil with in-place bounds clamping.
    """
    t_len, num_pts, _ = landmarks_seq.shape
    if t_len == 0:
    ...
    if t_len >= 5 and smooth:
        ...
        # Fused 5-point quadratic Savitzky-Golay 1st derivative (velocity)
        # v[t] = (-2*pos[t-2] - pos[t-1] + pos[t+1] + 2*pos[t+2]) * 0.1
        v_mid = (-2.0 * p0 - p1 + p3 + 2.0 * p4) * 0.1
        np.clip(v_mid, -3.5, 3.5, out=vel_slice[2:-2])
        vel_slice[1] = np.clip((pos[2] - pos[0]) * 0.5, -3.5, 3.5)
        vel_slice[0] = np.clip(pos[1] - pos[0], -3.5, 3.5)
        vel_slice[-2] = np.clip((pos[-1] - pos[-3]) * 0.5, -3.5, 3.5)
        vel_slice[-1] = np.clip(pos[-1] - pos[-2], -3.5, 3.5)

        # Fused 5-point quadratic Savitzky-Golay 2nd derivative (acceleration)
        # a[t] = (2*pos[t-2] - pos[t-1] - 2*pos[t] - pos[t+1] + 2*pos[t+2]) / 7.0
        a_mid = (2.0 * p0 - p1 - 2.0 * p2 - p3 + 2.0 * p4) * (1.0 / 7.0)
        np.clip(a_mid, -8.0, 8.0, out=acc_slice[2:-2])
        a1 = np.clip(pos[2] - 2.0 * pos[1] + pos[0], -8.0, 8.0)
        acc_slice[0] = a1
        acc_slice[1] = a1
        a_end = np.clip(pos[-1] - 2.0 * pos[-2] + pos[-3], -8.0, 8.0)
        acc_slice[-2] = a_end
        acc_slice[-1] = a_end
    elif t_len > 2:
        vel_slice[1:-1] = np.clip((pos[2:] - pos[:-2]) * 0.5, -3.5, 3.5)
        vel_slice[0] = np.clip(pos[1] - pos[0], -3.5, 3.5)
        vel_slice[-1] = np.clip(pos[-1] - pos[-2], -3.5, 3.5)

        acc_slice[1:-1] = np.clip(pos[2:] - 2.0 * pos[1:-1] + pos[:-2], -8.0, 8.0)
...
def interpolate_missing_hand_landmarks(landmarks_seq: np.ndarray, valid_mask: np.ndarray) -> np.ndarray:
    """Smoothly interpolates hand landmark trajectories during dropped hand detections with fast-path filtering."""
    t_len, num_pts, _ = landmarks_seq.shape
    if t_len <= 1:
        return landmarks_seq
    ...
    for pt_idx in range(hand_pts):
        ...
        if len(valid_t) == 1:
            cleaned_seq[:, pt_idx, :] = cleaned_seq[valid_t[0], pt_idx, :]
            continue
...
def compute_19d_phonology(landmarks_seq: np.ndarray, val_mask: np.ndarray) -> np.ndarray:
    ...
    phonology[:, 7] = np.sqrt(d_lh_face[:, 0] ** 2 + d_lh_face[:, 1] ** 2 + d_lh_face[:, 2] ** 2)
    phonology[:, 8] = np.sqrt(d_rh_face[:, 0] ** 2 + d_rh_face[:, 1] ** 2 + d_rh_face[:, 2] ** 2)

    # 4. Finger Curl / Aperture (10 Dims: 5 LH + 5 RH)
    lh_tips = [4, 8, 12, 16, 20]
    rh_tips = [25, 29, 33, 37, 41]
    diff_lh = pos[:, lh_tips, :3] - pos[:, 0:1, :3]
    phonology[:, 9:14] = np.sqrt(np.sum(diff_lh * diff_lh, axis=-1))

    diff_rh = pos[:, rh_tips, :3] - pos[:, 21:22, :3]
    phonology[:, 14:19] = np.sqrt(np.sum(diff_rh * diff_rh, axis=-1))
...
def main():
    ...
    parser.add_argument("--chunksize", type=int, default=None, help="Optional chunksize for imap_unordered (auto-tuned if not set)")
    parser.add_argument("--disable-streaming", action="store_true", help="Accumulate all records in RAM before writing shards (not recommended for large datasets)")

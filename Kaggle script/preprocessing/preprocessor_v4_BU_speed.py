...
def augment_landmarks_3d(landmarks_seq: np.ndarray, rng: Optional[np.random.RandomState] = None) -> np.ndarray:
    ...
    # 1. In-plane camera roll rotation (+- 7 deg)
...
def compute_9d_kinematics(landmarks_seq: np.ndarray, smooth: bool = True) -> np.ndarray:
    ...
    Applies binomial temporal jitter smoothing and physical bounds clamping to prevent
    gradient explosions from tracking pops.
    """
    t_len, num_pts, _ = landmarks_seq.shape
    if t_len == 0:
    ...
    if t_len > 2:
        acc[1:-1] = pos[2:] - 2.0 * pos[1:-1] + pos[:-2]
        acc[0] = acc[1]
        acc[-1] = acc[-2]

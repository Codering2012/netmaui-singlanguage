...
class FrankensteinDataProcessor:
    ...
    def _record_completed_keys(self, records: list, tag: str) -> None:
        """Record completed keys into the JSONL manifest after a batch save."""
        if self.temp_shard_dir is None or not records:
            return
        gpu_suffix = f"_gpu{self.gpu_id}" if self.gpu_id is not None else ""
        manifest_file = self.temp_shard_dir / f"completed_{tag}{gpu_suffix}.jsonl"
        try:
    ...
    def flush_batch(self, records: list, tag: str) -> None:
        ...
        try:
            ...
            tmp_sp.replace(sp)
            self._record_completed_keys(records, tag)
            self._batch_counters[tag] += 1
            log_msg(f"[Batch Save] Flushed {len(records)} {tag} records -> {sp.name}")
        except Exception as e:
            log_msg(f"[!] Error flushing batch for {tag}: {e}")
        finally:
            records.clear()
    ...
    def process_asl_alphabet(self, split="train"):
        ...
        tasks = self._shard_tasks(tasks, tag="alphabet")
        ...
        if records and self.temp_shard_dir is not None:
            self.flush_batch(records, tag="alphabet")
        elapsed = time.time() - t0
        rate = total_extracted / elapsed if elapsed > 0 else 0.0
        log_msg(
            f"[+] Loaded/Flushed {total_extracted} ASL Alphabet samples for {split} in {elapsed:.2f}s ({rate:.2f} samples/sec)."
        )
        profiler.print_top5()
        return records
    ...
    def process_asl_citizen(self, split="train"):
        ...
        tasks = self._shard_tasks(tasks, tag="citizen")
        if getattr(self, "is_test", False):
            tasks = tasks[:100]
        records = []
        total_extracted = 0
        ...
        rate = total_extracted / elapsed if elapsed > 0 else 0.0
        log_msg(
            f"[+] Loaded/Flushed {total_extracted} ASL Citizen samples for {split} in {elapsed:.2f}s ({rate:.2f} samples/sec)."
        )
        profiler.print_top5()
        return records
    ...
    def process_how2sign_holistic(self, split="train", min_quality=0.20, limit=None):
        ...
        tasks = self._shard_tasks(tasks, tag="how2sign_holistic")
        if getattr(self, "is_test", False):
            tasks = tasks[:100]
        records = []
        total_extracted = 0
        ...
        rate = total_extracted / elapsed if elapsed > 0 else 0.0
        log_msg(
            f"[+] Loaded/Flushed {total_extracted} How2Sign samples for {split} in {elapsed:.2f}s ({rate:.2f} samples/sec)."
        )
        profiler.print_top5()
        return records
    ...
    def process_chicago_fswild(self, split="train"):
        ...
        tasks = self._shard_tasks(tasks, tag="chicago")
        if getattr(self, "is_test", False):
            tasks = tasks[:100]
        records = []
        total_extracted = 0
        ...
        rate = total_extracted / elapsed if elapsed > 0 else 0.0
        log_msg(
            f"[+] Loaded/Flushed {total_extracted} ChicagoFSWild samples for {split} in {elapsed:.2f}s ({rate:.2f} samples/sec)."
        )
        profiler.print_top5()
        return records
    ...
    def process_synthetic_numbers(self, split="train"):
        ...
        _DIR_FILE_CACHE.clear()
        tasks = self._shard_tasks(tasks, tag="numbers")
        if getattr(self, "is_test", False):
            tasks = tasks[:100]
        records = []
        total_extracted = 0
        ...
        rate = total_extracted / elapsed if elapsed > 0 else 0.0
        log_msg(
            f"[+] Loaded/Flushed {total_extracted} Synthetic Numbers samples for {split} in {elapsed:.2f}s ({rate:.2f} samples/sec)."
        )
        profiler.print_top5()
        return records
    ...
    def process_wlasl(self, split="train"):
        ...
        tasks = self._shard_tasks(tasks, tag="wlasl")
        if getattr(self, "is_test", False):
            tasks = tasks[:100]
        records = []
        total_extracted = 0
        ...
        rate = total_extracted / elapsed if elapsed > 0 else 0.0
        log_msg(
            f"[+] Loaded/Flushed {total_extracted} WLASL samples for {split} in {elapsed:.2f}s ({rate:.2f} samples/sec)."
        )
        profiler.print_top5()
        return records
...
def build_split(processor, split, normalizer, augment=False, label_to_idx=None):
    ...
    processor.temp_shard_dir = temp_shard_dir
    ...
    try:
        ...
        # Only remove temp shards if we are not exiting Phase A early for subsequent merging.
        if getattr(processor, "phase", "all") != "extract":
            for sp in temp_shard_dir.glob("*"):
                try:
                    sp.unlink()
                except Exception:
                    pass
...
def main(argv=None):
    ...
    parser.add_argument(
        "--batch-flush-size",
        type=int,
        default=BATCH_FLUSH_SIZE,
        help="Number of records before flushing a batch shard to disk during extraction",
    )
    parser.add_argument(
        "--workers",
        "-w",
    ...
    for split in splits:
        ...
        if split in ["val", "test"] and master_label_to_idx is None:
            if train_map_path.exists():
                with open(train_map_path, "r", encoding="utf-8") as f:
                    master_label_to_idx = json.load(f)
            if master_label_to_idx is None:
        ...
        # Reset phase for next split iteration if master orchestrator
        processor.phase = args.phase

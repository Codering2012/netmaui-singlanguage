...
# Danh sách 20 đường nối xương của MediaPipe Hands
HAND_CONNECTIONS = [
...
def get_accurate_asl_template(letter: str) -> np.ndarray:
    ...
    if letter == 'A':
        ...
        pts[3] = [0.52, 0.64, 0.01]; pts[4] = [0.56, 0.64, 0.01]
    elif letter == 'N':
        for m in [5, 9, 13, 17]: curl(m, target_y=0.62)
        pts[3] = [0.48, 0.64, 0.01]; pts[4] = [0.50, 0.64, 0.01]
    elif letter == 'O':
        for m in [5, 9, 13, 17]:
        ...
        for m in [9, 13, 17]: curl(m)
        pts[3] = [0.45, 0.60, 0.03]; pts[4] = [0.48, 0.58, 0.04]
    elif letter == 'OPEN_PALM':
        for m in [5, 9, 13, 17]: ext(m, (0.0, -1.0, 0.0))
        pts[3] = [0.34, 0.62, -0.02]; pts[4] = [0.26, 0.58, -0.02]
    elif letter == 'THUMBS_UP':
        for m in [5, 9, 13, 17]: curl(m, target_y=0.64)
        pts[3] = [0.40, 0.52, -0.02]; pts[4] = [0.40, 0.40, -0.03]

    return pts

# Xây dựng bảng từ điển mẫu 26 chữ cái ASL + 2 cử chỉ điều khiển
ALPHABET = [chr(65+i) for i in range(26)] + ["OPEN_PALM", "THUMBS_UP"]
CANONICAL_TEMPLATES = {c: get_bone_direction_vector(get_accurate_asl_template(c)) for c in ALPHABET}

def match_asl_template(landmarks_21: np.ndarray) -> tuple:
    """So khớp hướng xương Cosine Similarity với 26 mẫu ASL A - Z."""
    ...
    # Chuyển similarity (0.6 - 1.0) thành độ tin cậy Softmax
    conf = float(np.clip((best_sim - 0.5) / 0.5, 0.50, 0.99))
    return best_char, conf

if __name__ == "__main__":
    print("Kiểm tra bộ so khớp mẫu hướng xương 3D:")
    for letter in [chr(65+i) for i in range(26)]:
        lm = get_accurate_asl_template(letter)
        # Thêm nhiễu ngẫu nhiên
        lm_noisy = lm + np.random.normal(0, 0.005, size=lm.shape)
        pred, conf = match_asl_template(lm_noisy)
        print(f"Sign '{letter}' -> Matched '{pred}' ({conf*100:.1f}%)")

...
# Persistent XLA compilation cache directory path: Prefer high-speed RAM /dev/shm or /tmp to prevent
# exhausting Kaggle's disk quota with multi-gigabyte compiled HLO graphs across 8 cores.
_default_cache = "/tmp/xla_cache" if os.name != "nt" else "./xla_cache"
if os.name != "nt" and os.path.exists("/dev/shm"):
    try:
        import shutil
        if shutil.disk_usage("/dev/shm").free > 10 * 1024 * 1024 * 1024:
            _default_cache = "/dev/shm/xla_cache"
    except Exception:
        pass
...
# Critical for PyTorch/XLA: prevent transformers packed sequence check from triggering synchronous device execution
try:
    import transformers.masking_utils
    transformers.masking_utils.find_packed_sequence_indices = lambda *args, **kwargs: None
except Exception:
    pass
...
def smart_checkpoint(function, *args, use_reentrant: bool = True, **kwargs):
    """
    Accelerator-aware activation checkpointing.
    Uses torch_xla.utils.checkpoint on TPU (which inserts optimization barriers)
    and torch.utils.checkpoint elsewhere.
    """
    if IS_TPU:
        try:
            from torch_xla.utils.checkpoint import checkpoint as xla_checkpoint
            return xla_checkpoint(function, *args, use_reentrant=use_reentrant, **kwargs)
        except Exception:
            pass
    import torch.utils.checkpoint
    return torch.utils.checkpoint.checkpoint(function, *args, use_reentrant=use_reentrant, **kwargs)
...
def get_dynamic_loader_len(
    loader: Any, default_steps: int = 2500, args: Optional[Any] = None, is_val: bool = False
) -> int:
    """Computes dynamic epoch steps accurately without artificial capping unless requested."""
    if is_val:
        target_limit = getattr(args, "val_check_steps", 0) if args is not None else 0
    else:
        target_limit = (
    ...
    try:
        ...
        if target_limit and target_limit > 0:
            return min(cur_len, target_limit)
        return cur_len
    except (TypeError, AttributeError):
        pass
    ds = getattr(loader, "dataset", None)
    d_len = getattr(ds, "total_records", None) or (getattr(args, "dataset_len", None) if args is not None else None) or (getattr(args, "total_samples", None) if args is not None else None)
    world_sz = 1
    if IS_TPU:
        try:
            import torch_xla.core.xla_model as xm
            world_sz = xm.xrt_world_size() if hasattr(xm, "xrt_world_size") else 8
        except Exception:
            world_sz = 8
    ...
    bs = getattr(loader, "batch_size", None) or (getattr(args, "batch_size", 64) if args is not None else 64)
    effective_bs = max(1, bs * max(1, world_sz))
    if d_len is not None and d_len > 0:
        calc_len = max(1, d_len // effective_bs)
        if target_limit and target_limit > 0:
            return min(calc_len, target_limit)
        return calc_len
    if ds is not None and hasattr(ds, "shard_files") and ds.shard_files:
        calc_len = max(1, (len(ds.shard_files) * 5000) // effective_bs)
        if target_limit and target_limit > 0:
            return min(calc_len, target_limit)
        return calc_len
    return target_limit if (target_limit and target_limit > 0) else default_steps
...
class CosineLinear(nn.Module):
    ...
    def forward(self, input_x: torch.Tensor) -> torch.Tensor:
        ...
        if self.actual_vocab_size is not None and logits.shape[-1] > self.actual_vocab_size:
            mask = torch.arange(logits.shape[-1], device=logits.device) >= self.actual_vocab_size
            logits = torch.where(mask, torch.tensor(-65500.0, dtype=logits.dtype, device=logits.device), logits)
        return logits
...
class InGraphAugmentor(nn.Module):
    ...
    def __init__(
        ...
        if num_keypoints >= 60:
            jitter_mask[:, :, 0:42, :] = 1.0  # Hands (0:21 LH, 21:42 RH) get prioritized jitter
    ...
    def forward(
        ...
        # 1. Sagittal Horizontal Reflection (Mirroring X and Full Bilateral Arm/Hand Swapping)
        if self.mirror_prob > 0 and K >= 60:
            ...
            # Swap Left Hand (indices 0:21) and Right Hand (indices 21:42)
            lh = pos_reflected[:, :, 0:21, :]
            rh = pos_reflected[:, :, 21:42, :]
            swapped_lh = torch.where(mirror_apply.expand(-1, T, 21, 3), rh, lh)
            swapped_rh = torch.where(mirror_apply.expand(-1, T, 21, 3), lh, rh)

            # Swap Shoulders (42 <-> 43), Hips (44 <-> 45), Elbows (46 <-> 47)
            l_sh = pos_reflected[:, :, 42:43, :]
            r_sh = pos_reflected[:, :, 43:44, :]
            swapped_l_sh = torch.where(mirror_apply.expand(-1, T, 1, 3), r_sh, l_sh)
            swapped_r_sh = torch.where(mirror_apply.expand(-1, T, 1, 3), l_sh, r_sh)

            l_hip = pos_reflected[:, :, 44:45, :]
            r_hip = pos_reflected[:, :, 45:46, :]
            swapped_l_hip = torch.where(mirror_apply.expand(-1, T, 1, 3), r_hip, l_hip)
            swapped_r_hip = torch.where(mirror_apply.expand(-1, T, 1, 3), l_hip, r_hip)

            l_el = pos_reflected[:, :, 46:47, :]
            r_el = pos_reflected[:, :, 47:48, :]
            swapped_l_el = torch.where(mirror_apply.expand(-1, T, 1, 3), r_el, l_el)
            swapped_r_el = torch.where(mirror_apply.expand(-1, T, 1, 3), l_el, r_el)

            # Face: 48..51 midline (unchanged), 52 <-> 53 eyes, 54 <-> 55 mouth corners, 56..59 contour
            face_mid = pos_reflected[:, :, 48:52, :]
            l_eye = pos_reflected[:, :, 52:53, :]
            r_eye = pos_reflected[:, :, 53:54, :]
            swapped_l_eye = torch.where(mirror_apply.expand(-1, T, 1, 3), r_eye, l_eye)
            swapped_r_eye = torch.where(mirror_apply.expand(-1, T, 1, 3), l_eye, r_eye)

            l_mouth = pos_reflected[:, :, 54:55, :]
            r_mouth = pos_reflected[:, :, 55:56, :]
            swapped_l_mouth = torch.where(mirror_apply.expand(-1, T, 1, 3), r_mouth, l_mouth)
            swapped_r_mouth = torch.where(mirror_apply.expand(-1, T, 1, 3), l_mouth, r_mouth)

            face_contour = pos_reflected[:, :, 56:60, :]

            pos = torch.cat([
                swapped_lh,                   # Left Hand (0..20)
                swapped_rh,                   # Right Hand (21..41)
                swapped_l_sh,                 # Left Shoulder (42)
                swapped_r_sh,                 # Right Shoulder (43)
                swapped_l_hip,                # Left Hip (44)
                swapped_r_hip,                # Right Hip (45)
                swapped_l_el,                 # Left Elbow (46)
                swapped_r_el,                 # Right Elbow (47)
                face_mid,                     # Face Midline (48..51)
                swapped_l_eye,                # Left Eye (52)
                swapped_r_eye,                # Right Eye (53)
                swapped_l_mouth,              # Left Mouth Corner (54)
                swapped_r_mouth,              # Right Mouth Corner (55)
                face_contour,                 # Forehead/Contour (56..59)
                pos_reflected[:, :, 60:, :],  # Extra keypoints if any
            ], dim=2)
        ...
        if self.unilateral_drop_prob > 0 and K >= 60:
            ...
            pos = torch.cat([
                pos[:, :, 0:21, :] * lh_keep_factor,  # Left Hand (0..20)
                pos[:, :, 21:, :],                    # Right Hand (21..41) + Pose (42..47) + Face (48..59)
            ], dim=2)
...
class TopologicalBoneLengthLoss(nn.Module):
    ...
    def __init__(self):
        ...
        lh_bones = [(u + 0, v + 0) for u, v in hand_bones]
        rh_bones = [(u + 21, v + 21) for u, v in hand_bones]
    ...
    def forward(self, features: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        bone_vecs = v_pos - u_pos
        bone_lens = torch.norm(bone_vecs + 1e-8, dim=-1)
        
        # Valid active bone mask (neither endpoint is zero/absent)
        u_active = (u_pos.abs().sum(dim=-1) > 1e-4)
        v_active = (v_pos.abs().sum(dim=-1) > 1e-4)
        bone_active = u_active & v_active & mask.unsqueeze(-1)
        
        # Compute mean rest length strictly over active frames
        active_counts = bone_active.sum(dim=1, keepdim=True).clamp(min=1.0)
        active_lens = torch.where(bone_active, bone_lens, torch.zeros_like(bone_lens))
        mean_lens = active_lens.sum(dim=1, keepdim=True) / active_counts
        
        diff_sq = (bone_lens - mean_lens) ** 2
        masked_diff = torch.where(bone_active, diff_sq, torch.zeros_like(diff_sq))
        
        total_valid = bone_active.sum().clamp(min=1.0)
        return masked_diff.sum() / total_valid
...
class SignToGPT2Bridge(nn.Module):
    ...
    def __init__(
        ...
        if hasattr(self.gpt2.config, "_attn_implementation"):
            self.gpt2.config._attn_implementation = "sdpa"
        ...
        if torch_dtype is not None:
            self.gpt2 = self.gpt2.to(torch_dtype)
        ...
        if torch_dtype is not None:
            self.visual_proj = self.visual_proj.to(torch_dtype)
            self.prefix_tokens.data = self.prefix_tokens.data.to(torch_dtype)
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        if getattr(self, "freeze_gpt2", True):
            self.gpt2.eval()
        return self
    ...
    def forward(
        ...
        # Dynamic sequence length configured from model/args (e.g. 128).
        # Perfectly aligns with TPU v5e / v3 128x128 systolic array tiles (1 x 128).
        target_total_len = getattr(self, "max_total_len", 128)
        ...
        # On TPU v5e, keep num_chunks=1 for per-core batch <= 192 (h_flat <= 21504).
        # For per-core batch >= 256 (h_flat >= 28672), use exactly 16 systolic tile-aligned chunks
        # (1,792 tokens each, exact multiple of 128) to cut backward transient memory by 2.70GB (saving 180MB).
        if IS_TPU:
            if h_flat.shape[0] >= 24576:
                num_chunks = 8
            elif h_flat.shape[0] >= 12288:
                num_chunks = 4
            else:
                num_chunks = 1
        else:
            num_chunks = max(1, math.ceil(h_flat.shape[0] / 768))
...
class ASLFoundationModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.gradient_checkpointing = gradient_checkpointing
        self.enable_ctc = kwargs.get("enable_ctc", not kwargs.get("disable_ctc", False))
        self.use_swin_1d = use_swin_1d
        self.swin_window_size = swin_window_size
        self.actual_vocab_size = vocab_size
        self.english_max_len = english_max_len if english_max_len is not None else max_dec_len
        self.chicago_max_len = chicago_max_len if chicago_max_len is not None else max_dec_len
        ...
        # World-first DKP-Apex: Differential-Geometric Kinematic Phase Flow & Phonological Apex Attention
        self.dkp_apex = DifferentialKinematicApex(d_model=d_enc, gamma_min=0.15)
        ...
        if self.use_gpt2:
            try:
                self.gpt2_bridge = SignToGPT2Bridge(
                    ...
                    max_total_len=getattr(self, "english_max_len", max_enc_len),
        ...
        self.early_ctc_head = CTCHead(d_enc, self.vocab_size, actual_vocab_size=self.actual_vocab_size)
    ...
    def compute_phonology_features(
        self, pos: torch.Tensor, vel: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        Vectorized 19D Phonological Feature Extraction directly on device.
        Extracts palm normals, bimanual synchrony, facial anchoring, and finger curls.
        """
        if vel is None:
            vel = F.pad(pos[:, 1:] - pos[:, :-1], (0, 0, 0, 0, 1, 0))
        if pos.shape[2] >= 60:
            ...
            face_centroid = pos[:, :, 48:60, :3].mean(dim=2, keepdim=True)
        else:
            ...
            face_centroid = pos[:, :, 0:1, :3].mean(dim=2, keepdim=True) * 0.0

        lh_wrist = pos[:, :, lh_w, :3]
        rh_wrist = pos[:, :, rh_w, :3]
        lh_u = pos[:, :, lh_idx, :3] - lh_wrist
        lh_v = pos[:, :, lh_pky, :3] - lh_wrist
        lh_normal = F.normalize(fast_cross(lh_u, lh_v), p=2, dim=-1, eps=1e-5)

        rh_u = pos[:, :, rh_idx, :3] - rh_wrist
        rh_v = pos[:, :, rh_pky, :3] - rh_wrist
        rh_normal = F.normalize(fast_cross(rh_u, rh_v), p=2, dim=-1, eps=1e-5)

        lh_vel = vel[:, :, lh_w, :3]
        rh_vel = vel[:, :, rh_w, :3]
        bimanual_sync = safe_cosine_sim(lh_vel, rh_vel).unsqueeze(-1)

        lh_face_dist = safe_norm(lh_wrist.unsqueeze(2) - face_centroid, dim=-1).squeeze(2).unsqueeze(-1)
        rh_face_dist = safe_norm(rh_wrist.unsqueeze(2) - face_centroid, dim=-1).squeeze(2).unsqueeze(-1)
    ...
    def _encode(
        ...
        inter_idx = pool_idx + 1  # Capture *after* pooling (Claims 54-55)
        if inter_idx < 0:
            inter_idx = 0
        early_idx = max(0, min(pool_idx - 1, len(self.blocks) // 3))
        ...
        if x_in.dim() == 4 and x_in.size(2) == 60:
            ...
            # Vectorized canonical 60-keypoint anatomical normalization:
            # 0:21 -> Left Hand (wrist-relative: 0:1)
            # 21:42 -> Right Hand (wrist-relative: 21:22)
            # 42:48 -> Torso / Pose (mid-shoulder relative: 42:43 & 43:44)
            # 48:60 -> Face (face center relative: 48:60)
            l_wrist = xk[:, :, 0:1, :3]
            l_hand = xk[:, :, 0:21, :3] - l_wrist
            r_wrist = xk[:, :, 21:22, :3]
            r_hand = xk[:, :, 21:42, :3] - r_wrist
            shoulder_mid = (xk[:, :, 42:43, :3] + xk[:, :, 43:44, :3]) * 0.5
            torso = xk[:, :, 42:48, :3] - shoulder_mid
            face_center = torch.mean(xk[:, :, 48:60, :3], dim=2, keepdim=True)
            face = xk[:, :, 48:60, :3] - face_center
            pos_normed = torch.cat([l_hand, r_hand, torso, face], dim=2)
            if xk.size(-1) > 3:
        ...
        if x_in.dim() == 4:
            apex_pos = x_in[:, :, :, :3]
            apex_vel = x_in[:, :, :, 3:6] if x_in.size(-1) >= 6 else None
        else:
            x_4d = x_in.reshape(batch_sz, seq_len, 60, -1)
            apex_pos = x_4d[:, :, :, :3]
            apex_vel = x_4d[:, :, :, 3:6] if x_4d.size(-1) >= 6 else None

        geo_emb, apex_bias, _ = self.dkp_apex(apex_pos, apex_vel)
        hidden_h = self.input_stem(x_stem_in) + geo_emb.to(v_tokens.dtype)
        if not self.is_causal:
            hidden_h = torch.cat([self.cls_token.expand(batch_sz, -1, -1), hidden_h], dim=1)
            # Pad CLS token with neutral 0.0 attention bias
            block_apex_bias = F.pad(apex_bias, (1, 0, 1, 0), mode="constant", value=0.0).to(hidden_h.dtype)
            ...
            block_apex_bias = apex_bias.to(hidden_h.dtype)
            ...
            if isinstance(block, TemporalStridedPool):
                ...
                if not self.is_causal:
                    ...
                    if cur_mask is not None:
                        ...
                        kpm = None
                block_apex_bias = None
            else:
                # Conformer blocks uncheckpointed: eliminates recomputation latency in backward pass (~60ms speedup);
                # HBM budget is fully secured by fused decoder checkpointing and orig_x deduplication (>450MB free headroom).
                if hasattr(block, "mha") and block_apex_bias is not None and block_apex_bias.shape[-1] == hidden_h.shape[1]:
                    hidden_h = block(hidden_h, key_padding_mask=kpm, frame_indices=pos_fi, cache=cache, attn_mask=block_apex_bias)
                else:
    ...
    def forward(
        ...
        gpt2_seq: Optional[torch.Tensor] = None,
        mlm_mask: Optional[torch.Tensor] = None,
        frame_indices: Optional[torch.Tensor] = None,
        ...
        if True:
            ...
            phonology_features = self.compute_phonology_features(pos, vel).to(target_dtype)
        ...
        h_cls, h_seq, enc_mask, used_mlm_mask, fi_out, orig_enc_mask, inter_h, token_sizes = enc_res[:8]
        early_h = enc_res[8] if len(enc_res) > 8 else None

        dec_logits, dec_hidden = None, None
        chicago_logits, english_logits = None, None
        h_e = None

        use_gpt2_active = getattr(self, "use_gpt2", False) and getattr(self, "gpt2_bridge", None) is not None
        can_fuse_decoders = (
        ...
        if can_fuse_decoders:
            # Fused Multi-Task Batch Execution: runs all decoders simultaneously with zero bypasses
            tgt_g = gloss_seq[:, :-1]
            tgt_c = chicago_seq[:, :-1]
            ...
            include_english = (
                english_seq is not None
                and self.english_decoder is not None
                and not getattr(self, "use_gpt2", False)
            )
            ...
            if include_english:
                ...
                orig_e_len = 0
                max_tgt_len = max(orig_g_len, orig_c_len)

            # Hardware Systolic Tile Alignment:
            # Active token bounds: all ASL glosses in dataset <= 50 tokens, all Chicago <= 35 chars.
            # With fused batch size 512 (256 gloss + 256 chicago), slicing to static 64 tokens
            # yields exactly 512 * 64 = 32,768 tokens (256 * 128 systolic MXU tiles), perfectly
            # aligned to TPU v5e hardware while saving >1.2 GB of HBM and halving decoder compute.
            target_dec_len = min(64, max_tgt_len) if max_tgt_len >= 64 else 64
            if target_dec_len <= 0:
                target_dec_len = 64
            
            tgt_g = tgt_g[:, :target_dec_len]
            tgt_c = tgt_c[:, :target_dec_len]
            orig_g_len = tgt_g.size(1)
            orig_c_len = tgt_c.size(1)
            max_tgt_len = target_dec_len
            ...
            if include_english:
                tgt_e = tgt_e[:, :target_dec_len]
                if tgt_e.size(1) < max_tgt_len:
                    tgt_e = F.pad(tgt_e, (0, max_tgt_len - tgt_e.size(1)), value=EnglishVocabulary.PAD_ID)
            ...
            if include_english:
                ...
                h_fused = torch.cat([hg, hc], dim=0)

            b_sz = gloss_seq.size(0)
            if h_seq.dtype != h_fused.dtype:
                h_seq = h_seq.to(h_fused.dtype)

            # Native Cross-Attention broadcast without allocating redundant mem_fused repeat tensor
            enc_kpm = (~enc_mask.bool()) if enc_mask is not None else None
            for layer in self.decoder.layers:
                if getattr(self, "gradient_checkpointing", False) and self.training:
                    def _ckpt_dec_layer(l_mod, x_in, mem_in, mask_in):
                        return l_mod(x_in, mem_in, memory_key_padding_mask=mask_in)[0]
                    h_fused = smart_checkpoint(
                        _ckpt_dec_layer, layer, h_fused, h_seq, enc_kpm, use_reentrant=True
                    )
                else:
                    h_fused = layer(h_fused, h_seq, memory_key_padding_mask=enc_kpm)[0]
            ...
            if include_english:
                ...
                h_g, h_c = torch.split(h_fused, b_sz, dim=0)
                h_e = None
            ...
            if english_seq is not None and self.english_decoder is not None:
                ...
                english_logits, h_e, _, _ = decode_seq(
                    ...
                    bos_idx=eng_bos,
                    eos_idx=eng_eos,
                    compute_head=(not self.training),
                )

        # Compute output heads
        if getattr(self, "enable_ctc", True):
            ctc_log_probs = self.ctc_head(h_seq)
            inter_ctc_log_probs = (
                self.inter_ctc_head(inter_h) if (inter_h is not None and getattr(self, "enable_inter_ctc", True)) else None
            )
            early_ctc_log_probs = (
                self.early_ctc_head(early_h) if (hasattr(self, "early_ctc_head") and early_h is not None and getattr(self, "enable_early_ctc", getattr(self, "enable_inter_ctc", True))) else None
            )
        else:
            ctc_log_probs = None
            inter_ctc_log_probs = None
            early_ctc_log_probs = None
        ...
        sent_emb = None
        if gloss_seq is not None and self.decoder is not None:
            # Active token bounds: all actual glosses in dataset <= 50 tokens.
            # Slicing to 64 tokens eliminates redundant pad embedding lookups, saving >50MB HBM.
            tgt = gloss_seq[:, 1:65]  # [B, 64]
            non_pad = (
                (tgt != GlossVocabulary.PAD_ID) & (tgt != GlossVocabulary.EOS_ID)
            ).long()
            valid_lens = non_pad.sum(dim=1).clamp(min=1)

            embedded_text = self.decoder.token_emb(tgt)  # [B, 64, D]
        ...
        gpt2_loss = None
        if getattr(self, "gpt2_bridge", None) is not None:
            eff_gpt2 = gpt2_seq if gpt2_seq is not None else english_seq
            if eff_gpt2 is not None:
                try:
                    target_labels = eff_gpt2[:, 1:]
                    pad_token_id = getattr(self.gpt2_bridge.gpt2.config, "pad_token_id", None)
                    if pad_token_id is None:
                        pad_token_id = getattr(self.gpt2_bridge.gpt2.config, "eos_token_id", 50256)
                    pad_mask = (target_labels == pad_token_id)
                    if has_valid_english is not None:
                        pad_mask = pad_mask | (~has_valid_english.bool().unsqueeze(-1))
                    target_labels = torch.where(
                        pad_mask,
                        torch.tensor(-100, dtype=target_labels.dtype, device=target_labels.device),
                        target_labels,
                    )

                    gpt2_out = self.gpt2_bridge(h_seq, text_ids=eff_gpt2[:, :-1], labels=target_labels)
                    gpt2_loss = gpt2_out.loss
                    if gpt2_loss is not None:
                        if has_valid_english is not None:
                            has_any_eng = (has_valid_english.float().sum() > 0).to(gpt2_loss.dtype)
                            gpt2_loss = gpt2_loss * has_any_eng
                        gpt2_loss = torch.nan_to_num(gpt2_loss, nan=0.0, posinf=0.0, neginf=0.0)
                except Exception as e:
        ...
        return {
            ...
            "orig_x": orig_input_x if orig_input_x is not None else input_x,
            "ctc_log_probs": ctc_log_probs,
            "inter_ctc_log_probs": inter_ctc_log_probs,
            "early_ctc_log_probs": early_ctc_log_probs,
    ...
    def generate(
        ...
        if h_seq is not None:
            ...
            phonology_features = self.compute_phonology_features(features_3d, vel)
            ...
            h_seq, enc_mask = enc_res[1], enc_res[2]
        ...
        for step in range(max_new_tokens):
            ...
            if repetition_penalty != 1.0 and step > 0:
                V_size = next_token_logits.shape[-1]
                seen = torch.zeros((batch_sz, V_size), dtype=torch.bool, device=device)
                valid_prev = generated[:, : step + 1].clamp(min=0, max=V_size - 1)
                seen.scatter_(1, valid_prev, True)
                if dec_pad_id is not None and dec_pad_id < V_size:
                    seen[:, dec_pad_id] = False
                penalized = torch.where(
                    next_token_logits > 0,
                    next_token_logits / repetition_penalty,
                    next_token_logits * repetition_penalty,
                )
                next_token_logits = torch.where(seen, penalized, next_token_logits)
    ...
    def decode_ctc(
        ...
        """
        Fast Non-Autoregressive CTC Greedy Decoding.
        Decodes continuous sequences in O(1) single forward pass (~100x faster than autoregressive search).
        Automatically collapses consecutive identical tokens and filters out blank tokens.
        """
        if h_seq is None:
            if features is None:
                raise ValueError("Must provide either features or h_seq to decode_ctc()")
            pos = features[..., :3] if features.size(-1) >= 3 else features
            vel = features[..., 3:6] if features.size(-1) >= 6 else None
            phonology_features = self.compute_phonology_features(pos, vel)
            enc_res = self._encode(features, phonology_features, mask, frame_indices=frame_indices)
            h_seq = enc_res[1]

        if not hasattr(self, "ctc_head") or self.ctc_head is None:
            raise ValueError("Model does not have a configured ctc_head.")

        log_probs = self.ctc_head(h_seq)
        best_paths = log_probs.argmax(dim=-1)
        batch_results = []
        for b in range(best_paths.shape[0]):
            path = best_paths[b]
            collapsed = []
            prev = None
            for token in path:
...
def compute_chunked_linear_seq_and_eos_loss(
    ...
    with torch.no_grad():
        ...
        raw_nll_seq = _distributed_normalize((ce_unsmoothed * vf_seq).float().sum(), vf_seq.float().sum())
    ...
    return loss_seq, loss_eos, total_correct, c_valid, raw_nll_seq
...
def compute_seq_and_eos_loss(
    ...
    if return_raw_nll:
        vf_seq = valid_mask_seq.reshape(-1).to(lf.dtype) if valid_mask_seq is not None else torch.ones_like(tf, dtype=lf.dtype)
        vf_seq = vf_seq * (tf != eos_id).to(lf.dtype)
        if sw is not None:
            vf_seq = vf_seq * sw.to(lf.dtype)
        raw_nll_seq = _distributed_normalize((ce_unsmoothed * vf_seq).float().sum(), vf_seq.float().sum())
        return loss_seq, loss_eos, raw_nll_seq
...
def _compute_ctc_loss_safe(
    ...
    # For PyTorch CTC loss on XLA, lengths passed to F.ctc_loss must be strictly positive (>= 1)
    # to avoid ATen C++ edge-case branching / CPU synchronization stalls.
    # Non-gloss samples are given safe clamped lengths, and their loss is zeroed post-computation via valid_f.
    safe_target_lengths = torch.clamp(target_lengths * valid_ctc.to(torch.int32), min=1)
    actual_seq_len = torch.full(
        (batch_sz,), seq_len, dtype=torch.int32, device=ctc_log_probs.device
    )
    safe_input_lengths = torch.clamp(torch.minimum(input_lengths, actual_seq_len), min=1)
    ...
    loss_raw = loss_raw / safe_target_lengths.float().clamp_min(1.0)
...
def _move_batch_to_device(batch, device, prec_dtype, args, is_train=True):
    ...
    gpt2_seq = (
        batch["gpt2_seq"]
        if ("gpt2_seq" in batch and batch["gpt2_seq"] is not None and batch["gpt2_seq"].device == device)
        else (
            batch["gpt2_seq"].to(device, non_blocking=True)
            if ("gpt2_seq" in batch and batch["gpt2_seq"] is not None)
            else None
        )
    )
...
def _async_phase2_step_print(
    ...
    eng_nll_val = rest[0] if rest else eng_val
    ...
    eng_nll_val = log_vals[14] if len(log_vals) > 14 else eng_val
    eng_ppl_val = (
        math.exp(min(float(eng_nll_val), 20.0)) if float(eng_nll_val) > 0 else 0.0
    )
    pct = (float(st_idx) / max(1.0, float(m_batches))) * 100.0
    e_ppl_str = (
        f" PPL(Eng:{eng_ppl_val:.1f}) |" if float(eng_nll_val) > 0 else ""
    )
    ...
    try:
        ...
        with open(metrics_csv_path, "a", newline="", encoding="utf-8") as f:
            ...
            if write_header:
                writer.writerow(
                    ["epoch", "step", "phase", "loss", "acc", "seq", "ctc", "sem", "chi", "eng", "gpt2", "speed_seq_s"]
                    + [f"aux_{i}" for i in range(6)]
                )
            writer.writerow(
                [ep, st_idx, "train", float(l_val), g_acc, float(s_val), float(c_val), float(sm_val), float(chi_val), float(eng_val), float(gpt2_val), round(samples_per_s, 1)]
                + [0.0] * 6
            )
    except Exception:
        pass
...
def _async_phase2_closure_wrapper(mv, st_idx, m_batches, ep, tot_ep, lr_val, t_start, t_prev_box, b_sz, l_freq, mdl_dir="."):
    import torch_xla.core.xla_model as xm
    if xm.is_master_ordinal():
        ...
        log_vals = [
            ...
            mv_list[14], # e_nc_t
            mv_list[15], # e_nt_t
            mv_list[21] if len(mv_list) > 21 else mv_list[19], # eng_nll_val
        ]
        _async_phase2_step_print(log_vals, st_idx, m_batches, ep, tot_ep, lr_val, t_start, t_prev_box, b_sz, l_freq, mdl_dir=mdl_dir)
        # Step 2 metrics report disabled to prevent blocking TPU device pipeline
        pass
...
def train_epoch_tpu(
    ...
    # Static compilation invariants: keep hyperparameters constant across epochs to ensure
    # identical HLO graph hash, completely eliminating XLA recompilations and program binary loading OOM
    progress = 0.10
    grl_alpha = 0.10
    ...
    if is_xla:
        # On TPU v5e, default to 1 for safety, or use user-specified batches_per_execution
        bpe = getattr(args, "batches_per_execution", 1)
        ...
        try:
            para_loader = pl.MpDeviceLoader(
                loader,
                device,
                batches_per_execution=bpe,
        ...
        min_batches = min(min_batches, total_batches)
    else:
        para_loader = loader
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        def forward_and_losses(
            ...
            if dec_logits is not None:
                dec_len = dec_logits.size(1)
                eff_gt_tokens = gt_tokens[:, :dec_len]
                eff_valid_gloss_mask = valid_gloss_mask[:, :dec_len]
                eff_token_mask = token_mask[:, :dec_len]
                loss_seq, loss_eos = compute_seq_and_eos_loss(
                    dec_logits,
                    eff_gt_tokens,
                    eff_valid_gloss_mask,
                    eff_token_mask,
            ...
            if chicago_logits is not None and c_valid_seq_mask is not None:
                chi_len = chicago_logits.size(1)
                eff_c_sub = c_sub[:, :chi_len]
                eff_c_valid_seq_mask = c_valid_seq_mask[:, :chi_len]
                eff_c_tok_mask = c_tok_mask[:, :chi_len]
                loss_chicago, loss_chicago_eos = compute_seq_and_eos_loss(
                    chicago_logits,
                    eff_c_sub,
                    eff_c_valid_seq_mask,
                    eff_c_tok_mask,
                    sample_weights=sample_weight,
                    label_smoothing=0.1,
                    pad_id=GlossVocabulary.PAD_ID,
            ...
            english_raw_nll = torch.zeros((), device=device)
            if english_hidden is not None and e_valid_seq_mask is not None:
                loss_english, loss_english_eos, english_nc_t, english_nt_t, english_raw_nll = (
                ...
                loss_english, loss_english_eos, english_raw_nll = compute_seq_and_eos_loss(
                    english_logits,
                    e_sub,
                    e_valid_seq_mask,
                    e_tok_mask,
            ...
            enable_ctc = not getattr(args, "disable_ctc", False)
            ctc_freq = getattr(args, "ctc_freq", 1)
            compute_ctc_now = enable_ctc and (step_idx % max(1, ctc_freq) == 0)
            if compute_ctc_now:
            ...
            if inter_ctc_logits is not None and compute_ctc_now:
            ...
            if early_ctc_logits is not None and compute_ctc_now:
                ...
                loss_early_ctc = torch.zeros((), device=device)
            ...
            with torch.no_grad():
                # Gloss Metrics
                if dec_logits is not None:
                    d_len = dec_logits.size(1)
                    preds = dec_logits.argmax(dim=-1)
                    nc_t = ((preds == gt_tokens[:, :d_len]) & valid_gloss_mask[:, :d_len]).float().sum()
                    nt_t = valid_gloss_mask[:, :d_len].float().sum()
                else:
                    preds = gt_tokens
                    nc_t = torch.zeros((), device=device)
                    nt_t = torch.zeros((), device=device)

                # Chicago Metrics
                chicago_nc_t = torch.zeros((), device=device)
                chicago_nt_t = torch.zeros((), device=device)
                if chicago_logits is not None and c_valid_seq_mask is not None:
                    c_len = chicago_logits.size(1)
                    c_preds = chicago_logits.argmax(dim=-1)
                    chicago_nc_t = ((c_preds == c_sub[:, :c_len]) & c_valid_seq_mask[:, :c_len]).float().sum()
                    chicago_nt_t = c_valid_seq_mask[:, :c_len].float().sum()
        ...
        with torch.no_grad():
            # Static 22-element metric vector computed on 100% of steps (guarantees single static XLA graph)
            ...
            # Throttle warmup logging to steps 1, 5, 10 on epoch 1 to provide feedback without per-step host syncs
            should_log = (epoch == 1 and step_idx in (1, 5, 10)) or (step_idx % log_freq == 0) or (step_idx >= min_batches)
            save_dir_str = str(getattr(args, "save_dir", ".") or ".") if args else "."
            if is_xla:
                if should_log:
                    args_tuple = (
                        metrics_vec,
                ...
                if is_master and should_log:
                    ...
                    log_vals = [
                        ...
                        mv_list[15],
                        mv_list[21] if len(mv_list) > 21 else mv_list[19],
                    ]
        ...
        bpe_val = getattr(args, "batches_per_execution", 1) if args is not None else 1
        bpe_val = max(1, bpe_val)
        should_mark_step = (bpe_val <= 1) or (step_idx % bpe_val == 0) or (step_idx >= min_batches)
...
def validate_epoch_tpu(
    ...
    val_limit = getattr(args, "val_check_steps", 0)
    total_val_batches = get_dynamic_loader_len(loader, default_steps=(val_limit if val_limit > 0 else 500), args=args, is_val=True)
    if val_limit > 0:
        total_val_batches = min(total_val_batches, val_limit)
    ...
    if is_xla:
        ...
        if val_limit > 0:
            min_val_batches = min(min_val_batches, val_limit)
        bpe = 1 if is_xla else (max(1, getattr(args, "batches_per_execution", 1)) if args is not None else 1)
        para_loader = pl.MpDeviceLoader(loader, device, batches_per_execution=bpe)
    else:
        para_loader = loader
        if val_limit > 0:
            min_val_batches = min(total_val_batches, val_limit)
        ...
        for step_idx, batch in zip(range(1, min_val_batches + 1), para_loader):
            ...
            def forward_and_losses(
                ...
                if dec_logits is not None:
                    dec_len = dec_logits.size(1)
                    eff_gt_tokens = gt_tokens[:, :dec_len]
                    eff_valid_mask = valid_mask[:, :dec_len]
                    eff_token_mask = token_mask[:, :dec_len]
                    loss_seq, loss_eos = compute_seq_and_eos_loss(
                        dec_logits,
                        eff_gt_tokens,
                ...
                if dec_logits is not None:
                    ...
                    eff_valid_mask = valid_mask[:, :dec_len]
                    preds = dec_logits.argmax(dim=-1)
                    valid_f = eff_valid_mask.float()
                    nc_t = ((preds == eff_gt_tokens).float() * valid_f).float().sum()
                ...
                if chicago_logits is not None:
                    chi_len = chicago_logits.size(1)
                    c_sub = chicago_seq[:, 1:chi_len + 1]
                    eff_chi_valid = (
                        (c_sub != GlossVocabulary.PAD_ID)
                        & (c_sub != GlossVocabulary.EOS_ID)
                    ...
                    c_valid = eff_chi_valid.float()
                    c_nc_t = (
                        (chicago_logits.argmax(dim=-1) == c_sub).float()
                        * c_valid
                    ).sum()
                ...
                eng_raw_nll = torch.zeros((), device=device)
                if english_logits is not None:
                    eng_len = english_logits.size(1)
                    e_sub = english_seq[:, 1:eng_len + 1]
                    eff_eng_valid = (
                        (e_sub != EnglishVocabulary.PAD_ID)
                        & (e_sub != EnglishVocabulary.EOS_ID)
                        & (e_sub != EnglishVocabulary.UNK_ID)
                    ...
                    loss_eng, loss_eng_eos, eng_raw_nll = compute_seq_and_eos_loss(
                        english_logits,
                        e_sub,
                        eff_eng_valid,
                        eff_eng_token,
                    ...
                    e_valid = eff_eng_valid.float()
                    e_nc_t = (
                        (english_logits.argmax(dim=-1) == e_sub).float()
                        * e_valid
                    ).sum()
                ...
                eng_nll_sum = eng_raw_nll * e_nt_t
                return (
                    raw_loss,
                    loss_chi,
                    eng_nll_sum,
    ...
    val_eng_nll = (
        tracker["eng_loss"] / max(1.0, tracker.get("eng_total", 0.0))
        if tracker.get("eng_total", 0.0) > 0.0
        else 0.0
    )
    eng_ppl_str = (
        f" | EngPPL: {math.exp(min(val_eng_nll, 20.0)):.1f}"
        if val_eng_nll > 0
        else ""
    )
...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        if getattr(args, "profile_server", False):
            try:
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # If english_max_len <= 128, attention matrix is compact,
            # allowing batch 256-512 (2048-4096 global batch, 128x128 systolic MXU tiles) inside 1.37-4.5GB HBM.
            # If english_max_len > 128 (e.g. 256), cap to 64 to prevent OOM.
            eng_len = getattr(args, "english_max_len", 128)
            max_safe_batch = 512 if eng_len <= 128 else 64
        else:
            max_safe_batch = 512
    else:
        # GPU: 16-24GB VRAM
    ...
    if user_core_batch > 0:
        ...
        eng_len = getattr(args, "english_max_len", 128)
        max_allowed_tpu = 512 if (not is_gpt2_active or eng_len <= 128) else 64
        if IS_TPU and user_core_batch > max_allowed_tpu:
            if is_master:
                print(
                    f"[!] WARNING: --per-core-batch={user_core_batch} exceeds TPU v5e safety limit ({max_allowed_tpu}) with GPT-2. "
                    f"Capping to {max_allowed_tpu}. Running with global batch {max_allowed_tpu * world_size}.",
                    flush=True,
                )
            target_per_core_batch = max_allowed_tpu
        ...
        if IS_TPU:
            if requested_batch_size > 0:
                # User specified an explicit global batch size (e.g. 2048 -> 256 per core)
                target_per_core_batch = max(1, requested_batch_size // world_size)
            elif is_phase1:
                target_per_core_batch = 128
            elif is_gpt2_active:
                eng_len = getattr(args, "english_max_len", 128)
                target_per_core_batch = 128 if eng_len <= 128 else 32
            else:
                target_per_core_batch = 64
            requested_batch_size = target_per_core_batch * world_size
            args.batch_size = requested_batch_size
        else:
            target_per_core_batch = max(1, requested_batch_size // world_size)
    ...
    # NEVER auto-enable gradient checkpointing on TPU when activation memory easily fits in HBM.
    # Checkpointing creates duplicate attention subgraphs in XLA during backward, doubling forward compute time.
    no_grad_ckpt = getattr(args, "no_gradient_checkpointing", False)
    if no_grad_ckpt:
        args.gradient_checkpointing = False
        if is_master:
            print(
                f"[INFO] High-Throughput Mode: Gradient Checkpointing explicitly DISABLED via --no-gradient-checkpointing. "
                f"Eliminating redundant forward pass on backward to maximize TPU v5e MXU throughput.",
                flush=True,
            )
    elif getattr(args, "gradient_checkpointing", False):
        if is_master:
            print(
                f"[INFO] TPU Architecture Guard: Gradient Checkpointing explicitly enabled by user (scoped to trainable layers).",
                flush=True,
            )
    elif not is_phase1 and effective_loader_batch >= 64:
        args.gradient_checkpointing = True
        if is_master:
            print(
                f"[INFO] Auto-enabled Gradient Checkpointing for trainable layers (loader batch {effective_loader_batch} >= 64).",
                flush=True,
            )
    else:
        args.gradient_checkpointing = False

    # DataLoader workers on TPU:
    # Allow user-specified --num-dataloader-workers to be respected, ensuring background C++ prefetching
    # keeps MpDeviceLoader saturated without thread starvation.
    user_workers = getattr(args, "num_dataloader_workers", None)
    if user_workers is not None:
        effective_num_dl_workers = user_workers
    else:
        effective_num_dl_workers = 0 if IS_TPU else 2
    ...
    if hasattr(args, "phase1_epochs") and args.phase1_epochs > 0:
        ...
        if IS_TPU:
            # High-Throughput Phase 1: On TPU v5e, text pre-training has no heavy pose graphs or bridges
            phase1_core_batch = max(getattr(args, "phase1_core_batch", 0), target_core_batch, 256)
    ...
    train_loader = create_dataloader(
        ...
        gloss_max_len=getattr(args, "gloss_max_len", getattr(args, "english_max_len", 128)),
        drop_last=True,
    )
    ...
    model = ASLFoundationModel(
        ...
        max_dec_len=getattr(args, "gloss_max_len", getattr(args, "max_len", 256)),
        ...
        torch_dtype=torch.bfloat16 if getattr(args, "precision", "float32") == "bfloat16" else None,
    ).to(device)

    # Tie embeddings only if explicitly requested by args (disabled by default to preserve pretrained embeddings)
    if getattr(args, "tie_weights", False):
        if hasattr(model, "english_decoder") and model.english_decoder is not None and hasattr(model.english_decoder, "token_emb"):
            model.english_decoder.token_emb.weight = model.english_decoder.lm_head.weight
        if hasattr(model, "chicago_decoder") and model.chicago_decoder is not None and hasattr(model.chicago_decoder, "token_emb"):
            model.chicago_decoder.token_emb.weight = model.chicago_decoder.lm_head.weight
        if hasattr(model, "decoder") and model.decoder is not None and hasattr(model.decoder, "token_emb"):
    ...
    global_min_batches = min(global_min_batches, train_loader_len)

    opt_groups = _get_optimizer_groups(model, loss_wrapper, args.weight_decay)
    if IS_TPU:
        try:
            from torch_xla.amp import syncfree
            optimizer = syncfree.AdamW(opt_groups, lr=args.lr)
            if is_master:
                print("[INFO] TPU Acceleration: Enabled syncfree.AdamW optimizer for zero-sync updates.", flush=True)
        except Exception:
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if is_master:
                ...
                try:
                    save_dir_val = getattr(args, "save_dir", ".") or "."
                    csv_target = os.path.join(save_dir_val, "training_metrics.csv")
                    out_target = os.path.join(save_dir_val, "loss_curves.png")
                    print_console_line_charts(epoch, csv_path=csv_target)
                    save_epoch_loss_curves_png(epoch, csv_path=csv_target, out_path=out_target)
                except Exception:
                    pass
            ...
            # Save checkpoint every 5 epochs, on epoch 1, or on final epoch
            save_freq = min(getattr(args, "checkpoint_freq", 5), getattr(args, "save_every_epoch", 5))
            if epoch % save_freq == 0 or epoch == 1 or epoch == args.epochs:
...
def _async_phase1_step_print(loss_cpu, acc_eng_cpu, acc_gloss_cpu, step_idx, step_fmt_str, lr_val, elapsed_val, samples_per_sec_val, steps_per_sec_val, ep, tot_ep, save_dir=".", loss_eng_cpu=None):
    try:
        if IS_TPU:
            import torch_xla.core.xla_model as xm
            if not xm.is_master_ordinal():
        ...
        if loss_eng_cpu is not None:
            try:
                loss_eng_val = float(loss_eng_cpu.item()) if hasattr(loss_eng_cpu, "item") else float(loss_eng_cpu)
            except Exception:
                loss_eng_val = float(loss_eng_cpu) if isinstance(loss_eng_cpu, (int, float)) else loss_val
        else:
            loss_eng_val = loss_val
        ...
        ppl = math.exp(min(loss_eng_val, 20.0))

        msg = (
            f"Phase 1 | Epoch {ep:03d}/{tot_ep:03d} | "
            f"Step {step_fmt_str} | Loss: {loss_val:.4f} (EngLoss: {loss_eng_val:.4f}) | Acc(E): {acc_e_val:.2f}% | Acc(G): {acc_g_val:.2f}% | PPL(Eng): {ppl:.1f} | "
        ...
        try:
            metrics_csv_path = os.path.join(save_dir if save_dir else ".", "phase1_metrics.csv")
            write_header = not os.path.exists(metrics_csv_path)
            with open(metrics_csv_path, "a", encoding="utf-8") as f:
                if write_header:
                    f.write("phase,epoch,step,loss,acc_eng,acc_gloss,ppl,speed_samp_s\n")
                f.write(f"phase1,{ep},{step_fmt_str},{loss_val:.4f},{acc_e_val:.2f},{acc_g_val:.2f},{ppl:.1f},{samples_per_sec_val:.1f}\n")
        except Exception:
            pass
    except Exception as e:
        try:
...
def prune_tmp_xla_caches(base_cache_path=None):

    """Safely prunes duplicate worker rank compilation caches and old verbose TPU logs to prevent exhausting Kaggle /tmp disk quota."""
    try:
        if not base_cache_path:
            base_cache_path = os.environ.get("XLA_PERSISTENT_CACHE_PATH", "/tmp/xla_cache")
        parent = os.path.dirname(base_cache_path)
        base_name = os.path.basename(base_cache_path)
        # Prune duplicate worker rank directories 1 through 7
        for r in range(1, 8):
            rank_dir = os.path.join(parent, f"{base_name}_{r}")
            if os.path.exists(rank_dir):
                import glob
                for f in glob.glob(os.path.join(rank_dir, "*")):
                    try:
                        os.remove(f)
                    except Exception:
                        pass
        # Prune old TPU logs older than 30 mins
        tpu_logs_dir = "/tmp/tpu_logs"
        if os.path.exists(tpu_logs_dir):
            import time, glob
            cutoff = time.time() - 1800
            for f in glob.glob(os.path.join(tpu_logs_dir, "*")):
                try:
...
def text_pretrain_loop(args, device, is_master, per_core_batch=None, accum_steps=1):
                        ...
                        for fname in candidate_vocab_names:
                            ...
                            if match:
                                ...
                                if isinstance(raw_map, dict) and "label_to_idx" in raw_map:
                                    ...
                                    gloss_vocab = GlossVocabulary(label_to_idx=raw_map)
                                if gloss_vocab is not None:
                                    print(f"[INFO] Phase 1 GlossVocabulary loaded from zip '{d}!/{match}'", flush=True)
                                    break
                except Exception:
                    pass
            else:
    ...
    p1_bpe = max(1, getattr(args, "batches_per_execution", 1))
    if is_master and p1_bpe > 1:
        print(f"[INFO] Phase 1: Enabled batches_per_execution={p1_bpe} for double-buffered device infeed.", flush=True)
    if IS_TPU:
    ...
    for epoch in range(start_epoch, args.phase1_epochs):
        ...
        for step, batch in enumerate(loader):
            should_log = (epoch == start_epoch and step < 5) or ((step + 1) % getattr(args, "log_freq", 25) == 0) or ((step + 1) == total_steps)
...
def build_parser():
    ...
    parser.add_argument(
        "--phase1-only",
        action="store_true",
        help="Run Phase 1 text pretraining only and exit immediately",
    )
    parser.add_argument(
        "--phase1-core-batch",
        type=int,
        default=256,
        help="Per-core batch size override for Phase 1 text pretraining (Default: 256 for >=8k samples/s)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Explicit per-core micro-batch override (Default: 0 for auto-calculation from --batch-size // world_size)",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable auxiliary intermediate CTC loss (Default: False to eliminate CPU dynamic programming fallback stalls)",
    )
    parser.add_argument(
        "--ctc-freq",
        type=int,
        default=1,
        help="Frequency (in steps) to compute CTC loss to amortize CPU fallback overhead (Default: 1)",
    )
    parser.add_argument(
        "--disable-ctc",
        action="store_true",
        default=False,
        help="Completely disable CTC loss to achieve pure accelerator execution and 2k+ seq/s throughput",
    )
    parser.add_argument(
        "--disable-bpe",
        action="store_true",
        help="Disable HuggingFace BPE Tokenizer for English decoder and use custom EnglishVocabulary",
    )
    parser.add_argument(
        "--use-gpt2",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Max decoder sequence length for ASL gloss head (Default: 128)",
    )
    parser.add_argument(
        "--compile",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Explicitly disable gradient checkpointing to eliminate redundant forward pass recomputations when HBM memory is sufficient.",
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Skip autoregressive token generation during validation to prevent unrolled graph compilation hangs on TPU (default: True)",
    )
    parser.add_argument(
        "--enable-val-generation",
        action="store_true",
        default=False,
        help="Explicitly enable autoregressive generation during validation (not recommended on TPU)",
    )
    parser.add_argument(
        "--val-check-steps",
        type=int,
        default=0,
        help="Max validation batches to evaluate per epoch (0 = evaluate full validation set)",
    )
    parser.add_argument(
        "--single-process",
        action="store_true",
        default=False,
        help="Run on a single TPU process directly on xla:0 without xmp.spawn",
    )
    return parser
...
def _apply_model_scale_presets(args):
    ...
    if scale == "xl":
        ...
        if not hasattr(args, "mtp_k") or args.mtp_k == 2:
            args.mtp_k = 4
        setattr(args, "enable_inter_ctc", True)
        setattr(args, "enable_early_ctc", True)
    elif scale == "base":
        if args.d_model == 512:
            args.d_model = 320
        if args.num_layers == 12:
...
def main():
    ...
    if IS_TPU:
        if "LOCAL_RANK" in os.environ:
            ...
            if getattr(args, "single_process", False):
                print("[INFO] Single TPU worker process mode enabled. Running directly on xla:0...", flush=True)
                _tpu_worker_fn(0, args)
            else:
                # Spawns 8 processes (one per TPU core) to utilize all 8 cores and exceed 2,000 seq/s
                import torch_xla.distributed.xla_multiprocessing as xmp

                # Pop conflicting Kaggle TPU addresses so local 8-core mesh initializes cleanly
                os.environ.pop("TPU_PROCESS_ADDRESSES", None)
                ...
                try:
                    _start_method = "fork" if os.name != "nt" else "spawn"
                    ...
                    print(
                        f"[WARNING] xmp.spawn encountered exception: {_spawn_err}. Falling back to direct single-process execution on xla:0...",
                        flush=True,
                    )
                    _tpu_worker_fn(0, args)
    else:
    ...
    if not (getattr(args, "epochs", 0) == 0 or getattr(args, "phase1_only", False)):
        try:
...
def print_console_line_charts(epoch, csv_path="training_metrics.csv"):
    if not os.path.exists(csv_path):
        candidate = "training_metrics.csv"
...
def generate_training_report(args, device):
    ...
    # 1. Plot Metrics
    csv_candidates = [
        os.path.join(getattr(args, "save_dir", "."), "training_metrics.csv"),
        "training_metrics.csv",
    ]
    csv_target = next((c for c in csv_candidates if os.path.exists(c)), None)
    if csv_target is not None:
        try:
            df = pd.read_csv(csv_target, on_bad_lines="skip")
            if "mode" in df.columns:
                df = df.rename(
                    columns={"mode": "phase", "loss_total": "loss", "acc_gloss": "acc"}
                )
                ...
                df = pd.read_csv(
                    csv_target, names=["epoch", "step", "phase", "loss", "acc"], on_bad_lines="skip"
                )
        except Exception:
            df = pd.DataFrame()
        ...
        plt.legend()
        plt.grid(True)
        out_loss_path = os.path.join(getattr(args, "save_dir", "."), "loss_curves.png")
        plt.savefig(out_loss_path)
        print(f"Saved loss curves to {out_loss_path}")
        plt.close()
    ...
    val_loader = create_dataloader(
        dataset_dir=args.data_dir,
        split="val",
        batch_size=min(16, max(1, getattr(args, "batch_size", 8))),
    ...
    model = ASLFoundationModel(
        channels_per_kp=getattr(args, "channels_per_kp", 9),
        vocab_size=actual_vocab_size,
        d_enc=args.d_model,
        d_dec=args.d_model,
        nhead_enc=args.nhead,
        nhead_dec=args.nhead,
        num_enc_layers=getattr(args, "num_enc_layers", 0),
        num_dec_layers=args.num_layers,
        dropout=args.dropout,
        english_vocab_size=aligned_eng_vocab_size,
        eng_pad_id=eng_vocab.PAD_ID,
        eng_bos_id=eng_vocab.BOS_ID,
        eng_eos_id=eng_vocab.EOS_ID,
        max_enc_len=getattr(args, "max_enc_len", getattr(args, "max_len", 256)),
        max_dec_len=getattr(args, "max_dec_len", getattr(args, "max_len", 256)),
        use_mamba=getattr(args, "use_mamba", True),
        tome_r=getattr(args, "tome_r", 0),
        scale_embeddings=getattr(args, "scale_embeddings", True),
        use_swin_1d=getattr(args, "use_swin_1d", getattr(args, "use_swin", False)),
        swin_window_size=getattr(args, "swin_window_size", getattr(args, "swin_window", 128)),
        is_causal=getattr(args, "is_causal", False),
        enable_aux_decoders=getattr(args, "enable_aux_decoders", True),
    ).to(device)
    ...
    ckpt_candidates = [
        os.path.join(save_dir, "asl_model_latest.pt"),
    ...
    if ckpt_path is not None:
        try:
            ...
            state_dict = loaded.get("model_state_dict", loaded.get("model", loaded)) if isinstance(loaded, dict) else loaded
            target_state = model.state_dict()
            adapted_state = {}
            for k, v in state_dict.items():
                clean_k = k.replace("module.", "")
                if clean_k in target_state and target_state[clean_k].shape == v.shape:
                    adapted_state[clean_k] = v
            missing, unexpected = model.load_state_dict(adapted_state, strict=False)
            if hasattr(model, "english_decoder") and model.english_decoder is not None and hasattr(model.english_decoder, "token_emb"):
                model.english_decoder.token_emb.weight = model.english_decoder.lm_head.weight
            if hasattr(model, "chicago_decoder") and model.chicago_decoder is not None and hasattr(model.chicago_decoder, "token_emb"):
            ...
            if hasattr(model, "decoder") and model.decoder is not None and hasattr(model.decoder, "token_emb"):
                model.decoder.token_emb.weight = model.decoder.lm_head.weight
            print(f"[INFO] Loaded checkpoint from {ckpt_path} ({len(adapted_state)}/{len(target_state)} tensors transferred)", flush=True)
        except Exception as e:
            print(f"[WARNING] Could not load checkpoint from {ckpt_path}: {e}. Evaluating with current model weights.", flush=True)
    else:
        print(f"Warning: Checkpoint not found in {save_dir}. Using current model weights.", flush=True)

    model.eval()

    word_counts = collections.Counter()
    max_eval_batches = min(5, getattr(args, "val_batches", 5))

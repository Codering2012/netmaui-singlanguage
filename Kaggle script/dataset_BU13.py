...
def get_gpt2_tokenizer():
    ...
    try:
        ...
        local_tok = "/kaggle/working/gpt2_tok"
        if os.path.exists(local_tok):
            try:
                _GLOBAL_GPT2_TOKENIZER = GPT2Tokenizer.from_pretrained(local_tok)
                return _GLOBAL_GPT2_TOKENIZER
            except Exception:
                pass
        candidate_paths = [
        ...
        try:
            ...
            _GLOBAL_GPT2_TOKENIZER.save_pretrained(local_tok)
        except Exception:
            pass
    except Exception:
...
class LandmarkAugmenter:
    ...
    def __call__(
        ...
        if np.random.rand() < finger_drop_prob:
            all_finger_groups = [
                list(range(1, 5)),    # LH Thumb (1..4)
                list(range(5, 9)),    # LH Index (5..8)
                list(range(9, 13)),   # LH Middle (9..12)
                list(range(13, 17)),  # LH Ring (13..16)
                list(range(17, 21)),  # LH Pinky (17..20)
                list(range(22, 26)),  # RH Thumb (22..25)
                list(range(26, 30)),  # RH Index (26..29)
                list(range(30, 34)),  # RH Middle (30..33)
                list(range(34, 38)),  # RH Ring (34..37)
                list(range(38, 42)),  # RH Pinky (38..41)
            ]
...
def apply_spatial_kinematic_augmentations(
    ...
    # 4. Localized Gaussian Jitter on Hand Keypoints (LH: 0..20, RH: 21..41)
    if pos.shape[1] >= 42 and np.random.rand() < 0.5:
        hand_jitter = np.random.normal(0.0, jitter_std, size=(pos.shape[0], 42, 3)).astype(np.float32)
        pos[:, 0:42, :] += hand_jitter
...
def apply_hand_masking_augmentation(
    ...
    if choice == 0 and out_arr.shape[1] >= 21:
        out_arr[start_t:end_t, 0:21, :] = 0.0   # LH (0..20)
    elif choice == 1 and out_arr.shape[1] >= 42:
        out_arr[start_t:end_t, 21:42, :] = 0.0  # RH (21..41)
    elif choice == 2 and out_arr.shape[1] >= 42:
        out_arr[start_t:end_t, 0:42, :] = 0.0   # Both Hands (0..41)
...
def clean_out_of_bounds_hands(feat_arr: np.ndarray) -> np.ndarray:
    ...
    """
    if feat_arr.ndim != 3 or feat_arr.shape[1] < 42 or feat_arr.shape[0] == 0:
        return feat_arr
    ...
    for h_start, h_end in [(0, 21), (21, 42)]:
        ...
        # Non-zero check
        is_active = (np.abs(h_pos).sum(axis=(1, 2)) > 1e-4)
        ...
        spatial_std = h_pos.std(axis=1).mean(axis=-1)
...
class ASLShardedDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Neutralize phantom points from isolated cropped hand datasets (ASL_Alphabet / Synthetic_Numbers / static_alphabet):
        # MediaPipe Holistic hallucinated torso (42..47) and face (48..59) keypoints because no body was in the frame.
        # We zero them out to force the model to rely purely on valid hand shape and finger articulation.
        if source_str in ("ASL_Alphabet", "Synthetic_Numbers") or task_str == "static_alphabet":
            if feat_arr.shape[1] >= 60:
                feat_arr[:, 42:60, :] = 0.0

        # Sanitize out-of-bounds, exploded, or collapsed tracking artifacts on both hands
        if feat_arr.shape[0] > 0 and feat_arr.shape[1] >= 42:
            feat_arr = clean_out_of_bounds_hands(feat_arr)
        ...
        MAX_GLOSS_LEN = min(self.max_len, getattr(self, "gloss_max_len", None) or 128)
        MAX_CHICAGO_LEN = min(self.max_len, getattr(self, "chicago_max_len", None) or 128)
        MAX_ENGLISH_LEN = min(self.max_len, getattr(self, "english_max_len", None) or 128)
        MAX_GPT2_LEN = getattr(self, "gpt2_max_len", 112 if MAX_ENGLISH_LEN <= 128 else 240)
...
class ASLStreamedDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for shard_path in shards:
                try:
                    if hasattr(self, "_shard_cache") and shard_path in self._shard_cache:
                        items = self._shard_cache[shard_path]
                        if self.split == "train":
                            items = list(items)
                            random.shuffle(items)
                    else:
                        ...
                        if not hasattr(self, "_shard_cache"):
                            self._shard_cache = {}
                        if len(self._shard_cache) < 500:
                            self._shard_cache[shard_path] = list(items)
                ...
                if not hasattr(self, "_shard_cache") or shard_path not in self._shard_cache:
                    del items
    ...
    def _process_record(
        ...
        # Neutralize phantom points from isolated cropped hands (ASL_Alphabet / Synthetic_Numbers / static_alphabet)
        rec_src = str(rec.get("source", ""))
        rec_task = str(rec.get("task", ""))
        if rec_src in ("ASL_Alphabet", "Synthetic_Numbers") or rec_task == "static_alphabet":
            if self.num_keypoints >= 60:
                feat_out[:T_cap, 42:60, :] = 0.0

        # Note: Hand tracking artifact sanitization is vectorized across the entire batch in fast_vectorized_collate_fn
        # for >3000x faster execution without Python GIL stalling
...
def fast_vectorized_collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    # High-Throughput Batched Vectorized Hand Sanitization: cleans tracking artifacts across [B, T, K, C] in 1 fused pass
    if features.shape[2] >= 42:
        for h_start, h_end in [(0, 21), (21, 42)]:
            h_pos = features[:, :, h_start:h_end, :3].float()
            wrist = h_pos[:, :, 0:1, :]
            is_act = (h_pos.abs().sum(dim=-1).sum(dim=-1) > 1e-4)
            oob = (h_pos[..., 0].abs() > 2.2) | (h_pos[..., 1].abs() > 2.5)
            has_oob = oob.any(dim=-1)
            dists = torch.norm(h_pos - wrist, dim=-1)
            has_exploded = (dists.max(dim=-1).values > 0.55)
            spatial_std = h_pos.std(dim=2).mean(dim=-1)
            has_collapsed = (spatial_std < 0.015) & is_act
            inv = (has_oob | has_exploded | has_collapsed) & is_act
            if inv.any():
                features[:, :, h_start:h_end, :][inv] = 0.0
    ...
    if "gpt2_seq" in first and first["gpt2_seq"] is not None:
        if isinstance(first["gpt2_seq"], np.ndarray):
            ...
            raw_lists = [b.get("gpt2_seq", [50256]) for b in batch]
            max_g_len = max((len(l) for l in raw_lists), default=1)
            padded_lists = [list(l) + [50256] * (max_g_len - len(l)) for l in raw_lists]
            gpt2_seq = torch.tensor(padded_lists, dtype=torch.long)
    else:
        gpt2_seq = None
...
class KDWDDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        if getattr(self, "cached_padded", None) is None:
...
class ASLGPC12Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.max_merge_samples = max_merge_samples
        try:
            _p_tokens = self.eng_vocab.encode(".") if hasattr(self.eng_vocab, "encode") else [41]
            self.period_id = _p_tokens[0] if _p_tokens else 41
        except Exception:
            self.period_id = 41
    ...
    def __getitem__(self, idx: int):
        ...
        if total_samples > 0:
            ...
            if getattr(self, "enable_sentence_merging", False) and (item_idx % 2 == 1):
                ...
                period_id = getattr(self, "period_id", 41)

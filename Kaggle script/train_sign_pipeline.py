...
def save_sharded_payload(payload, split, output_dir, shard_size=1000):
    """Save the canonical payload and, when large enough, write shard copies too."""
    ...
    records = payload.get("isolated_records")
    if not isinstance(records, list) or len(records) <= shard_size:
        return out_pt
    ...
    print(f"[*] Sharding {split} payload into {num_shards} chunks for easier reuse.")
    for i in range(num_shards):
        start = i * shard_size
        end = min(len(records), (i + 1) * shard_size)
        shard_records = records[start:end]
        shard_payload = dict(payload)
        shard_payload["isolated_records"] = shard_records
        if isinstance(sequences, list):
            shard_payload["sequences"] = sequences[start:end]
        if isinstance(lengths, np.ndarray) and len(lengths) >= end:
            shard_payload["lengths"] = lengths[start:end]
        shard_payload.setdefault("metadata", {})
        shard_payload["metadata"] = {
            **dict(shard_payload["metadata"]),
            "shard_index": i,
            "total_shards": num_shards,
            "shard_range": [start, end],
        }
        shard_path = shard_dir / f"asl_frankenstein_{split}_shard_{i:03d}.pt"
        tmp_shard_path = shard_dir / f"asl_frankenstein_{split}_shard_{i:03d}.pt.tmp"
...
def build_split(processor, split, normalizer, augment=False, label_to_idx=None):
    ...
    isolated_records.extend(processor.process_asl_alphabet(split=split))
    isolated_records.extend(processor.process_asl_citizen(split=split))
    isolated_records.extend(processor.process_chicago_fswild(split=split))
    isolated_records.extend(processor.process_synthetic_numbers(split=split))
    isolated_records.extend(processor.process_wlasl(split=split))
    sentence_records.extend(processor.process_how2sign_holistic(split=split))

    fused_dataset = FusedASLDataset(
        isolated_records, label_to_idx=label_to_idx, transform=None
    )

    processed_isolated = []
    for idx in tqdm(range(len(fused_dataset)), desc=f"Serializing Fused Map [{split}]"):
        item = fused_dataset[idx]
        processed_isolated.append(
            {
                "task": "isolated_gloss",
                "label": fused_dataset.idx_to_label[item["label"].item()],
                "signer_id": item["signer_id"],
                "features": item["features"].numpy().astype(np.float16),
                "source": item["source"],
                "split": item["split"],
                "quality": float(item["quality"].item()),
                "sample_weight": float(item["sample_weight"].item()),
            }
        )

    # Variable-length sequence list + per-sample frame counts.
    # Each element of `sequences` is a float16 array of shape (T_i, NUM_LANDMARKS, C)
    # where T_i varies per video.  `lengths` provides the matching T_i values.
    sequences = [r["features"] for r in processed_isolated]
    lengths = np.array([s.shape[0] for s in sequences], dtype=np.int32)

    save_payload = {
        "isolated_records": processed_isolated,  # per-record dicts (backward compat)
        "sequences": sequences,  # list of variable-T float16 arrays
        "lengths": lengths,  # (N,) int32  — frame count per sample
        "sentence_records": sentence_records,
        "vocabulary": fused_dataset.classes,
        "label_to_idx": fused_dataset.label_to_idx,
        "metadata": {
            "split": split,
            "target_fps": TARGET_FPS,
            "num_landmarks": NUM_LANDMARKS,
            "sequence_mode": "variable_length_native_float16",
            "quality_threshold": processor.quality_threshold,
        },
    }

    processor.save_quality_summary(split)
    return save_payload, fused_dataset

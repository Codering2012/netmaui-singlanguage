...
def fast_vectorized_v3_collate_fn(
    ...
    # G. Hand Presence Mask [B, actual_len, 2] (Left Hand, Right Hand)
    has_hmask = any(b.get("hand_mask") is not None for b in batch)
    if has_hmask:
        hmask_list = []
        for item in batch:
            hm = item.get("hand_mask")
            if hm is None:
                hmask_list.append(torch.ones((actual_len, 2), dtype=torch.bool))
                continue
            if not isinstance(hm, torch.Tensor):
                hm = torch.tensor(hm, dtype=torch.bool)
            cur_len = hm.shape[0]
            if cur_len < actual_len:
                hm = F.pad(hm, (0, 0, 0, actual_len - cur_len), value=False)
            elif cur_len > actual_len:
                hm = hm[:actual_len]
            hmask_list.append(hm)
        collated["hand_mask"] = torch.stack(hmask_list, dim=0)
    else:
        collated["hand_mask"] = None

    # H. Stochastic Modality Dropout for multi-sensor robust deployment
    ...
    # I. Target Sentence Embeddings [B, 384] for Stage 2 & 3 Semantic Anchoring
    has_sent = any((b.get("sentence_embedding") is not None or b.get("target_sentence_embeddings") is not None) for b in batch)
    if has_sent:
        sent_list = []
        for item in batch:
            se = item.get("sentence_embedding", item.get("target_sentence_embeddings"))
            if se is None:
    ...
    # J. Text Tokens explicit mapping from english_seq
    if "text_tokens" not in collated and "english_seq" in collated:

    ...
    def forward(self, losses: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        for name, loss_val in losses.items():
            if name in self.log_vars:
                ...
                s = torch.clamp(self.log_vars[name], min=-4.0, max=8.0)
...
class V3TrainingOrchestrator:
    ...
    def __init__(
        ...
        # Optimize model + loss wrapper parameters jointly
        all_params = list(self.model.parameters()) + list(self.loss_wrapper.parameters())
    ...
    def train_step(self, batch: Dict[str, Any], sync_metrics: bool = False) -> Dict[str, Any]:
        ...
        # Translation Cross Entropy Loss if text tokens are present
        if output.decoder_logits is not None and text_tokens is not None and text_tokens.shape[1] > 1:

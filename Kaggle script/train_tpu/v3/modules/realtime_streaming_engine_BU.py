...
class RealtimeAdaptiveStreamer:
    ...
    def _process_chunk(self) -> List[int]:
        ...
        with torch.no_grad():
            ...
            # Greedy CTC decoding of English tokens on this chunk
            ctc_logits = output.english_ctc_logits  # [1, T, V]
            pred_tokens = torch.argmax(ctc_logits, dim=-1)[0].tolist()

            # Collapse consecutive repeats and blanks (0 = PAD/BLANK)
            collapsed = []
            prev = None
            ...
            # Flicker-Free Local Agreement Prefix Commit:
            # Add to hypothesis history
            self.hypothesis_history.append(collapsed)
            if len(self.hypothesis_history) > self.commit_horizon:
                self.hypothesis_history.pop(0)

            # Find longest common prefix across the hypothesis history
            if len(self.hypothesis_history) == self.commit_horizon:
                common_prefix = self._get_common_prefix(self.hypothesis_history)
                # Commit new tokens that haven't been committed yet
                for tok in common_prefix[len(self.committed_tokens):]:
                    self.committed_tokens.append(tok)

        return collapsed

    def _get_common_prefix(self, token_lists: List[List[int]]) -> List[int]:
        """Finds the longest common prefix among multiple hypothesis token lists."""
        if not token_lists:
            return []
        min_len = min(len(tl) for tl in token_lists)
        prefix = []
        for i in range(min_len):
            first_tok = token_lists[0][i]
            if all(tl[i] == first_tok for tl in token_lists):

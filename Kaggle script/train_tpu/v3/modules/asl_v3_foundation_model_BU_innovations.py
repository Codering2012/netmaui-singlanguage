...
class ASLV3FoundationModel(nn.Module):
    ...
    def forward(
        ...
        # Apply on-device physical SpecAugment-Sign augmentation during training
        if self.training and enable_specaugment:
        ...
        # 8. Engine 4: Monotonic Chunk Permutation Transducer (OSV -> SVO reordering via Log-Domain Sinkhorn)
        h_reordered, P_sinkhorn = self.sinkhorn_transducer(h)

        # 8b. Innovation 3: Multi-Granularity Sentence-Embedding Semantic Anchor
        if target_sentence_embeddings is not None:
            multi_task_losses["loss_semantic_bridge"] = self.semantic_anchor(h_reordered, target_sentence_embeddings)

        # 8c. Innovation 1: Harsh Masked Articulator Modeling (MAM)
        if self.training and enable_mam:
            mam_mask = self.mam.generate_mask(B, T, kinematics.device)
            multi_task_losses["loss_mam"] = self.mam.compute_loss(h, kinematics, mam_mask)

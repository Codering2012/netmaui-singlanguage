...
def _distributed_normalize(
    ...
    local_sum = torch.nan_to_num(local_sum, nan=0.0, posinf=0.0, neginf=0.0)
    normed = local_sum / local_weight.clamp_min(1e-8)
    return torch.where(local_weight > 0, normed, torch.zeros_like(normed))
...
class DecoderCrossAttention(nn.Module):
    ...
    def forward(
        ...
        attn_mask = (
            memory_key_padding_mask.view(batch_sz, 1, 1, -1).to(query_q.dtype) * -10000.0
...
def xla_clip_grad_norm_(parameters, max_norm=1.0, norm_type=2.0):
    """XLA-safe distributed gradient clipping delegating to PyTorch native clip_grad_norm_ (patched by torch_xla)."""
    if isinstance(parameters, torch.Tensor):
        parameters = [parameters]
    params = [p for p in parameters if p.grad is not None]
    if not params:
        return torch.tensor(0.0)
    return torch.nn.utils.clip_grad_norm_(params, max_norm=float(max_norm), norm_type=float(norm_type))
...
class SignToGPT2Bridge(nn.Module):
    ...
    def forward(
        ...
        # Fused Linear Cross Entropy: computes loss and analytical gradients in static systolic-aligned tiles of 768 tokens (6 x 128).
        # NEVER materializes full [N, 50257] in memory and saves ZERO logits on autograd tape.
        # Keeps peak chunk memory <78MB and eliminates XLA tile padding overhead.
...
def train_epoch_tpu(
    ...
    try:
        ...
        total_batches = default_step_limit
    total_batches = min(total_batches, default_step_limit)
    ...
    if is_xla:
        ...
        min_batches = min(min_batches, default_step_limit)
    else:
        para_loader = loader
        min_batches = min(total_batches, default_step_limit)
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        def forward_and_losses(
            ...
            loss_bone = out.get("bone_loss", torch.zeros((), device=device))
...
def validate_epoch_tpu(
    ...
    default_val_limit = getattr(args, "val_check_steps", 0) or 50
    try:
        total_val_batches = len(loader)
    except TypeError:
        total_val_batches = default_val_limit
    total_val_batches = min(total_val_batches, default_val_limit)
    ...
    if is_xla:
        ...
        min_val_batches = min(min_val_batches, default_val_limit)
        bpe = 1 if is_xla else (getattr(args, "batches_per_execution", 1) if args is not None else 1)
        para_loader = pl.MpDeviceLoader(loader, device, batches_per_execution=bpe)
    else:
        para_loader = loader
        min_val_batches = min(total_val_batches, default_val_limit)
...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        # 2. In Phase 2 with GPT-2 at sequence length 256 and 3 multi-task decoders (Gloss, Chicago, English)
        #    active simultaneously with zero bypasses: running per-core micro-batch of 16 (with auto
        #    accum_steps=8 for global batch 1024) restricts peak activations to ~4.5 GB, leaving >10 GB
        #    of HBM safety headroom on 15.75 GB TPU v5e/v3 while fully saturating TPU MXU systolic arrays!
        # 3. In Phase 2 standard decoder (no GPT-2): vocab is 3,586, allowing per-core batch 64.
        if is_phase1:
            max_safe_batch = 256
        elif is_gpt2_active:
            max_safe_batch = 16
    ...
    if user_core_batch > 0:
        max_safe_batch = user_core_batch
    ...
    # Auto-enable gradient checkpointing only when physical micro-batch per core >= 64.
    # For micro-batch 16, checkpointing is unneeded (<4.5GB activations) and avoiding it prevents
    # XLA remat graph duplication across the 5 consumers of h_seq.
    if not is_phase1 and effective_loader_batch >= 64 and not getattr(args, "gradient_checkpointing", False):
        args.gradient_checkpointing = True
        if is_master:
            print(
                f"[INFO] High-Throughput Mode: Auto-enabled Gradient Checkpointing for Phase 2 per-core batch {effective_loader_batch} "
    ...
    try:
        ...
        train_loader_len = default_step_limit
        
    train_loader_len = min(train_loader_len, default_step_limit)
    ...
    global_min_batches = min(global_min_batches, default_step_limit)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=400,
        help="Max steps per training epoch (default: 400, strictly < 500).",
    )
    parser.add_argument(
        "--skip-val-generation",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Stop validation early after this many steps (default: 50).",
    )
    parser.add_argument(
        "--phase1-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Explicit per-core micro-batch override (e.g. 16 or 32; default: 0 for auto-calculated safe batch)",
    )

...
try:
    ...
    from vac_smkd_engine import ASLVACSMKDEngine
    from sign_clip_engine import ASLSignCLIPEngine
    from anatomical_barrier_engine import ASLAnatomicalBarrierEngine
    from hierarchical_chunk_encoder import ASLHierarchicalChunkStreamingEncoder
    from speed_variance_curriculum import ASLSpeedVarianceCurriculum
except ImportError:
    try:
        from train_tpu.v2.modules.spatiotemporal_rope import SpatiotemporalRoPE
        from train_tpu.v2.modules.vac_smkd_engine import ASLVACSMKDEngine
        from train_tpu.v2.modules.sign_clip_engine import ASLSignCLIPEngine
        from train_tpu.v2.modules.anatomical_barrier_engine import ASLAnatomicalBarrierEngine
        from train_tpu.v2.modules.hierarchical_chunk_encoder import ASLHierarchicalChunkStreamingEncoder
        from train_tpu.v2.modules.speed_variance_curriculum import ASLSpeedVarianceCurriculum
    except Exception:
...
class VisualROI256Stem(nn.Module):
    """
    TPU-Accelerated Depthwise Separable Visual Patch Stem for 256x256 Body ROI Crops.
    Transforms [B, T, 3, 256, 256] -> [B, T, d_model] with high efficiency.
    """
    ...
    def __init__(self, d_model: int = 512, in_channels: int = 3):
        ...
        # Stage 1: 256x256 -> 128x128 (Conv stride 2)
        self.stem_conv = nn.Conv2d(in_channels, 64, kernel_size=4, stride=2, padding=1, bias=False)
        self.norm1 = RMSNorm(64)
        
        # Stage 2: 128x128 -> 64x64 (Depthwise Separable stride 2)
        self.dw2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, groups=64, bias=False)
        self.pw2 = nn.Conv2d(64, 128, kernel_size=1, bias=False)
        self.norm2 = RMSNorm(128)
        
        # Stage 3: 64x64 -> 32x32 (Depthwise Separable stride 2)
        self.dw3 = nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1, groups=128, bias=False)
        self.pw3 = nn.Conv2d(128, 256, kernel_size=1, bias=False)
        self.norm3 = RMSNorm(256)
        
        # Stage 4: 32x32 -> 16x16 (Depthwise Separable stride 2)
        self.dw4 = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1, groups=256, bias=False)
        self.pw4 = nn.Conv2d(256, d_model, kernel_size=1, bias=False)
        self.norm4 = RMSNorm(d_model)
        
        # Spatial Attention Pooling: 16x16 -> 1 token per frame
    ...
    def forward(self, roi_visual: torch.Tensor) -> torch.Tensor:
        """
        roi_visual: [B, T, 3, 256, 256] or [B, T, 256, 256, 3] (float or uint8)
        Returns: [B, T, d_model] visual token representation.
        """
        if roi_visual.dim() == 4:
            roi_visual = roi_visual.unsqueeze(1)
            
        if roi_visual.dtype == torch.uint8:
            roi_visual = roi_visual.float() / 255.0 - 0.5
            
        # Ensure channels-first [B, T, 3, H, W]
        if roi_visual.size(-1) == 3 and roi_visual.size(2) != 3:
            roi_visual = roi_visual.permute(0, 1, 4, 2, 3)
            
        bsz, seq_len, c_dim, h_dim, w_dim = roi_visual.shape
        x_flat = roi_visual.reshape(bsz * seq_len, c_dim, h_dim, w_dim)
        
        # Align dtype with conv weights for TPU bfloat16 safety
        target_dtype = self.stem_conv.weight.dtype
        if x_flat.dtype != target_dtype:
            x_flat = x_flat.to(target_dtype)
            
        # Stage 1
        h1 = F.gelu(self.norm1(self.stem_conv(x_flat).permute(0, 2, 3, 1)).permute(0, 3, 1, 2))
        
        # Stage 2
        h2 = self.pw2(self.dw2(h1))
        h2 = F.gelu(self.norm2(h2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))
        
        # Stage 3
        h3 = self.pw3(self.dw3(h2))
        h3 = F.gelu(self.norm3(h3.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))
        
        # Stage 4
        h4 = self.pw4(self.dw4(h3))
        h4 = F.gelu(self.norm4(h4.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))  # [B*T, d_model, 16, 16]
        
        # Spatial Attention Pooling over 16x16 grid
        _, d_c, grid_h, grid_w = h4.shape
        grid_tokens = h4.permute(0, 2, 3, 1).reshape(bsz * seq_len, grid_h * grid_w, d_c)  # [B*T, 256, d_model]
        
        attn_weights = F.softmax(self.spatial_attn(grid_tokens), dim=1)  # [B*T, 256, 1]
        pooled = (grid_tokens * attn_weights).sum(dim=1)  # [B*T, d_model]
        
        visual_tokens = self.out_norm(pooled).reshape(bsz, seq_len, d_c)
        return visual_tokens


class VisualHandCrop128Stem(nn.Module):
    """
    TPU-Accelerated Depthwise Separable Visual Patch Stem for 128x128 Dominant Hand Crops.
    Transforms [B, T, 3, 128, 128] or [B, T, 128, 128, 3] -> [B, T, d_model] with high efficiency.
    """
    ...
    def __init__(self, d_model: int = 512, in_channels: int = 3):
        ...
        # Stage 1: 128x128 -> 64x64 (Conv stride 2)
        self.stem_conv = nn.Conv2d(in_channels, 64, kernel_size=4, stride=2, padding=1, bias=False)
        self.norm1 = RMSNorm(64)

        # Stage 2: 64x64 -> 32x32 (Depthwise Separable stride 2)
        self.dw2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, groups=64, bias=False)
        self.pw2 = nn.Conv2d(64, 128, kernel_size=1, bias=False)
        self.norm2 = RMSNorm(128)

        # Stage 3: 32x32 -> 16x16 (Depthwise Separable stride 2)
        self.dw3 = nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1, groups=128, bias=False)
        self.pw3 = nn.Conv2d(128, d_model, kernel_size=1, bias=False)
    ...
    def forward(self, hand_visual: torch.Tensor) -> torch.Tensor:
        """
        hand_visual: [B, T, 3, 128, 128] or [B, T, 128, 128, 3] (float or uint8)
        Returns: [B, T, d_model] visual token representation.
        """
        if hand_visual.dim() == 4:
            hand_visual = hand_visual.unsqueeze(1)

        if hand_visual.dtype == torch.uint8:
            hand_visual = hand_visual.float() / 255.0 - 0.5

        # Ensure channels-first [B, T, 3, H, W]
        if hand_visual.size(-1) == 3 and hand_visual.size(2) != 3:
            hand_visual = hand_visual.permute(0, 1, 4, 2, 3)

        bsz, seq_len, c_dim, h_dim, w_dim = hand_visual.shape
        x_flat = hand_visual.reshape(bsz * seq_len, c_dim, h_dim, w_dim)
        ...
        # Stage 3
        h3 = self.pw3(self.dw3(h2))
        h3 = F.gelu(self.norm3(h3.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))  # [B*T, d_model, 16, 16]

        # Spatial Attention Pooling over 16x16 grid
        _, d_c, grid_h, grid_w = h3.shape
        grid_tokens = h3.permute(0, 2, 3, 1).reshape(bsz * seq_len, grid_h * grid_w, d_c)

        attn_weights = F.softmax(self.spatial_attn(grid_tokens), dim=1)
        pooled = (grid_tokens * attn_weights).sum(dim=1)
...
class ASLFoundationModel(nn.Module):
    ...
    def __init__(
        ...
        self.is_causal = is_causal
        self.visual_stem = VisualROI256Stem(d_model=d_enc)
        self.hand_stem = VisualHandCrop128Stem(d_model=d_enc)
        self.corr_net = TrajectoryCorrelationModule(d_model=d_enc, is_causal=is_causal)
        self.mstp_pyramid = MultiScaleTemporalPerception(d_model=d_enc, is_causal=is_causal)
        self.part_graph = PartAwareSpatialTemporalGraph(in_channels=channels_per_kp, out_dim=128)
        self.cross_modal_fusion = GatedCrossModalFusion(d_model=d_enc)
        ...
        self.landmark_stem = self.visual_encoder
        ...
        if getattr(self, "enable_aux_decoders", False):
            ...
            self.english_decoder = ASLTransformerDecoder(
                vocab_size=max(4, self.english_vocab_size),  # English vocab
                d_model=d_dec,
                nhead=nhead_dec,
                kv_heads=kv_heads_dec,
                num_layers=num_dec_layers,
        ...
        if use_gpt2:
            try:
                ...
                print(f"[WARN] Failed to initialize SignToGPT2Bridge: {e}")
        ...
        self.english_ctc_head = CTCHead(d_enc, self.english_vocab_size, actual_vocab_size=self.actual_english_vocab_size)
        self.english_inter_ctc_head = CTCHead(d_enc, self.english_vocab_size, actual_vocab_size=self.actual_english_vocab_size)
        self.english_early_ctc_head = CTCHead(d_enc, self.english_vocab_size, actual_vocab_size=self.actual_english_vocab_size)
        self.phonology_head = nn.Linear(d_enc, 19)
        ...
        # ─── SOTA ACCURACY ENGINES ───
        if SpatiotemporalRoPE is not None:
            self.spatiotemporal_rope = SpatiotemporalRoPE(head_dim=max(4, (d_enc // nhead_enc)))
        else:
            self.spatiotemporal_rope = None

        if ASLVACSMKDEngine is not None:
            self.vac_engine = ASLVACSMKDEngine(vocab_size=self.vocab_size, d_vis=d_enc, d_ctx=d_enc)
            self.english_vac_engine = ASLVACSMKDEngine(vocab_size=self.english_vocab_size, d_vis=d_enc, d_ctx=d_enc)
        else:
            self.vac_engine = None
            self.english_vac_engine = None

        if ASLAnatomicalBarrierEngine is not None:
            self.anatomical_barrier = ASLAnatomicalBarrierEngine(d_model=d_enc, in_channels=channels_per_kp, num_keypoints=num_keypoints)
        else:
            self.anatomical_barrier = None

        if ASLHierarchicalChunkStreamingEncoder is not None:
            self.hierarchical_chunk_encoder = ASLHierarchicalChunkStreamingEncoder(
                in_channels=channels_per_kp, num_keypoints=num_keypoints, d_model=d_enc
            )
        else:
            self.hierarchical_chunk_encoder = None
    ...
    def _encode(
        ...
        cache: Optional[Dict[str, torch.Tensor]] = None,
        roi_visual: Optional[torch.Tensor] = None,
        hand_visual: Optional[torch.Tensor] = None,
    ):
        """Internal helper method _encode."""
        batch_sz, seq_len = input_x.size(0), input_x.size(1)
        if phonology_features is None:
            phonology_features = torch.zeros((batch_sz, seq_len, 19), device=input_x.device, dtype=input_x.dtype)
        ...
        if hasattr(self, "part_graph"):
            part_tokens = self.part_graph(x_in)
            if part_tokens.dtype != v_tokens.dtype:
                part_tokens = part_tokens.to(v_tokens.dtype)
            v_tokens = v_tokens + part_tokens
        ...
        # Dual-Stream Visual ROI & Hand Crop Gated Fusion
        v_tokens_extra = None
        if roi_visual is not None and hasattr(self, "visual_stem"):
            v_tokens_extra = self.visual_stem(roi_visual)

        if hand_visual is not None and hasattr(self, "hand_stem"):
            v_hand = self.hand_stem(hand_visual)
            if v_tokens_extra is not None:
                v_tokens_extra = 0.5 * (v_tokens_extra + v_hand)
            else:
                v_tokens_extra = v_hand

        if v_tokens_extra is not None and hasattr(self, "cross_modal_fusion"):
            hidden_h = self.cross_modal_fusion(hidden_h, v_tokens_extra)
        if not self.is_causal:
        ...
        # SOTA Trajectory Correlation & Multi-Scale Temporal Perception Enhancement
        if hasattr(self, "corr_net"):
            hidden_h = self.corr_net(hidden_h, mask=cur_mask)
        if hasattr(self, "mstp_pyramid"):
            hidden_h = self.mstp_pyramid(hidden_h, mask=cur_mask)
    ...
    def forward(
        ...
        if True:
            ...
            x = input_x
            if x.ndim == 3:
                num_kp = getattr(self, "num_keypoints", 75)
                c_kp = x.shape[-1] // num_kp if (x.shape[-1] % num_kp == 0) else x.shape[-1]
                if x.shape[-1] % num_kp == 0:
                    x = x.view(x.shape[0], x.shape[1], num_kp, c_kp)
                else:
                    x = x.unsqueeze(-1)
        ...
        if True:
            ...
            if pos.shape[2] >= 60:
                ...
                lh_tips = [4, 8, 12, 16, 20]
                rh_tips = [25, 29, 33, 37, 41]
                face_centroid = pos[:, :, 48:60, :3].mean(dim=2)
            else:
                lh_w, lh_idx, lh_pky = 0, min(5, pos.shape[2]-1), min(17, pos.shape[2]-1)
                rh_w, rh_idx, rh_pky = min(21, pos.shape[2]-1), min(26, pos.shape[2]-1), min(38, pos.shape[2]-1)
                lh_tips = [min(i, pos.shape[2]-1) for i in [4, 8, 12, 16, 20]]
        ...
        if can_fuse_decoders:
            ...
            if include_english and orig_e_len < max_tgt_len:
            ...
            for layer in self.decoder.layers:
                if getattr(self, "gradient_checkpointing", False) and self.training:
                    ...
                    h_fused = torch.utils.checkpoint.checkpoint(
                        _ckpt_dec_layer, layer, h_fused, h_seq, enc_kpm, use_reentrant=False
                    )
                else:
        ...
        # Compute output heads unconditionally
        ctc_log_probs = self.ctc_head(h_seq)
        inter_ctc_log_probs = (
            self.inter_ctc_head(inter_h) if inter_h is not None else None
        )
        early_ctc_log_probs = (
            self.early_ctc_head(early_h) if (hasattr(self, "early_ctc_head") and early_h is not None) else None
        )
        ...
        if compute_mlm and mlm_mask is not None:
        ...
        barrier_loss = torch.zeros((), device=h_seq.device)
        if self.training and getattr(self, "anatomical_barrier", None) is not None and orig_input_x.dim() == 4:
            try:
                b_out = self.anatomical_barrier(orig_input_x)
                barrier_loss = b_out.barrier_loss
            except Exception:
                pass

        english_ctc_log_probs = None
        english_inter_ctc_log_probs = None
        english_early_ctc_log_probs = None
        if hasattr(self, "english_ctc_head") and self.english_ctc_head is not None:
            english_ctc_logits = self.english_ctc_head(h_seq)
            english_ctc_log_probs = F.log_softmax(english_ctc_logits, dim=-1)

            if hasattr(self, "english_inter_ctc_head") and inter_h is not None:
                english_inter_ctc_logits = self.english_inter_ctc_head(inter_h)
                english_inter_ctc_log_probs = F.log_softmax(english_inter_ctc_logits, dim=-1)

            if hasattr(self, "english_early_ctc_head") and early_h is not None:
                english_early_ctc_logits = self.english_early_ctc_head(early_h)
                english_early_ctc_log_probs = F.log_softmax(english_early_ctc_logits, dim=-1)

        vis_ctc_logits = None
        if hasattr(self, "early_ctc_head") and early_h is not None:
            try:
                vis_ctc_logits = self.early_ctc_head(early_h)
            except Exception:
                vis_ctc_logits = None

        vac_distill_loss = torch.zeros((), device=h_seq.device)
        vac_smooth_loss = torch.zeros((), device=h_seq.device)
        if self.training and getattr(self, "vac_engine", None) is not None and early_h is not None:
            try:
                # If sample has valid English targets and no valid gloss (How2Sign continuous mode), use English VAC engine
                if has_valid_english is not None and has_valid_english.any() and getattr(self, "english_vac_engine", None) is not None:
                    vis_l = self.english_vac_engine.vis_head(early_h)
                    ctx_l = self.english_vac_engine.ctx_head(h_seq)
                    cur_vac = self.english_vac_engine
                else:
                    vis_l = self.vac_engine.vis_head(early_h) if hasattr(self.vac_engine, "vis_head") else vis_ctc_logits
                    ctx_l = self.vac_engine.ctx_head(h_seq) if hasattr(self.vac_engine, "ctx_head") else self.ctc_head(h_seq)
                    cur_vac = self.vac_engine

                if vis_l is not None and ctx_l is not None:
                    if vis_ctc_logits is None:
                        vis_ctc_logits = vis_l
                    if vis_l.size(1) != ctx_l.size(1):
                        vis_l_for_distill = F.adaptive_avg_pool1d(vis_l.transpose(1, 2), ctx_l.size(1)).transpose(1, 2)
                    else:
                        vis_l_for_distill = vis_l
                    in_lens = enc_mask.sum(dim=1).to(torch.int32) if enc_mask is not None else None
                    d_v2c, d_c2v = cur_vac.compute_mutual_distillation(vis_l_for_distill, ctx_l, input_lengths=in_lens)
                    vac_distill_loss = 0.5 * (d_v2c + d_c2v)
                    vac_smooth_loss = 0.5 * (
                        cur_vac.compute_temporal_smoothing(ctx_l, in_lens) +
                        cur_vac.compute_temporal_smoothing(vis_l_for_distill, in_lens)
                    )
            except Exception:
                vac_distill_loss = torch.zeros((), device=h_seq.device)
                vac_smooth_loss = torch.zeros((), device=h_seq.device)

        # Phonological auxiliary reconstruction loss
        phonology_loss = torch.zeros((), device=h_seq.device)
        if hasattr(self, "phonology_head") and phonology_features is not None and enc_mask is not None:
            try:
                pred_phon = self.phonology_head(h_seq)
                if phonology_features.size(1) != pred_phon.size(1):
                    p_pooled = F.adaptive_avg_pool1d(phonology_features.transpose(1, 2), pred_phon.size(1)).transpose(1, 2)
                else:
                    p_pooled = phonology_features
                diff_sq = (pred_phon.float() - p_pooled.float()) ** 2
                phonology_loss = (diff_sq.mean(dim=-1) * enc_mask.float()).sum() / enc_mask.float().sum().clamp(min=1.0)
            except Exception:
                phonology_loss = torch.zeros((), device=h_seq.device)
        ...
        return {
            ...
            "english_ctc_log_probs": english_ctc_log_probs,
            "english_inter_ctc_log_probs": english_inter_ctc_log_probs,
            "english_early_ctc_log_probs": english_early_ctc_log_probs,
            ...
            "vac_distill_loss": vac_distill_loss,
            "vac_smooth_loss": vac_smooth_loss,
            "phonology_loss": phonology_loss,
            "gpt2_loss": gpt2_loss,
        }
    ...
    def generate(
        ...
        if h_seq is not None:
            ...
            if features_3d.shape[2] >= 60:
                ...
                lh_tips = [4, 8, 12, 16, 20]
                rh_tips = [25, 29, 33, 37, 41]
                face_centroid = features_3d[:, :, 48:60, :3].mean(dim=2)
            else:

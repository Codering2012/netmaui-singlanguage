...
def xla_clip_grad_norm_(parameters, max_norm, norm_type=2.0):
    """XLA-safe distributed gradient clipping using pure tensor operations (0 device-to-host syncs)."""
    ...
    for p in params:
        total_sq = total_sq + p.grad.float().pow(2).sum()
    
    if _XLA_AVAILABLE:
        try:
            import torch_xla.core.xla_model as xm
            total_sq = xm.mesh_reduce("grad_norm_sq", total_sq, sum)
        except Exception:
            pass
        
    total_norm = total_sq.sqrt()
    clip_coef = max_norm / (total_norm + 1e-6)
    scale = clip_coef.clamp(max=1.0)
    for p in params:

...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # When BOTH multi-task decoders (Gloss, Chicago, English) AND GPT-2 bridge are active with NO bypasses,
            # running per-core micro-batch of 32 (with auto accum_steps=4 for global batch 1024)
            # drops activation memory from 21.9GB down to ~5.5GB HBM (giving 10GB safety headroom)
            # while fully saturating TPU MXU systolic arrays!
            max_safe_batch = 32
        else:
            max_safe_batch = 64
    else:

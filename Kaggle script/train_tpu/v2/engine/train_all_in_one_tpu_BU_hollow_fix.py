...
# ==============================================================================
#  SOTA MODULE 7: SOFT-DTW SPEED & DURATION INVARIANT ALIGNMENT LOSS
# ==============================================================================
...
class MouthingFacialExpressionHead(nn.Module):
    """
    Extracts 14 facial keypoints (lips, eyebrows, nose, jaw) and models
    ...
    def __init__(self, in_kp_dim: int = 14 * 9, out_dim: int = 128):
        super().__init__()
    ...
    def forward(self, features_60: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        face_pts = features_60[:, :, :14, :].contiguous().view(B, T, -1)
...
class ASLFoundationModel(nn.Module):
    ...
    def _encode(
        ...
        # Dual-Stream Visual ROI Gated Fusion
        if roi_visual is not None and hasattr(self, "visual_stem") and hasattr(self, "cross_modal_fusion"):
            v_roi_tokens = self.visual_stem(roi_visual)
            hidden_h = self.cross_modal_fusion(hidden_h, v_roi_tokens)
        if not self.is_causal:
        ...
        if self.training and getattr(self, "vac_engine", None) is not None and early_h is not None:
            try:
                ...
                if vis_l is not None and ctx_l is not None:
                    ...
                    d_v2c, d_c2v = self.vac_engine.compute_mutual_distillation(vis_l_for_distill, ctx_l, input_lengths=in_lens)
                    vac_distill_loss = 0.5 * (d_v2c + d_c2v)
                    vac_smooth_loss = 0.5 * (
                        self.vac_engine.compute_temporal_smoothing(ctx_l, in_lens) +
                        self.vac_engine.compute_temporal_smoothing(vis_l_for_distill, in_lens)
                    )
            except Exception:
                vac_distill_loss = torch.zeros((), device=h_seq.device)
                vac_smooth_loss = torch.zeros((), device=h_seq.device)
...
def train_epoch_tpu(
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        def forward_and_losses(
            features=features, roi_visual=roi_visual, mask=mask, labels=labels, frame_indices=frame_indices,

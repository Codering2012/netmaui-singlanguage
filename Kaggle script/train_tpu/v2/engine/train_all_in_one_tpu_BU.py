...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # Memory Guard: With GPT-2 (124M) + 3 decoders active, per-core batch 128 uses ~9.85 GB HBM
            # with our optimized pipeline. Per-core batch 256 uses >21 GB and causes fatal HBM OOM.
            max_safe_batch = 128
        else:
            max_safe_batch = 128
    else:
    ...
    if user_core_batch > 0:
        ...
        if IS_TPU and is_gpt2_active and user_core_batch > 128:
            if is_master:
                print(
                    f"[!] WARNING: --per-core-batch={user_core_batch} exceeds TPU v5e 16GB HBM capacity with GPT-2 (~21GB). "
                    f"Capping to 128 to prevent fatal HBM OOM. Running with global batch 1024 (~9.85GB HBM, ~208 steps/epoch).",
                    flush=True,
                )
        ...
        # On TPU: default to 128 per core (1024 total batch across 8 cores)
        if IS_TPU:
            target_per_core_batch = 128

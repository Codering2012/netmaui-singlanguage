...
if "LIBTPU_INIT_ARGS" not in os.environ:
    os.environ["LIBTPU_INIT_ARGS"] = "--xla_tpu_enable_flash_attention=true --xla_tpu_enable_data_parallel_all_reduce_opt=true --xla_tpu_enable_async_collective_fusion_multiple_bars=true"

_existing_xla = os.environ.get("XLA_FLAGS", "")
_fast_flags = [
    "--xla_cpu_multi_thread_eigen=true",
    "--xla_tpu_fast_math=true",
]
...
class HomoscedasticLossWrapper(nn.Module):
    ...
    def forward(self, losses: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        s_vec = torch.stack([self.log_vars[k] for k in self.keys])

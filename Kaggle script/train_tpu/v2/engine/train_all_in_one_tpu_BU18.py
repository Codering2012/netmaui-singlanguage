...
class FusedLinearCrossEntropyFunction(torch.autograd.Function):
    ...
    def forward(ctx, h_flat, weight, bias, targets, num_chunks=4):
        ...
        for i in range(num_chunks):
            ...
            chunk_loss = F.cross_entropy(z_chunk, t_chunk, ignore_index=-100, reduction="sum")
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(num_chunks):
            ...
            # Factored embedding lookup: avoids creating dense [chunk, V] one-hot tensor
            # (P - OneHot(Y)) @ W = P @ W - W[Y], saving an extra ~1.5GB HBM during backward
            p_masked = torch.where(valid_t.unsqueeze(-1), p_chunk, torch.zeros_like(p_chunk))
            grad_p = (p_masked @ weight) * scale
            grad_y = F.embedding(t_safe, weight) * scale
            grad_y_masked = torch.where(valid_t.unsqueeze(-1), grad_y, torch.zeros_like(grad_y))

            grad_chunks.append(grad_p - grad_y_masked)

        grad_h = torch.cat(grad_chunks, dim=0) if len(grad_chunks) > 1 else grad_chunks[0]
        return grad_h, None, None, None, None

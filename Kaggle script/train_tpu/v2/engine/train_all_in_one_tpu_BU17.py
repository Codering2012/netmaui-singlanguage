...
class FusedLinearCrossEntropyFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(num_chunks):
            ...
            one_hot = F.one_hot(t_safe, num_classes=V).to(p_chunk.dtype)
            one_hot = torch.where(valid_t.unsqueeze(-1), one_hot, torch.zeros_like(one_hot))

            d_z = (p_chunk - one_hot) * scale
            d_z = torch.where(valid_t.unsqueeze(-1), d_z, torch.zeros_like(d_z))

            grad_chunks.append(d_z @ weight)

...
def _tpu_worker_fn(rank, args):
    ...
    if user_core_batch > 0:
        ...
        max_safe_batch = max(max_safe_batch, user_core_batch)
        requested_batch_size = user_core_batch * world_size
        args.batch_size = requested_batch_size
    else:
        # If user didn't explicitly pass --per-core-batch:
        # On TPU: default to 128 per core (1024 total batch across 8 cores) if requested_batch_size is at small default (<= 256)
        if IS_TPU and requested_batch_size <= 256:
            target_per_core_batch = 128
            requested_batch_size = 128 * world_size
            args.batch_size = requested_batch_size
        else:
...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="Global physical batch size across all TPU cores / GPUs (Default: 1024 for 128 per core on 8-core TPU)",
    )
    parser.add_argument(
        "--per-core-batch",
        type=int,
        default=128,
        help="Explicit per-core micro-batch override (Default: 128 for TPU v5e systolic 128x128 MXU tile alignment and ~208 steps/epoch; use 256 for ~104 steps/epoch)",
    )

...
class ASLShardedDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        MAX_GLOSS_LEN = min(self.max_len, getattr(self, "gloss_max_len", 128))
        MAX_CHICAGO_LEN = min(self.max_len, getattr(self, "chicago_max_len", 128))
        MAX_ENGLISH_LEN = min(self.max_len, getattr(self, "english_max_len", 128))
...
def fast_vectorized_collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    return {
        ...
        "has_valid_english": has_valid_english,
        "is_isolated": is_isolated,
        "mlm_mask": torch.zeros_like(masks),
        "english_trunc": english_trunc,
    }

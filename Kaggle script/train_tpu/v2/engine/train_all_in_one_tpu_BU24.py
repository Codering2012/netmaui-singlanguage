            ...
            for layer in self.decoder.layers:
                if getattr(self, "gradient_checkpointing", False) and self.training:
                    ...
                    h_fused = torch.utils.checkpoint.checkpoint(
                        _ckpt_dec_layer, layer, h_fused, h_seq, enc_mask, use_reentrant=True
                    )
                else:
                    h_fused = layer(h_fused, h_seq, memory_key_padding_mask=enc_mask)[0]
        ...
        if is_master:
            ...
            if IS_TPU and auto_accum == 1:
                print(
                    ...
                    f"Effective step batch: {safe_per_core_batch * world_size} sequences.",
                    flush=True,
                )
            else:
                print(
                    f"[INFO] {hw_name} Memory Guard Active: Capping per-device loader batch to {safe_per_core_batch} "
                    f"(Requested total: {requested_batch_size}, World size: {world_size}, MaxLen: {args.max_len}, d_model: {getattr(args, 'd_model', 384)}, BPE: {bpe_val}). "
                    f"Auto-setting accum_steps={args.accum_steps} for 100% OOM safety & identical gradient updates (Effective Total Batch: {safe_per_core_batch * world_size * args.accum_steps}).",
                    flush=True,
                )
    else:

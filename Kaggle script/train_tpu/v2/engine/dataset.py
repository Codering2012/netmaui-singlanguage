...
def fast_vectorized_collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    # Collate phonology if present
    if any(b.get("phonology") is not None for b in batch):
        phon_list = []
        for b in batch:
            p = b.get("phonology")
            if p is not None:
                p_t = p if isinstance(p, torch.Tensor) else torch.from_numpy(p)
                if p_t.size(0) < max_len:
                    p_t = F.pad(p_t, (0, 0, 0, max_len - p_t.size(0)))
                elif p_t.size(0) > max_len:
                    p_t = p_t[:max_len]
                phon_list.append(p_t.to(prec_dtype))
            else:
                phon_list.append(torch.zeros((max_len, 19), dtype=prec_dtype))
        ret["phonology"] = torch.stack(phon_list, dim=0)

    # Collate roi_visual if present
    if any(b.get("roi_visual") is not None for b in batch):
        roi_list = []
        for b in batch:
            r = b.get("roi_visual")
            if r is not None:
                r_t = r if isinstance(r, torch.Tensor) else torch.from_numpy(r)
                if r_t.size(0) < max_len:
                    pad_shape = [max_len - r_t.size(0)] + list(r_t.shape[1:])
                    r_t = torch.cat([r_t, torch.zeros(pad_shape, dtype=r_t.dtype)], dim=0)
                elif r_t.size(0) > max_len:
                    r_t = r_t[:max_len]
                roi_list.append(r_t)
            else:
                roi_list.append(torch.zeros((max_len, 3, 256, 256), dtype=torch.uint8))
        ret["roi_visual"] = torch.stack(roi_list, dim=0)

    # Collate hand_visual if present
    if any(b.get("hand_visual") is not None for b in batch):
        hand_list = []
        for b in batch:
            h = b.get("hand_visual")
            if h is not None:
                h_t = h if isinstance(h, torch.Tensor) else torch.from_numpy(h)
                if h_t.size(0) < max_len:
                    pad_shape = [max_len - h_t.size(0)] + list(h_t.shape[1:])
                    h_t = torch.cat([h_t, torch.zeros(pad_shape, dtype=h_t.dtype)], dim=0)
                elif h_t.size(0) > max_len:
                    h_t = h_t[:max_len]
                hand_list.append(h_t)
            else:
                hand_list.append(torch.zeros((max_len, 3, 128, 128), dtype=torch.uint8))
        ret["hand_visual"] = torch.stack(hand_list, dim=0)

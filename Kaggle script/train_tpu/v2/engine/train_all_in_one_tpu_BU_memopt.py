...
def _tpu_worker_fn(rank, args):
    ...
    if user_core_batch > 0:
        ...
        # On TPU: default to 256 per core (2048 total batch across 8 cores) if requested_batch_size is at small default (<= 256)
        if IS_TPU and requested_batch_size <= 256:
            target_per_core_batch = 256
            requested_batch_size = 256 * world_size
            args.batch_size = requested_batch_size
        else:
...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="Global physical batch size across all TPU cores / GPUs (Default: 2048 for 256 per core on 8-core TPU to achieve >=2,000 seq/s throughput and ~104 steps/epoch)",
    )
    parser.add_argument(
        "--per-core-batch",
        type=int,
        default=256,
        help="Explicit per-core micro-batch override (Default: 256 for TPU v5e systolic 128x128 MXU tile alignment and >=2,000 seq/s throughput with ~104 steps/epoch)",
    )

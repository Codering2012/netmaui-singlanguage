...
class ASLFoundationModel(nn.Module):
    ...
    def forward(
        ...
        vis_ctc_logits = None
        if hasattr(self, "visual_encoder") and hasattr(self, "ctc_head") and orig_input_x.dim() == 4:
            try:
                v_tok = self.visual_encoder(orig_input_x)
                vis_ctc_logits = self.ctc_head(v_tok)
            except Exception:
                pass
...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # Per-core batch 32 uses ~7.1 GB HBM (45% utilization with 8.6 GB safety headroom).
            # Global physical batch: 32 x 8 cores = 256.
            max_safe_batch = 32
        else:
            # Without GPT-2, 3 decoders + conformer at per-core batch 64 uses ~8.5 GB HBM.
            max_safe_batch = 64
    else:
        # GPU: 16-24GB VRAM
    ...
    if user_core_batch > 0:
        ...
        if IS_TPU and is_gpt2_active and user_core_batch > 32:
            if is_master:
                print(
                    f"[!] WARNING: --per-core-batch={user_core_batch} exceeds TPU v5e 16GB HBM capacity with GPT-2 (~22GB at 128). "
                    f"Capping to 32 to prevent fatal HBM OOM. Running with global batch {32 * world_size} (~7.1GB HBM).",
                    flush=True,
                )
            target_per_core_batch = 32
        elif IS_TPU and not is_gpt2_active and user_core_batch > 64:
            if is_master:
                print(
                    f"[!] WARNING: --per-core-batch={user_core_batch} exceeds TPU v5e 16GB HBM capacity without GPT-2. "
                    f"Capping to 64 to prevent fatal HBM OOM. Running with global batch {64 * world_size} (~8.5GB HBM).",
                    flush=True,
                )
        ...
        # On TPU: safe calibrated per-core batch (32 with GPT-2 = 256 total; 64 without GPT-2 = 512 total; 128 in Phase 1 = 1024 total)
        if IS_TPU:
            if is_phase1:

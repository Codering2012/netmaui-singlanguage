...
class SignToGPT2Bridge(nn.Module):
    ...
    def forward(
        ...
        # Static 256-token sequence length (16 prefix + 240 text tokens).
        # Perfectly aligns with TPU v5e / v3 128x128 systolic array tiles (2 x 128).
        # Guarantees 100% static tensor shapes across all batches, steps, and epochs (0 XLA recompilations).
        target_total_len = 256
...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # GPT-2 has 50,257 vocab (2.88GB logits at b128) + 12 layers of 192MB attention.
            # Per-core batch 64 (512 physical across 8 TPU cores with accum_steps=2 for effective 1024 batch)
            # reduces peak HBM from 18.09GB to ~10.2GB, fitting 100% safely inside 15.75GB with zero OOM.
            max_safe_batch = 64
        else:
            max_safe_batch = 128
    else:

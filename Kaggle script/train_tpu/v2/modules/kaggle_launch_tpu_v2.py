...
def setup_tpu_environment():
    ...
    if "LIBTPU_INIT_ARGS" not in os.environ:
        os.environ["LIBTPU_INIT_ARGS"] = (
            ...
            "--xla_jf_auto_cross_replica_sharding=true "
            "--xla_tpu_overlap_transfers=true"
        )
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=1024, help="Global batch size across 8 TPU cores (128 per core, accum_steps=1, ~9.85GB HBM)")
    parser.add_argument("--per-core-batch", type=int, default=128, help="Per-core micro-batch size (128 for TPU v5e systolic tile alignment, ~208 steps/epoch)")
    ...
    cmd = [
        ...
        "--per-core-batch", str(args.per_core_batch),
        "--accum-steps", "1",

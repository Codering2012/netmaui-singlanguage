    ...
    def generate_views(
        ...
        for scale in self.scales:
            ...
            if self.enable_mirroring:
                ...
                # Swap Left Hand (18..38) and Right Hand (39..59) keypoints
                if K >= 60:
                    # Swap Shoulders (14 <-> 15) and Elbows (16 <-> 17)
                    l_sh = x_mirrored[:, :, 14, :].clone()
                    r_sh = x_mirrored[:, :, 15, :].clone()
                    x_mirrored[:, :, 14, :] = r_sh
                    x_mirrored[:, :, 15, :] = l_sh

                    l_el = x_mirrored[:, :, 16, :].clone()
                    r_el = x_mirrored[:, :, 17, :].clone()
                    x_mirrored[:, :, 16, :] = r_el
                    x_mirrored[:, :, 17, :] = l_el

                    # Swap Left Hand and Right Hand
                    lh = x_mirrored[:, :, 18:39, :].clone()
                    rh = x_mirrored[:, :, 39:60, :].clone()
                    x_mirrored[:, :, 18:39, :] = rh
                    x_mirrored[:, :, 39:60, :] = lh

...
# Tối ưu hóa bộ nhớ và hiệu năng phần cứng TPU v5e
...
def _kill_all_child_subprocesses():
    """Bulletproof process reaper: Terminates all child/grandchild worker processes on any exit/signal/error."""
    try:
        import psutil
...
# --- OpenXLA Hardware Fusion Flags cho TPU v5e ---
...
class SwiGLUFFN(nn.Module):
    ...
    def forward(self, input_x: torch.Tensor) -> torch.Tensor:
        ...
        gate_up = self.w_gate_up(input_x)
    ...
    def forward(self, q, k, frame_indices=None):
        ...
        def apply_rotary_emb(x, cos, sin):
            ...
            if frame_indices is not None:
                # Continuous RoPE frequency computation (Claims 5 & 6 Fix)
                # Preserves fractional pooled timestamps and supports positions > max_len
                inv_f = self.inv_freq.to(frame_indices.device)
                freqs = torch.einsum("bi,d->bid", frame_indices.float(), inv_f)
                emb = torch.cat([freqs, freqs], dim=-1)
                cos_dtype = emb.cos().unsqueeze(1).to(x.dtype)
                sin_dtype = emb.sin().unsqueeze(1).to(x.dtype)
            else:
...
class Swin1DAttention(nn.Module):
    ...
    def forward(self, input_x, key_padding_mask=None, frame_indices=None):
        ...
        if self.shift_size > 0:
            ...
            W = self.window_size
            img_mask = torch.zeros((1, L_feat, 1), device=input_x.device)
            for w_idx in range(num_windows):
                img_mask[:, w_idx * W : (w_idx + 1) * W, :] = float(w_idx)
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(
        ...
        if past_len > 0:
            ...
            max_len = key_k_lower.size(2)
            if past_len == 0 and seq_len == max_len:
                out = F.scaled_dot_product_attention(
                    query_q_lower, key_k_lower, val_v, is_causal=True, scale=self.scale,
                )
...
class ASLDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        iv = 0.1
        self.use_checkpoint = (num_layers > 6) if use_checkpoint is None else use_checkpoint
        self.norm1 = RMSNorm(d_model)
    ...
    def forward(
        ...
        if use_cache:
            ...
            def _inner_decoder_forward(t_in, m_in, t_kpm, m_kpm):
                t_in = t_in + self.drop1(
                    self.gamma1
                    * self.self_attn(self.norm1(t_in), padding_mask=t_kpm)
                )
                if m_in is not None:
                    t_in = t_in + self.drop2(
                        self.gamma2
                        * self.cross_attn(self.norm2(t_in), m_in, m_kpm)
                    )
                t_in = t_in + self.drop3(self.gamma3 * self.ffn(self.norm3(t_in)))
                return t_in

            if self.training and tgt.requires_grad and getattr(self, "use_checkpoint", False):
                tgt_out = checkpoint(
                    _inner_decoder_forward,
                    tgt,
                    memory,
...
class HomoscedasticLossWrapper(nn.Module):
    ...
    def forward(self, losses: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        active_mask = (loss_vec > 0.0)
        task_loss = torch.where(
            active_mask,
            (0.5 * prec_vec * loss_vec + 0.5 * s_clamped),
...
def xla_clip_grad_norm_(parameters, max_norm, norm_type=2.0):
    # Distributed gradient clipping without device-to-host sync on TPU (Claim 4 Fix)
    if IS_TPU:
        ...
        if norm_type == 2.0:
            local_norm_sq = sum(torch.sum(p.grad.detach() ** 2) for p in parameters)
            try:
                import torch_xla.core.xla_model as xm
                world_sz = float(get_xla_world_size())
                if world_sz > 1.0:
                    # All-reduce sum across TPU cores to get true global gradient norm
                    global_norm_sq = xm.all_reduce("sum", local_norm_sq)
                    total_norm = torch.sqrt(global_norm_sq + 1e-12) / world_sz
                else:
                    total_norm = torch.sqrt(local_norm_sq + 1e-12)
            except Exception:
                total_norm = torch.sqrt(local_norm_sq + 1e-12)
        else:
...
def _compute_poly1_loss(
    ...
    if class_weights is not None:
        vf = vf * class_weights[tf].to(focal_weight.dtype)
...
def compute_seq_and_eos_loss(
    ...
    """Computes both sequence and EOS focal loss efficiently in one pass with memory-saving chunking."""
    ...
    total_tokens = lf.shape[0]
    chunk_size = 16384 if IS_TPU else 4096
    ...
    # Chunked computation to prevent large tensor buffer allocation (saves 94% peak loss memory)
    vf_seq_all = valid_mask_seq.reshape(-1).to(lf.dtype) if valid_mask_seq is not None else torch.ones_like(tf, dtype=lf.dtype)
    vf_eos_all = valid_mask_eos.reshape(-1).to(lf.dtype) if valid_mask_eos is not None else torch.ones_like(tf, dtype=lf.dtype)
    ...
    for i in range(0, total_tokens, chunk_size):
        c_lf = lf[i : i + chunk_size]
        c_tf = tf[i : i + chunk_size]
        c_vf_seq = vf_seq_all[i : i + chunk_size] * (c_tf != eos_id).to(lf.dtype)
        c_vf_eos = vf_eos_all[i : i + chunk_size] * (c_tf == eos_id).to(lf.dtype)

        if punct_ids is not None and len(punct_ids) > 0:
            is_punct = torch.zeros_like(c_tf, dtype=torch.bool)
            for pid in punct_ids:
                is_punct = is_punct | (c_tf == pid)
            c_vf_seq = c_vf_seq * torch.where(is_punct, 2.0, 1.0).to(lf.dtype)

        if class_weights is not None:
            c_vf_seq = c_vf_seq * class_weights[c_tf].to(lf.dtype)

        if sw is not None:
            c_sw = sw[i : i + chunk_size].to(lf.dtype)
            c_vf_seq = c_vf_seq * c_sw
            c_vf_eos = c_vf_eos * c_sw

        c_log_probs = F.log_softmax(c_lf, dim=-1)
        c_nll = -c_log_probs.gather(dim=-1, index=c_tf.unsqueeze(-1)).squeeze(-1)
        c_nll = c_nll.masked_fill(c_tf == pad_id, 0.0)

        c_ce_unsmoothed = c_nll
        c_mean_log_prob = c_log_probs.mean(dim=-1)
        c_ce_smoothed_seq = (1.0 - label_smoothing) * c_nll - label_smoothing * c_mean_log_prob
        c_ce_smoothed_seq = c_ce_smoothed_seq.masked_fill(c_tf == pad_id, 0.0)

        c_p_target = torch.exp(-c_ce_unsmoothed).clamp(min=1e-6, max=1.0)
        c_focal_weight = torch.pow(1.0 - c_p_target.detach(), gamma)

        poly1_seq = c_focal_weight * (c_ce_smoothed_seq + POLY1_EPS * (1.0 - c_p_target))
        poly1_eos = c_focal_weight * (c_ce_unsmoothed + POLY1_EPS * (1.0 - c_p_target))

        total_num_seq = total_num_seq + (poly1_seq * c_vf_seq).float().sum()
        total_den_seq = total_den_seq + c_vf_seq.float().sum()

        total_num_eos = total_num_eos + (poly1_eos * c_vf_eos).float().sum()
        total_den_eos = total_den_eos + c_vf_eos.float().sum()

    loss_seq = _distributed_normalize(total_num_seq, total_den_seq)
    loss_eos = _distributed_normalize(total_num_eos, total_den_eos)
    return loss_seq, loss_eos
        ...
        for step_idx, batch in enumerate(para_loader, 1):
            ...
            def forward_and_losses(
                ...
                if not skip_gen and isinstance(out, dict) and "h_seq" in out:
                    ...
                    val_gen_len = max(64, min(gt_tokens.size(1), getattr(args, "max_target_len", 128))) if gt_tokens is not None else 64
...
def _tpu_worker_fn(rank, args):
    ...
    # Auto Memory Guard: Cap per-device micro-batch size to guarantee fitting inside accelerator VRAM
    requested_batch_size = args.batch_size
    target_per_core_batch = max(1, requested_batch_size // world_size)
    if IS_TPU:
        # TPU v3/v5e (16GB HBM per core): Tăng gấp đôi giới hạn an toàn lên 128 cho Video landmarks
        max_safe_batch = 128 if args.max_len <= 384 else 64
    else:
        # GPU: Tăng gấp đôi giới hạn an toàn lên 16/32
    ...
    if hasattr(args, "phase1_epochs") and args.phase1_epochs > 0:
        ...
        if IS_TPU:
            ...
            # GPU Memory Safe: Cap per-GPU physical batch to 32 for max_len >= 256 to fit comfortably inside 16GB VRAM
            max_gpu_batch = 32 if getattr(args, "max_len", 256) >= 256 else 64
            phase1_core_batch = min(target_core_batch, max_gpu_batch)
            phase1_accum = max(1, math.ceil(target_core_batch / phase1_core_batch)) * max(1, getattr(args, "accum_steps", 1))

        if is_master:
            dev_name = "TPU cores" if IS_TPU else "GPUs"
            print(
                f"[INFO] Phase 1 Batch Allocation: Core batch {phase1_core_batch} x {world_size} {dev_name} = Physical batch {phase1_core_batch * world_size} (accum_steps={phase1_accum}, Effective batch {requested_batch_size}).",
                flush=True,
            )
    ...
    if phase1_ep > 0:
        if os.path.exists(llm_ckpt_path):
            ...
            if any(k.startswith("english_decoder.") or k.startswith("decoder.") for k in ckpt.keys()):
                ...
                if is_master:
                    print("[INFO] Loaded legacy Phase 1 checkpoint into English Decoder.")
                ...
                if IS_TPU:
                    ...
                    cpu_state = {
                        ...
                        "loss_ema_state_dict": (
                            {
                                k: v.cpu().detach()
                                for k, v in loss_ema.shadow.items()
                            }
                            if loss_ema is not None
                            else None
                        ),
...
def inverted_gloss_pretrain_loop(args, device, is_master):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if IS_TPU:
            xm.mark_step()
            xm.rendezvous(f"phase2_save_barrier_{epoch}")
            if is_master:
                ...
                save_dict = {
                    ...
                    "english_emb": {k: v.detach().cpu() for k, v in english_emb.state_dict().items()},
            ...
            xm.rendezvous(f"phase2_save_done_{epoch}")
        elif is_master:
            os.makedirs(args.save_dir, exist_ok=True)
            save_dict = {
                "epoch": epoch,
...
def text_pretrain_loop(args, device, is_master, per_core_batch=None, accum_steps=1):
    ...
    for epoch in range(start_epoch, args.phase1_epochs):
        ...
        steps_limit = (
            getattr(args, "phase1_max_steps", 0)
            if getattr(args, "phase1_max_steps", 0) > 0
            else (getattr(args, "max_steps", 0) if getattr(args, "max_steps", 0) > 0 else (len(dataset) // cluster_batch if hasattr(dataset, "__len__") else 0))
        )
        ...
        for step, batch in enumerate(loader):
            ...
            target_ids = batch["target_ids"] if batch["target_ids"].device == device else batch["target_ids"].to(device, non_blocking=True)
            is_dae = batch["is_dae"] if ("is_dae" in batch and batch["is_dae"].device == device) else (batch["is_dae"].to(device, non_blocking=True) if "is_dae" in batch else torch.ones(input_ids.shape[0], dtype=torch.bool, device=device))
            ...
            phase1_accum = accum_steps if accum_steps > 1 else max(1, args.accum_steps)
            effective_accum = min(
                phase1_accum,
                dloader_len
                - (step // phase1_accum) * phase1_accum,
            )

            is_accumulating = ((step + 1) % effective_accum != 0) and ((step + 1) != dloader_len)
            sync_context = phase1_net.no_sync() if (is_accumulating and hasattr(phase1_net, "no_sync")) else contextlib.nullcontext()

            amp_enabled = not getattr(args, "disable_amp", False) and (device.type == "cuda" or (IS_TPU and target_dtype != torch.float32))
            with sync_context:
                with torch.autocast(
                    device_type="cuda" if "cuda" in device.type else "xla" if IS_TPU else "cpu",
                    dtype=target_dtype if target_dtype != torch.float32 else torch.bfloat16,
                    enabled=amp_enabled,
                ):
                    tgt_in = target_ids[:, :-1]
                    tgt_out = target_ids[:, 1:]
            ...
            if not is_accumulating:
                if IS_TPU:
                    ...
                    if scaler.is_enabled():
                        scaler.unscale_(optimizer)
                        torch.nn.utils.clip_grad_norm_(phase1_net.parameters(), max_norm=1.0)
                        scaler.step(optimizer)
                        scaler.update()
                    else:
                        torch.nn.utils.clip_grad_norm_(phase1_net.parameters(), max_norm=1.0)
            ...
            if step % args.log_freq == 0:
                with torch.no_grad():
                    ...
                    correct_sum = correct.to(logits.dtype).sum()
                    tot_tokens = valid_mask.to(logits.dtype).sum()
                ...
                if is_master:
                    ...
                    def _async_step_print(log_vec_cpu, step_idx, step_fmt_str, lr_val, elapsed_val, samples_per_sec_val, steps_per_sec_val, ep, tot_ep):
                        loss_val, loss_seq_val, c_sum, t_tokens = log_vec_cpu.cpu().tolist()
                        ...
                        ppl = math.exp(min(loss_seq_val, 20.0))

                        print(
                            f"Phase 1 | Epoch {ep:03d}/{tot_ep:03d} | "
                            f"Step {step_fmt_str} | Loss: {loss_val:.4f} (Seq: {loss_seq_val:.4f}){acc_str} | PPL: {ppl:.1f} | "
            ...
            steps_limit = (
                getattr(args, "phase1_max_steps", 0)
                if getattr(args, "phase1_max_steps", 0) > 0
                else (getattr(args, "max_steps", 0) if getattr(args, "max_steps", 0) > 0 else total_steps)
            )
        ...
        is_save_epoch = ((epoch + 1) % 5 == 0) or ((epoch + 1) == args.phase1_epochs)
        ...
        if IS_TPU:
            ...
            if is_master:
                os.makedirs(args.save_dir, exist_ok=True)
            xm.rendezvous(f"phase1_save_barrier_{epoch+1}")

...
class TemporalStridedPool(nn.Module):
    ...
    def forward(self, hidden_h, mask=None, **kwargs):
        ...
        if pad_len > 0:
            if self.is_causal:
                hidden_padded = torch.nn.functional.pad(hidden_h, (0, 0, pad_len, 0))
            else:
                hidden_padded = torch.nn.functional.pad(hidden_h, (0, 0, 0, pad_len))
        else:
...
class RotaryPositionalEncoding(nn.Module):
    ...
    def forward(self, q, k):
        ...
        def apply_rotary_emb(x, cos, sin):
            # x: [B, seq_len, num_heads, head_dim] (or similar)
            # Actually, x is [batch_sz, nhead, seq_len, rope_dim] when called from GroupedQueryEncoderAttention
...
class GroupedQueryEncoderAttention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            kpm = key_padding_mask.view(batch_sz, 1, 1, seq_len).bool()
        ...
        if self.is_causal and current_lookahead > 0:
...
class Swin1DAttention(nn.Module):
    ...
    def forward(self, input_x, key_padding_mask=None, frame_indices=None):
        ...
        if pad_l > 0:
            ...
            if key_padding_mask is not None:
                key_padding_mask = F.pad(key_padding_mask, (0, pad_l), value=0.0)
...
class SpatialTemporalSE(nn.Module):
    ...
    def forward(
        ...
        return input_x * torch.max(c_se_out, self.s_se(input_x))
...
class ConvNeXtTemporalBlock(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if key_padding_mask is not None:
            input_x = input_x * key_padding_mask.unsqueeze(-1)
        ...
        if key_padding_mask is not None:
            target_y = target_y * key_padding_mask.unsqueeze(-1)
        target_y = self.se(
...
class BiMamba2SSMBlock(nn.Module):
    ...
    def __init__(
        ...
        self.fwd_conv1d = nn.Conv1d(
            ...
            kernel_size=d_conv,
            groups=self.d_inner,
            padding=d_conv - 1 if is_causal else d_conv // 2,
        )
    ...
    def _ssd_multihead_scan(
        ...
        N_sz, device = batch_sz.shape[-1], input_x.device
        if reverse:
    ...
    def forward(
        ...
        **kwargs,
        ...
        x_fwd_h_padded = self.fwd_conv1d(x_proj.transpose(1, 2))
...
class MaskedGroupNorm(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        return x_normed * mask.unsqueeze(1).to(x_normed.dtype)
    ...
    def forward(
        ...
        if seq_len == 1:
            if past_key_value is not None and len(past_key_value) == 3:
                positions = torch.arange(key_k_lower.size(2), device=input_x.device)
                attn_mask = (positions <= past_len).view(1, 1, 1, -1)
                out = F.scaled_dot_product_attention(
                    query_q_lower,
                    key_k_lower,
                    val_v,
            ...
            if padding_mask is not None:
                ...
                causal_mask = torch.tril(
                    torch.ones(
                        seq_len, total_len, dtype=torch.bool, device=input_x.device
                    )
                )
                attn_mask = causal_mask.unsqueeze(0).unsqueeze(1) & (
                    ~padding_mask
...
class HomoscedasticLossWrapper(nn.Module):
    ...
    def forward(self, losses: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        s_vec = torch.clamp(s_vec, min=-20.0, max=20.0)
        prec_vec = torch.exp(-s_vec)
        is_active = (loss_vec != 0).float()

        s_val = s_vec * is_active + s_vec.detach() * (1.0 - is_active)
        prec_val = prec_vec * is_active + prec_vec.detach() * (1.0 - is_active)
...
class CrossModalInfoNCELoss(nn.Module):
    ...
    def forward(
        ...
        if gt_tokens_all is not None:
            ...
            # pos_mask retains self-identity so unique samples keep their primary positive
            pass
        else:
...
class DenseSentenceSemanticLoss(nn.Module):
    ...
    def forward(
        ...
        std_loss = torch.mean(F.relu(target_std - std_p))
    ...
    def forward(
        ...
        if world_size > 1 and IS_TPU and "xla" in str(device).lower():
            if sample_ids is not None:
                ...
                all_sample_ids = fused_all[:, -1].long()
            else:
...
class PositionalEncoding1D(nn.Module):
    ...
    def forward(
        ...
        if getattr(self, "scale_embeddings", True):
...
def xla_clip_grad_norm_(parameters, max_norm, norm_type=2.0):
    ...
    norms = [torch.norm(p.grad.detach(), norm_type) for p in parameters]
...
class ASLFoundationModel(nn.Module):
    ...
    def __init__(
        ...
        self.ctc_head = CTCHead(d_enc, self.vocab_size)
        self.inter_ctc_head = CTCHead(d_enc, self.vocab_size)
        self.lpc_proj = nn.Sequential(nn.Linear(d_enc, d_enc), RMSNorm(d_enc))
        self.mlm_head = LandmarkReconstructionHead(d_enc, 540)
        self.domain_head = nn.Sequential(nn.Dropout(0.1), nn.Linear(d_enc, 4))
        ...
        self.aux_gloss_head = CosineLinear(d_enc, self.vocab_size, init_tau=2.65)
    ...
    def forward(
        ...
        h_cls, h_seq, enc_mask, used_mlm_mask, fi_out, orig_enc_mask, inter_h = (
            self._encode(
                input_x,
                phonology_features,
                mask,
        ...
        if compute_lpc:
            ...
            if h_seq.shape[1] > 1:
                ...
                if enc_mask is not None:
                    ...
                    valid_diff_count = mask_valid.sum().clamp(min=1.0) * diff.shape[-1]
                    diff_mean = (diff**2).sum() / valid_diff_count
                else:
                    diff_mean = (diff**2).mean()
                loss_lpc = diff_mean + F.relu(1.0 - lpc_std)
                if enc_mask is not None:
    ...
    def generate(
        ...
        for step in range(max_new_tokens):
            # causes 64 PCIe roundtrips. Let the loop run to its static boundary.
            step_t = torch.tensor([step], device=device, dtype=torch.long)
            tgt_in = torch.index_select(generated, 1, step_t)
            dec_pad_id = getattr(
            ...
            next_step_t = torch.tensor([step + 1], device=device, dtype=torch.long)
            generated = generated.index_copy(1, next_step_t, next_tokens.unsqueeze(1))
...
class ModelEMA:
    ...
    def update(self, model: nn.Module, progress: float = 0.0):
        ...
        # baking (which forces XLA graph recompilation per step and defeats warmup).
        ...
        with torch.no_grad():
            for name, param in model.named_parameters():
                if param.requires_grad and name in self.shadow:
                    ...
                    self.shadow[name].lerp_(param, (1.0 - decay).to(param.dtype))
    ...
    def restore(self, model: nn.Module):
        """Provides functionality for restore."""

        with torch.no_grad():
            for name, param in model.named_parameters():
                if param.requires_grad and name in self.shadow:
                    # Swap original parameters back
                    temp = param.clone().detach()
                    param.copy_(self.shadow[name])
                    self.shadow[name].copy_(temp)
...
def compute_seq_and_eos_loss(
    ...
    # 3. Compute p_target detached for focal weights to save backward graph memory
    with torch.no_grad():
...
def _compute_ctc_loss_safe(
    ...
    # Sanitize targets so positions beyond target_lengths do not contain blank (0)
    # Use UNK_ID (3) for padding out-of-bounds targets because CTC loss rejects blanks in targets
    clean_targets = torch.where(valid_shifted, clean_targets, torch.tensor(GlossVocabulary.UNK_ID, dtype=clean_targets.dtype, device=clean_targets.device))
    ...
    # FastEmit Regularization: penalize non-blank tokens to encourage earlier emission
    # ctc_log_probs: [batch_sz, seq_len, num_classes]
    fastemit_lambda = 0.001
    prob_blank = torch.exp(ctc_log_probs[:, :, GlossVocabulary.PAD_ID])
    prob_non_blank = 1.0 - prob_blank
    
    # We want to minimize the non-blank probability accumulation delay
    fastemit_penalty = fastemit_lambda * prob_non_blank.sum(dim=1)
...
def _compute_mlm_loss_safe(
    ...
    if target.shape[1] != seq_len_mlm:
        mlm_logits = F.interpolate(
            mlm_logits.transpose(1, 2),
            size=target.shape[1],
            mode="linear",
            align_corners=False,
        ).transpose(1, 2)
        seq_len_mlm = target.shape[1]
    ...
    return _distributed_normalize(loss_raw.float().sum(), weighted_mask.float().sum())
                    ...
                    with open(metrics_csv_path, "a", newline="") as f:
                        ...
                        writer.writerow(
                            [
                                ...
                                round(eng_ppl_val, 2),
                                lr_val,
                                round(instant_it_s, 2),
                                round(samples_per_s, 1),
                            ]
                        )
                except Exception:
                    pass
            ...
            if is_master and (step_idx % 50 == 0 or step_idx == min_val_batches):
                ...
                if is_master:
                    metrics_csv_path = os.path.join(
                        args.model_dir if args and hasattr(args, "model_dir") else ".",
                        "training_metrics.csv",
                    )
                    with open(metrics_csv_path, "a", newline="") as f:
                        csv.writer(f).writerow(
                            [epoch + 1, step_idx, "val_intra", float(raw_loss.detach())]
                            + [0.0] * 12
                        )
...
def _tpu_worker_fn(rank, args):
    ...
    if hasattr(args, "phase1_epochs") and args.phase1_epochs > 0:
        text_pretrain_loop(args, device, is_master)
    ...
    if not data_dir.exists() or not has_pt_files:
        ...
        if (
            ...
            try:
                for pt_file in Path("/kaggle/input").rglob("*.pt"):
    ...
    llm_ckpt_path = os.path.join(
        args.save_dir, f"asl_llm_{getattr(args, 'phase1_epochs', 2) or 2}.pt"
    )
    if True:
        if os.path.exists(llm_ckpt_path):
            ...
            ckpt = torch.load(llm_ckpt_path, map_location="cpu", weights_only=True)
            target_decoder = (
    ...
    if is_master:
        ...
        print("[DEBUG 8/8] Starting main training loop (Epoch 1)...", flush=True)
...
def inverted_gloss_pretrain_loop(args, device, is_master):
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        collate_fn=functools.partial(phase2_collate_fn, eng_pad_id=eng_pad_id),
        drop_last=True,
    )
    ...
    for epoch in range(args.epochs):
        ...
        english_emb.train()
        if IS_TPU:
            loader = pl.MpDeviceLoader(dataloader, device, batches_per_execution=16)
        else:
            loader = dataloader
        ...
        for step, batch in enumerate(loader):
            ...
            if step % args.log_freq == 0:
                if is_master:
                    print(
                        f"Phase 2 Train | Epoch {epoch+1:03d} | Step {step:04d} | Loss: {loss.item():.4f}",
                        flush=True,
                    )
        ...
        if len(dataloader) % max(1, args.accum_steps) != 0:
            xm.optimizer_step(optimizer)
            xm.mark_step()
            optimizer.zero_grad(set_to_none=True)
...
def pseudo_gloss_gen_loop(args, device, is_master):
    ...
    gloss_vocab_path = os.path.join(args.data_dir, "vocab_map.json")
    if os.path.exists(gloss_vocab_path):
        with open(gloss_vocab_path, "r", encoding="utf-8") as f:
            gloss_vocab = GlossVocabulary(label_to_idx=json.load(f))
    else:
    ...
    if os.path.exists(ckpt_path):
        ...
        raise ValueError(
            "FATAL: inverted_gloss_model.pt not found! Aborting pseudo-label generation to prevent generating garbage labels."
        )
        ...
        for shard_path in local_shards:
            ...
            for i in range(0, len(items_list), batch_size):
                ...
                with torch.autocast(
                    ...
                    past_zero = torch.tensor([0], device=device, dtype=torch.long)
                    kv_caches.append(((mtp_k1, mtp_v1, past_zero), None))
                    mtp_k2 = torch.zeros(
                        (bsz, kv_heads, args.max_len, head_dim),
                        dtype=memory.dtype,
                        device=device,
                    ...
                    kv_caches.append(((mtp_k2, mtp_v2, past_zero), None))
...
def text_pretrain_loop(args, device, is_master):
    """
    ...
    if is_master:
        print(f"Starting Phase 1 Text Pre-training for {args.phase1_epochs} epochs...")
        os.makedirs(args.save_dir, exist_ok=True)
    ...
    if os.path.exists(gloss_vocab_path):
        ...
        gloss_vocab = GlossVocabulary(label_to_idx={})
    # max_len=256 risks silent truncation; 384 is safe for TPU HBM.
    bpe_max_len = max(args.max_len, 384)
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        batch_size=args.batch_size,
        num_workers=args.num_dataloader_workers,
        collate_fn=functools.partial(phase1_collate_fn, eng_pad_id=eng_pad_id),
    )
    ...
    model = ASLFoundationModel(
        ...
        num_enc_layers=0,
        d_enc=args.d_model,
        vocab_size=len(gloss_vocab) + GlossVocabulary.OFFSET,
        d_dec=args.d_model,
        nhead_enc=args.nhead,
        nhead_dec=args.nhead,
        num_dec_layers=args.num_layers,
    ...
    # Tie english_decoder embeddings to its language modeling head BEFORE moving to device
    ...
    optimizer = torch.optim.AdamW(
        model.english_decoder.parameters(), lr=args.lr, weight_decay=0.01
    )
    ...
    for epoch in range(args.phase1_epochs):
        ...
        total_steps = (
            len(dataset) // max(1, args.batch_size)
        ...
        for step, batch in enumerate(loader):
            ...
            is_dae = batch.get(
                "is_dae",
                torch.ones(input_ids.shape[0], dtype=torch.bool, device=device),
            )
            ...
            with torch.autocast(
                ...
                # Embed input text to serve as memory for the decoder
                ...
                # Phase 1 only uses seq loss, so we pass a zero mask for EOS loss and ignore it
                loss, _ = compute_seq_and_eos_loss(
                    logits,
                    tgt_out,
                    valid_mask,
            ...
            if step % args.log_freq == 0:
                ...
                if is_master:
                    ...
                    samples_per_sec = (
                        steps_per_sec * args.batch_size * (8 if IS_TPU else 1)
                    )
                    ...
                    def _async_step_print(log_vec_cpu, step_idx, step_fmt_str, lr_val, elapsed_val, samples_per_sec_val, steps_per_sec_val, ep, tot_ep):
                        ...
                        try:
                            ...
                            write_header = not os.path.exists(metrics_csv_path)
                            with open(
                                metrics_csv_path, "a", newline="", encoding="utf-8"
                            ) as f:
                                writer = csv.writer(f)
                                if write_header:
                                    writer.writerow(
                                ...
                                writer.writerow(
                                    [
                                        ...
                                        f"{ppl:.2f}",
                                        lr_val,
                                        round(steps_per_sec_val, 2),
                                        round(samples_per_sec_val, 1),
                                    ]
                                )
                        except Exception:
                            pass
        ...
        if (epoch + 1) % 10 == 0 or (epoch + 1) == args.phase1_epochs:
            ...
            if IS_TPU:
                xm.save(
                    model.english_decoder.state_dict(),
                    ckpt_path,
                    master_only=True,
                )
            elif is_master:
...
def main():
    ...
    if "/kaggle/input/" in args.data_dir:
        import shutil
        import os
        
        shm_dir = "/dev/shm/dataset"
        print(f"[*] Optimizing I/O: Copying Kaggle dataset from {args.data_dir} to {shm_dir}...", flush=True)
        try:
            if not os.path.exists(shm_dir):
                shutil.copytree(args.data_dir, shm_dir, dirs_exist_ok=True)
            args.data_dir = shm_dir
            print(f"[*] I/O Optimization successful. Using data_dir: {args.data_dir}", flush=True)
        except Exception as e:
            print(f"[*] I/O Optimization failed: {e}. Falling back to original data_dir.", flush=True)
    ...
    if IS_TPU:
        if "LOCAL_RANK" in os.environ:
            ...
            xmp.spawn(_tpu_worker_fn, args=(args,), nprocs=None, start_method="spawn")
    else:
...
def generate_training_report(args, device):
    ...
    if hasattr(val_loader.dataset, "label_to_idx") and val_loader.dataset.label_to_idx:
        actual_vocab_size = len(val_loader.dataset.label_to_idx)
    ...
    ckpt_path = os.path.join(
        args.model_dir if hasattr(args, "model_dir") else ".", "asl_model_latest.pt"
    )
    if os.path.exists(ckpt_path):
        model.load_state_dict(torch.load(ckpt_path, map_location=device), strict=False)
    else:
        print(
            f"Warning: Checkpoint not found at {ckpt_path}. Using current model weights."
        )
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            if i >= 50:  # evaluate 50 batches for speed
                break
            ...
            domain = batch.get(
                "domain_label", batch.get("source_id", torch.zeros_like(eng_seq[:, 0]))
            )
                ...
                if non_how2sign_mask.any():
                    ...
                    for b_idx in range(valid_preds.shape[0]):
                        for s_idx in range(valid_preds.shape[1]):
                            if s_idx == 0:
                                continue  # Skip BOS
                            gt_token = valid_gt[b_idx, s_idx].item()
                            ...
                            if pred_token == gt_token:
                                word_counts[gt_token] += 1

    print("\n=======================================================")
    print("   TOP 50 CORRECTLY GUESSED WORDS (NON-HOW2SIGN)   ")
    print("=======================================================\n")

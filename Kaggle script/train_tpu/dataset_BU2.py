...
class GlossVocabulary:
    ...
    def __init__(self, label_to_idx: Dict):
        ...
        min_idx = min(clean_l2i.values()) if clean_l2i else 0
        self.already_offset = min_idx >= self.OFFSET
...
class EnglishVocabulary:
    ...
    def __init__(
        ...
        if vocab_path and Path(vocab_path).exists():
            ...
            if vocab_path:
                vp = Path(vocab_path)
                candidates.extend([vp.parent / "english_vocab.json", vp.parent.parent / "english_vocab.json"])
            
            # Removed global fallback search across all kaggle datasets
        ...
        if resolved_path:
            ...
            raise FileNotFoundError(
                ...
                f"Please ensure english_vocab.json is included in your dataset!"
            )
...
class LandmarkAugmenter:
    ...
    def __call__(
        ...
        # 4. Jitter
        if jitter_std > 0:
            xyz = xyz + np.random.normal(0, jitter_std, size=xyz.shape).astype(
                np.float32
            )

        # Reconstruct pos
        # We only keep the 3 spatial dimensions; velocity and acceleration will be freshly computed.
        pos = xyz
        ...
        if T > 20 and np.random.rand() < timestretch_prob:
            ...
            pos_t = torch.from_numpy(pos).permute(1, 2, 0) # [K, C, T]
            pos_resampled = F_torch.interpolate(pos_t, size=new_T, mode='linear', align_corners=True)
            pos = pos_resampled.permute(2, 0, 1).numpy()
            T = new_T
...
class ASLShardedDataset(Dataset):
    ...
    def __init__(
        ...
        for search_p in [self.dataset_dir, self.dataset_dir.parent, self.dataset_dir.parent.parent]:
            if search_p.exists():
                for pat in ["*vocab*.json", "*mapping*.json"]:
                    vocab_candidates.extend(list(search_p.glob(pat)))
        ...
        self.dataset_id = f"{self.dataset_name}_{self.max_len}_{self.stride_length}_{self.channels_per_kp}_v2_{hash(str(self.dataset_dir))}"
    ...
    def _load_records_metadata(self) -> None:
        ...
        if not temp_metadata:
            ...
            cache_hash = hashlib.sha256(
                f"{self.split}_{self.worker_idx}_{len(self.shard_files)}_{self.max_len}_{mtime_sum}_{vocab_hash}_{label_hash}".encode()
            ).hexdigest()[:12]
...
class ASLStreamedDataset(IterableDataset):
    ...
    def __init__(
        ...
        if english_vocab_file.exists():
            ...
            if not getattr(self, "use_bpe", False):
    ...
    def __iter__(self):
        ...
        if self.split == "train":
            worker_seed = (
                worker_info.seed + getattr(self, "epoch", 0)
                if worker_info
                else 42 + getattr(self, "epoch", 0) + (self.worker_idx * 100)
            )
            random.seed(worker_seed)
        ...
        for shard_path in shards:
            try:
                ...
                if isinstance(shard_data, dict):
                    ...
                    if self.split == "train":
                        random.shuffle(keys)
                    items = [shard_data[k] for k in keys]
                else:
                    items = shard_data
                for rec in items:
                    if isinstance(rec, dict):
                        rec_to_process = rec
                        if self.augment and prev_raw_rec is not None and random.random() < 0.25:
    ...
    def _stitch_raw_records(self, rec1: dict, rec2: dict) -> Optional[dict]:
        ...
        try:
            ...
            BOS_ID, EOS_ID = 1, 2
            g1_clean = [t for t in g1 if t not in (BOS_ID, EOS_ID)] if g1 else []
            g2_clean = [t for t in g2 if t not in (BOS_ID, EOS_ID)] if g2 else []
            stitched_gloss = (g1_clean + g2_clean) if (g1_clean or g2_clean) else None
            ...
            if c1 is not None and c2 is not None:
                ...
                stitched_rec["chicago_seq"] = [t for t in c1_list if t not in (BOS_ID, EOS_ID)] + [t for t in c2_list if t not in (BOS_ID, EOS_ID)]
            ...
            if e1 is not None and e2 is not None:
                ...
                stitched_rec["english_seq"] = [t for t in e1_list if t not in (BOS_ID, EOS_ID)] + [t for t in e2_list if t not in (BOS_ID, EOS_ID)]
        ...
        if T > 0:
            ...
            if T > self.max_len:
                ...
                if feat_arr.shape[0] > 1 and feat_arr.shape[-1] >= 9:
                    ...
                    pos_s = feat_arr[:, :, :3]
                    vel[1:] = (pos_s[1:] - pos_s[:-1]) / actual_dt
                    vel[0] = 0.0
                    acc[1:] = (vel[1:] - vel[:-1]) / actual_dt
                    acc[0] = 0.0
                    feat_arr[:, :, 3:6] = vel
...
class KDWDDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        if page_csv.exists():
            ...
            print(
                "WARNING: KDWD page.csv not found, proceeding without view filtering."
            )
...
class ASLGPC12Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if Path(csv_path).exists():
            ...
            if not gloss_col or not text_col:
                raise ValueError(f"ASLG-PC12 CSV missing required gloss/text columns. Found: {list(df.columns)}")
            self.df = df.dropna(subset=[gloss_col, text_col])
            self.gloss_col = gloss_col
            self.text_col = text_col
        else:
            print(
                f"WARNING: ASLG-PC12 CSV not found at {csv_path}. Using empty dataset."
            )
            self.df = pd.DataFrame(columns=["gloss", "text"])
            self.gloss_col = "gloss"
            self.text_col = "text"
    ...
    def __getitem__(self, idx):
        ...
        gloss_str = str(row[self.gloss_col])
        text_str = str(row[self.text_col])

        gloss_ids = self.gloss_vocab.encode(gloss_str, is_chicago=False)
        text_ids = self.eng_vocab.encode(text_str)

        gloss_ids = (
            [self.gloss_vocab.BOS_ID]
            + gloss_ids[: self.max_len - 2]
            + [self.gloss_vocab.EOS_ID]
        )
        text_ids = (
            [self.eng_vocab.BOS_ID]
            + text_ids[: self.max_len - 2]
...
class Phase1MixedIterable(torch.utils.data.IterableDataset):
    ...
    def __len__(self) -> int:
        """Static epoch boundary for infinite IterableDataset (Bug 31)."""
        return 5000
    ...
    def __iter__(self):
        ...
        if aslg_len == 0:
            ...
            def _aslg_gen():
                n_valid = (aslg_len - 1 - worker_id) // total_workers + 1
                while True:
                    if n_valid > 0:
                        idx = random.randint(0, n_valid - 1) * total_workers + worker_id
                    else:
                        idx = random.randint(0, max(0, aslg_len - 1))
...
def phase1_collate_fn(batch, max_len=256, eng_pad_id=0):
    # Dynamic padding based on the longest sequence in the batch or passed max_len
    max_in = max_len
    max_tgt = 256

    bsz = len(batch)
    input_padded = torch.full((bsz, max_in), 0, dtype=torch.long)
    # ASLG items have Gloss targets (max ~2500 IDs); padding with 0 produces OOB cross-entropy.
    # Use GlossVocabulary.PAD_ID (0) as default and only override per-item if is_dae.
    target_padded = torch.full((bsz, max_tgt), 0, dtype=torch.long)  # Gloss PAD default
    ...
    for i, x in enumerate(batch):
        ...
        input_padded[i, :in_len] = (
            ...
            else torch.as_tensor(np.array(x["input_ids"][:in_len], dtype=np.int64))
        )
        target_padded[i, :tgt_len] = (
            x["target_ids"][:tgt_len]
            if isinstance(x["target_ids"], torch.Tensor)
            else torch.as_tensor(np.array(x["target_ids"][:tgt_len], dtype=np.int64))
        )
        # For DAE (BPE) items, fill unwritten positions with BPE PAD
        if x.get("is_dae", False):
            target_padded[i, len(x["target_ids"]) :] = eng_pad_id
...
def phase2_collate_fn(batch, eng_pad_id=0):

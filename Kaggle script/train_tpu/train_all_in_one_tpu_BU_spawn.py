...
def train_epoch_tpu(
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        if ema is not None and do_update:
            ...
            loss_ema.update(raw_loss_wrapper, float(epoch) / float(total_epochs))
...
def _tpu_worker_fn(rank, args):
    ...
    ema = ModelEMA(model)
    loss_ema = ModelEMA(raw_loss_wrapper)
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if ema is not None:
                ...
                loss_ema.apply_shadow(raw_loss_wrapper)
            ...
            if ema is not None:
                ema.restore(raw_m)
                loss_ema.restore(raw_loss_wrapper)
                if IS_TPU:
                    import torch_xla.core.xla_model as xm

...
class ASLShardedDataset(Dataset):
    ...
    def __init__(
        ...
        import hashlib
        self.dataset_id = f"{self.dataset_name}_{self.max_len}_{self.stride_length}_{self.channels_per_kp}_v2_{hashlib.md5(str(self.dataset_dir).encode()).hexdigest()[:8]}"
            ...
            def _index_shard(args):
                ...
                try:
                    ...
                    for key_or_idx, rec in items:
                        ...
                        print(f"DEBUG: Keeping record {f_key}/{item_idx} task={task_str} label={raw_label_str} token_ids={token_ids}")
                        local_metas.append(
                            (
                                shard_idx,
                                f_key,
...
class KDWDDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        valid_ids_cache = self.kdwd_dir / "valid_page_ids.pt"
    ...
    def _load_and_cache_tokens(self, max_samples: int = 30000):
        ...
        shm_cache_path = self.kdwd_dir / "kdwd_cached_tokens.pt"
            ...
            for line in islice(f, worker_id, None, total_workers):
                try:
                    ...
                    for sec in sections:
                        ...
                        yield {
                            "input_ids": torch.clamp(torch.tensor(corrupted_ids, dtype=torch.long), min=0, max=len(self.eng_vocab)-1),
                            "target_ids": torch.clamp(torch.tensor(text_ids, dtype=torch.long), min=0, max=len(self.eng_vocab)-1),
                            "is_dae": True,
                        }
                except Exception as e:
                    self.failures += 1
...
class ASLGPC12Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        g_st = self.gloss_offsets[idx].item()
        g_ed = self.gloss_offsets[idx + 1].item()
        t_st = self.text_offsets[idx].item()
        t_ed = self.text_offsets[idx + 1].item()

        gloss_ids = self.gloss_flat[g_st:g_ed].to(torch.long)
        text_ids = self.text_flat[t_st:t_ed].to(torch.long)

        if hasattr(self, "gloss_vocab") and hasattr(self.gloss_vocab, "vocab_size"):
            gloss_ids = torch.clamp(gloss_ids, min=0, max=self.gloss_vocab.vocab_size - 1)
        if hasattr(self, "eng_vocab") and hasattr(self.eng_vocab, "__len__"):
            text_ids = torch.clamp(text_ids, min=0, max=len(self.eng_vocab) - 1)

        if self.reverse:
            # English -> Gloss
            # Exclude BOS/EOS when flipping if necessary, but assuming simple flip here
            return {
                "input_ids": text_ids,
...
class Phase1MixedIterable(torch.utils.data.IterableDataset):
    ...
    def __len__(self) -> int:
        """Full epoch boundary ensuring 1 epoch covers 100% of ASLG-PC12 dataset."""
...
def phase1_collate_fn(batch, max_len=256, eng_pad_id=0):
    ...
    bsz = len(batch)
    input_padded = torch.full((bsz, max_in), eng_pad_id, dtype=torch.int32)
    target_padded = torch.full((bsz, max_tgt), 0, dtype=torch.int32)
    ...
    for i, x in enumerate(batch):
        ...
        input_padded[i, :in_len] = (
            ...
            else torch.as_tensor(np.array(x["input_ids"][:in_len], dtype=np.int32))
        )
        target_padded[i, :tgt_len] = (
            x["target_ids"][:tgt_len]
            if isinstance(x["target_ids"], torch.Tensor)
            else torch.as_tensor(np.array(x["target_ids"][:tgt_len], dtype=np.int32))
        )
...
def phase2_collate_fn(batch, max_len=256, eng_pad_id=0):
    ...
    bsz = len(batch)
    input_padded = torch.full((bsz, max_in), eng_pad_id, dtype=torch.int32)  # Qwen/English PAD
    target_padded = torch.full((bsz, max_tgt), 0, dtype=torch.int32)  # Gloss PAD

...
def train_epoch_tpu(
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        with torch.no_grad():
            ...
            # Throttle warmup logging to steps 1, 2, 3, 4, 5, 10 on epoch 1 to provide instant feedback and avoid per-step host syncs
            should_log = (epoch == 1 and step_idx in (1, 2, 3, 4, 5, 10)) or (step_idx % log_freq == 0) or (step_idx >= min_batches)

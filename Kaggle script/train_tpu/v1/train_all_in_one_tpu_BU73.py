...
def _tpu_worker_fn(rank, args):
    ...
    elif not is_phase1 and effective_loader_batch >= 128:
        # Only auto-enable if the network is deep (>8 layers or d_model > 512) and memory is actually constrained
        if getattr(args, "d_model", 512) > 512 or getattr(args, "num_layers", 4) > 8:
            args.gradient_checkpointing = True
            if is_master:
                print(
                    f"[INFO] Auto-enabled Gradient Checkpointing for deep architecture (layers > 8, d_model > 512).",
                    flush=True,
                )
        else:
            args.gradient_checkpointing = False
    ...
    if user_workers is not None and user_workers > 0:
        effective_num_dl_workers = min(4, user_workers)
    else:
        effective_num_dl_workers = 2 if IS_TPU else 2

...
class InGraphAugmentor(nn.Module):
    ...
    def __init__(
        ...
        if num_keypoints >= 60:
            jitter_mask[:, :, 18:60, :] = 1.0  # Hands (18:39 LH, 39:60 RH) get prioritized jitter
    ...
    def forward(
        ...
        if self.mirror_prob > 0 and K >= 60:
            ...
            # Swap Shoulders (14 <-> 15) and Elbows (16 <-> 17) for exact kinetic chain reflection
            l_sh = pos_reflected[:, :, 14:15, :]
            r_sh = pos_reflected[:, :, 15:16, :]
            l_el = pos_reflected[:, :, 16:17, :]
            r_el = pos_reflected[:, :, 17:18, :]
            ...
            pos = torch.cat([
                pos_reflected[:, :, :14, :],  # Face (0..13)
                swapped_l_sh,                 # Left Shoulder (14)
                swapped_r_sh,                 # Right Shoulder (15)
                swapped_l_el,                 # Left Elbow (16)
                swapped_r_el,                 # Right Elbow (17)
                swapped_lh,                   # Left Hand (18..38)
                swapped_rh,                   # Right Hand (39..59)
        ...
        if self.unilateral_drop_prob > 0 and K >= 60:
            ...
            pos = torch.cat([
                pos[:, :, :18, :],                     # Face (0..13) & Upper Body (14..17)
                pos[:, :, 18:39, :] * lh_keep_factor,  # Left Hand (18..38)
                pos[:, :, 39:, :],                     # Right Hand (39..59)
            ], dim=2)

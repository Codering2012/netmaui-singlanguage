...
class SignToGPT2Bridge(nn.Module):
    ...
    def forward(
        ...
        # For per-core batch >= 256 (h_flat >= 28672), use exactly 4 systolic tile-aligned chunks
        # (7,168 tokens each, exact multiple of 128) to cut backward transient memory by 2.16GB.
        if IS_TPU:
            num_chunks = 4 if h_flat.shape[0] >= 12288 else 1
        else:

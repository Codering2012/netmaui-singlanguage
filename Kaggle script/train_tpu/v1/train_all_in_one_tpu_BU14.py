...
# Persistent XLA compilation cache directory path
cache_dir = os.environ.get(
    "XLA_PERSISTENT_CACHE_PATH",
    "/kaggle/working/xla_cache" if os.path.isdir("/kaggle/working") else ("/tmp/xla_cache" if os.name != "nt" else "./xla_cache")
)
            ...
            for layer in self.decoder.layers:
                if getattr(self, "gradient_checkpointing", False) and self.training:
                    ...
                    h_fused = torch.utils.checkpoint.checkpoint(
                        _ckpt_dec_layer, layer, h_fused, mem_fused, enc_mask_fused, use_reentrant=False
                    )
                else:
...
def train_epoch_tpu(
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        with torch.no_grad():
            ...
            should_log = (step_idx <= 10) or (step_idx % log_freq == 0) or (step_idx >= min_batches)
            if is_xla:
                if should_log:
                    args_tuple = (
                        metrics_vec,
...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        try:
            _default_cache = (
                "/kaggle/working/xla_cache"
                if os.path.isdir("/kaggle/working")
                else ("/tmp/xla_cache" if os.name != "nt" else "./xla_cache")
            )
            ...
            if rank == 0:
                print(f"[XLA] Compilation disk cache active: {rank_cache} (Per-rank cache enabled)", flush=True)
    ...
    if IS_TPU:
        ...
        if is_gpt2_active:
            # GPT-2 has vocab 50257 with 12 layers (768-dim).
            # At batch 32 per core: peak HBM is ~5.3 GB (comfortably below 15.75 GB).
            # Batch 32 * 256 = 8,192 tokens (exact multiple of 128 systolic array tile: 64 x 128).
            # At batch 128 per core: lm_head alone is 3.1GB logits + 3.1GB grad + 9GB activations = 19.4GB (>15.75GB, fatal HBM OOM).
            max_safe_batch = 32
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Enable bidirectional Phase 1 knowledge distillation and cycle consistency (English <-> ASL Gloss).",
    )
    parser.add_argument(

    ...
    def forward(
        ...
        if getattr(self, "gpt2_bridge", None) is not None and english_seq is not None:
            try:
                ...
                if has_valid_english is not None and (has_valid_english.float().sum() == 0):
                    gpt2_loss = torch.zeros((), device=h_seq.device, dtype=h_seq.dtype)
                elif gpt2_loss is not None:
...
def validate_epoch_tpu(
    ...
    total_val_batches = get_dynamic_loader_len(loader, default_steps=(val_limit if val_limit > 0 else 500), args=args)
    if val_limit > 0:

...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # If english_max_len <= 128, attention matrix is 4x-16x smaller than L=256,
            # allowing batch 128 (1024 global batch, 128x128 systolic MXU tile) inside ~7.5GB HBM.
            # If english_max_len > 128 (e.g. 256), cap to 32 to prevent OOM.
            eng_len = getattr(args, "english_max_len", 128)
            max_safe_batch = 128 if eng_len <= 128 else 32
        else:
            max_safe_batch = 128
    else:
        # GPU: 16-24GB VRAM
    ...
    if user_core_batch > 0:
        ...
        eng_len = getattr(args, "english_max_len", 128)
        max_allowed_tpu = 128 if (not is_gpt2_active or eng_len <= 128) else 32
        if IS_TPU and user_core_batch > max_allowed_tpu:
            if is_master:
                print(
                    ...
                    f"Capping to {max_allowed_tpu}. Running with global batch {max_allowed_tpu * world_size} (~7.5GB HBM).",
                    flush=True,
                )

...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # When GPT-2 is active (124M params, 12 layers, 50k vocab), forward activations
            # and prefix backprop require ~7.1GB HBM at batch 32. Batch 128 requires 21.3GB (exceeds 15.75GB limit).
            max_safe_batch = 32
        else:
            max_safe_batch = 128
    else:
    ...
    if user_core_batch > 0:
        ...
        max_allowed_tpu = 32 if is_gpt2_active else 128
        if IS_TPU and user_core_batch > max_allowed_tpu:
            if is_master:
                print(
                    ...
                    f"Capping to {max_allowed_tpu}. Running with global batch {max_allowed_tpu * world_size} (~7.1GB HBM).",
                    flush=True,
                )

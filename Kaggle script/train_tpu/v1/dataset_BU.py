...
class ASLStreamedDataset(IterableDataset):
    ...
    def _process_record(
        ...
        # Sanitize out-of-bounds, exploded, or collapsed tracking artifacts on both hands
        if self.num_keypoints >= 42 and T_cap > 0:
            for h_start, h_end in [(0, 21), (21, 42)]:
                h_pos = feat_out[:T_cap, h_start:h_end, :3].float()
                wrist = h_pos[:, 0:1, :]

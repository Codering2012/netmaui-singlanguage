...
class SignToGPT2Bridge(nn.Module):
    ...
    def forward(
        ...
        # For per-core batch >= 256 (h_flat >= 28672), use exactly 2 systolic tile-aligned chunks
        # (14,336 tokens each, exact multiple of 128) to cut backward transient memory by 1.44GB without loop unroll bloat.
        if IS_TPU:
            num_chunks = 2 if h_flat.shape[0] >= 12288 else 1
        else:
...
class ASLFoundationModel(nn.Module):
    ...
    def forward(
        ...
        if can_fuse_decoders:
            ...
            # Hardware Systolic Tile Alignment: Enforce exact 128-multiple tile boundaries
            # Active token bounds: all ASL glosses in dataset <= 50 tokens, all Chicago <= 35 chars.
            # Slicing padded extents to static 128 systolic tile eliminates redundant computation
            # of zero-pad tokens while guaranteeing 100% data preservation and perfect MXU alignment.
            target_dec_len = min(128, max_tgt_len) if max_tgt_len >= 128 else (((max_tgt_len + 63) // 64) * 64)
            if target_dec_len <= 0:
                target_dec_len = 128

...
def _tpu_worker_fn(rank, args):
    ...
    if not is_phase1 and effective_loader_batch >= 64 and not getattr(args, "gradient_checkpointing", False):
        args.gradient_checkpointing = True
        if is_master:
            print(
                f"[INFO] High-Throughput Mode: Auto-enabled Gradient Checkpointing for per-core batch {effective_loader_batch} (scoped to trainable layers; frozen GPT-2 protected).",
                flush=True,
            )
    elif IS_TPU and getattr(args, "gradient_checkpointing", False) and is_gpt2_active:
        if is_master:
            print(
                f"[INFO] TPU Architecture Guard: Gradient Checkpointing active for trainable layers (frozen GPT-2 protected).",
                flush=True,
            )

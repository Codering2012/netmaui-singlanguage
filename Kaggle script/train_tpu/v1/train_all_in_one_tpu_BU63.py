...
class SignToGPT2Bridge(nn.Module):
    ...
    def forward(
        ...
        if IS_TPU:
            num_chunks = 4 if h_flat.shape[0] >= 24576 else (2 if h_flat.shape[0] >= 12288 else 1)
        else:
...
class ASLFoundationModel(nn.Module):
    ...
    def forward(
        ...
        if can_fuse_decoders:
            ...
            # Hardware Systolic Tile Alignment: Align to 64 if max_tgt_len <= 64, else 128.
            # With B_fused = 2*B = 512, 512 * 64 = 32,768 = 256 * 128 (exact systolic MXU alignment, saving >730MB HBM).
            if max_tgt_len <= 64:
                aligned_tgt_len = ((max_tgt_len + 63) // 64) * 64 if max_tgt_len > 0 else 64
            else:
                aligned_tgt_len = ((max_tgt_len + 127) // 128) * 128
            max_tgt_len = max(max_tgt_len, aligned_tgt_len)

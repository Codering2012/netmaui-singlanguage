    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            if use_cache:
                ...
                if getattr(self, "gradient_checkpointing", False) and self.training and not IS_TPU:
                    ...
                    hidden_h = torch.utils.checkpoint.checkpoint(
                        _ckpt_dec_layer_standalone, layer, hidden_h, memory, tgt_key_padding_mask, memory_key_padding_mask, use_reentrant=False
                    )
                else:
                    hidden_h = layer(
                        hidden_h,
        ...
        for idx, block in enumerate(self.blocks):
            if isinstance(block, TemporalStridedPool):
                ...
                if getattr(self, "gradient_checkpointing", False) and self.training and not IS_TPU:
                    ...
                    hidden_h = torch.utils.checkpoint.checkpoint(
                        _ckpt_enc_block, block, hidden_h, kpm, pos_fi, use_reentrant=False
                    )
                else:
...
def _tpu_worker_fn(rank, args):
    ...
    if not IS_TPU and not is_phase1 and effective_loader_batch >= 64 and not getattr(args, "gradient_checkpointing", False):
        ...
        if is_master:
            print(
                f"[INFO] TPU Architecture Guard: Gradient Checkpointing scoped to trainable Transformer Decoder layers only (frozen GPT-2 and Encoder protected).",
                flush=True,
            )

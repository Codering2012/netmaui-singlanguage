...
def _tpu_worker_fn(rank, args):
    ...
    if IS_TPU:
        ...
        if is_phase1:
            ...
            # With frozen GPT-2 (124M, requires_grad=False), backward activations are not saved.
            # Per-core batch 64 uses ~4.5GB HBM (global 512). Per-core batch 128 uses ~8.5GB HBM (global 1024).
            max_safe_batch = 128 if getattr(args, "freeze_gpt2", True) else 32
        else:
            max_safe_batch = 128
    else:
        # GPU: 16-24GB VRAM
    ...
    if user_core_batch > 0:
        ...
        max_allowed_tpu = 128 if (not is_gpt2_active or getattr(args, "freeze_gpt2", True)) else 32
        if IS_TPU and user_core_batch > max_allowed_tpu:
            if is_master:
                print(
                    f"[!] WARNING: --per-core-batch={user_core_batch} exceeds TPU v5e safety limit ({max_allowed_tpu}). "
        ...
        if IS_TPU:
            if is_phase1:
                ...
                target_per_core_batch = 64 if getattr(args, "freeze_gpt2", True) else 32
            else:

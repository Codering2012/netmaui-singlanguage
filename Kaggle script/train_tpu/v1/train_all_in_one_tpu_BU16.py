...
class ASLFoundationModel(nn.Module):
    ...
    def _encode(
        ...
        if x_in.dim() == 4 and x_in.size(2) == 60:
            if x_in.size(3) == 3:
                # Dynamic on-the-fly 9-channel kinematics: [position, velocity, acceleration]
                v = torch.zeros_like(x_in)
                v[:, 1:] = x_in[:, 1:] - x_in[:, :-1]
    ...
    def forward(
        ...
        # Compute output heads (Gated to eliminate dead graph allocations & compile overhead)
        if getattr(self, "disable_ctc", False):
            ctc_log_probs = None
        ...
        # Phonological Predictions (Pruned during training to eliminate dead autograd graph nodes)
        if self.training:
            pred_handshape, pred_location, pred_signtype = None, None, None
        else:
            pred_handshape = self.head_handshape(h_cls)
            pred_location = self.head_location(h_cls)
            pred_signtype = self.head_signtype(h_cls)

        # Gated Auxiliary Projections (Pruned when auxiliary losses are disabled)
        if getattr(self, "disable_aux_heads", False) and self.training:
            vis_emb = None
            proj_feats = None
            domain_logits = None
        else:
        ...
        sent_emb = None
        if not (getattr(self, "disable_aux_heads", False) and self.training):
            if gloss_seq is not None and self.decoder is not None:
                tgt = gloss_seq[:, 1:]  # [B, T]
                non_pad = (
    ...
    for step_idx, batch in enumerate(para_loader, start=1):
        ...
        def forward_and_losses(
            ...
            out = model(
                ...
                compute_lpc=(not getattr(args, "disable_lpc", False)),
            ...
            if getattr(args, "disable_lpc", False):
                loss_lpc = torch.zeros((), device=device)

            h_cls = out.get("h_cls")
            if getattr(args, "disable_aux_losses", False):
            ...
            if sent_emb is not None and vis_emb is not None and not getattr(args, "disable_aux_losses", False):
            ...
            if proj_feats is not None and not getattr(args, "disable_aux_losses", False):
            ...
            if domain_logits is not None and not getattr(args, "disable_aux_losses", False):
            ...
            if orig_x is not None and hasattr(raw_model, "bone_loss_fn") and not getattr(args, "disable_aux_losses", False):
            ...
            if getattr(args, "enable_phase1_distill", False) and not is_gpt2_mode and not getattr(args, "disable_aux_losses", False):
        ...
        raw_m = model.module if hasattr(model, "module") else model
        if hasattr(raw_m, "dense_sem_loss") and do_update and not getattr(args, "disable_aux_losses", False):
...
def _tpu_worker_fn(rank, args):
    ...
    if target_per_core_batch > max_safe_batch:
        ...
        if IS_TPU:
            # Keep accum_steps=1 on TPU to prevent dual compiled graphs host RAM overhead
            auto_accum = 1
        else:
    ...
    # Auto-enable gradient checkpointing only when effective batch >= 128 per core
    if IS_TPU and not is_phase1 and effective_loader_batch >= 128 and not getattr(args, "gradient_checkpointing", False):
        ...
        if is_master:
            ...
            if IS_TPU and auto_accum == 1:
                print(
                    ...
                    f"(Requested total: {requested_batch_size}, World size: {world_size}, MaxLen: {args.max_len}, BPE: {bpe_val}). "
                    f"Keeping accum_steps=1 on TPU to prevent dual-XLA-graph host RAM explosion (2 graphs × 8 processes × ~30GB = ~480GB > 330GB). "
                    f"Effective step batch: {effective_total} sequences.",
                    flush=True,
                )
            else:
                print(
    ...
    model.disable_ctc = getattr(args, "disable_ctc", False)
    model.disable_aux_heads = getattr(args, "disable_aux_losses", False)
...
def main():
    ...
    parser.add_argument(
        "--disable-phase1-distill",
        action="store_false",
        dest="enable_phase1_distill",
        help="Disable bidirectional Phase 1 knowledge distillation.",
    )
    parser.add_argument(
        "--disable-lpc",
        action="store_true",
        default=False,
        help="Disable Linear Predictive Coding (LPC) smoothness loss to prune graph size and reduce compute.",
    )
    parser.add_argument(
        "--disable-aux-losses",
        action="store_true",
        default=False,
        help="Disable auxiliary contrastive, dense semantic, domain, and bone losses for ultra-fast, minimal XLA computation graph.",
    )
    parser.add_argument(
        "--light-graph",
        action="store_true",
        default=False,
        help="Enable Ultra-Light XLA Graph mode: automatically disables CTC, LPC, MLM, teacher distillation, and auxiliary contrastive losses for minimal compile time, tiny HLO IR, and maximum execution speed.",
    )
    ...
    if getattr(args, "light_graph", False):
        args.disable_ctc = True
        args.disable_lpc = True
        args.disable_aux_losses = True
        args.enable_phase1_distill = False
        print("[INFO] Ultra-Light XLA Graph Mode Active: Disabled CTC, LPC, MLM, Teacher Distillation, and Auxiliary Contrastive losses.", flush=True)
